...
if(($_SESSION['eventInfo']['event_selected'] =="")){    header("location:event_selection_uat.php");	exit; }
if($_SESSION['eventInfo']['event_selected']!='combo'){  header("location:event_selection_uat.php"); exit; }
	...
	last_yr_participant=$("#last_yr_signature_participant").val();
		$.ajax({ 		
					...
					success: function(data){
								...
								$('#signature_sub_total_cost').val(data[1]);
								$('#signature_security_deposit').val(data[2]);
								$('#signature_govt_service_tax').val(data[3]);
								$('#signature_grand_total').val(data[4]);
...
<script>
  $("#signature_option").live('change',function(){
	  ...
	  else if(option=="Same stall position as off IIJS premiere 2022")
	  {
	  ...
	  var lastYearSignatureArea=$('#lastYearSignatureArea').val();
	  var lastYearArea=$('#lastYearArea').val();
	  $("#im_registration_no").attr("disabled", "disabled"); 
	  ...
	  $('#signature_grand_total').val('');
		...
		$.ajax({
					type: 'POST',
					url: 'signature_combo_ajax.php',
					data: "actiontype=optionValue&&option="+option+"&&lastYearArea="+lastYearArea+"&&lastYearSignatureArea="+
					lastYearSignatureArea+"&&section="+section+"&signature_selected_scheme_type="+signature_selected_scheme_type,
					dataType:'html',
					beforeSend: function(){
							$('#preloader').show();
							$('#status').show();
					...
					success: function(data){
							...
							if($.trim(res[2])!=""){
								$('#signature_area_hid').val(res[2]).html(res[2]);
...
$(document).ready(function(){
 ...
 //    $.ajax({ type: 'POST',
	...
	// }); 
  $("#tritiya_area").on('change',function(){
	 	$('#tritiya_tot_space_cost_rate').val('');
	  $('#tritiya_sub_total_cost').val('');
	  $('#tritiya_security_deposit').val('');
	  $('#tritiya_govt_service_tax').val('');
	  $('#tritiya_grand_total').val('');
	...
	selected_area=$("#tritiya_area").val();
	section=$("#tritiya_section").val();
	category=$('#tritiya_category').val();
  selected_premium_type=$("#tritiya_selected_premium_type").val();
  woman_entrepreneurs=$('#tritiya_woman_entrepreneurs').val();
	last_yr_participant=$("#last_yr_tritiya_participant").val();
   // alert(selected_area);
	if(selected_area ==""){
		return false;
	}
	if(section ==""){
		...
		$.ajax({ 		
					...
					success: function(data){
								...
								// alert();
								$('#submitTritiya').attr('disabled',false);
								$('#tritiya_tot_space_cost_rate').val(data[0]);								
								// $('#signature_tot_space_cost_discount').val(data[1]);
								// $('#signature_get_tot_space_cost_rate').val(data[2]);
								// $('#signature_get_category_rate').val(data[3]);								
								// $('#signature_selected_scheme_rate').val(data[4]);
								// $('#signature_selected_premium_rate').val(data[5]); 
								$('#tritiya_sub_total_cost').val(data[1]);
								$('#tritiya_security_deposit').val(data[2]);
								$('#tritiya_govt_service_tax').val(data[3]);
								$('#tritiya_grand_total').val(data[4]);
		...
		beforeSend: function(){
			...
			success:function(result){
				...
				if(result.status==result.name+'_empty'){
					...
					$("#submitSignature").attr('value',"Submit IIJS Tritiya 2023 Data");
...
<div class="inner_container">
    ...
    <div class="form_wrp">	
        <p style="text-align: center;"><strong>Note:</strong><span  style="color: red"> Kindly submit the IIJS SIGNATURE 2023 form first,in order to submit the IIJS TRITIYA 2023 Form</span></p>
        <div style="text-align: center;margin-bottom:10px "></div>

  <!----------------------------------------------  IIJS PREMIERE START --------------------------------------------------------------------------->

    <form  method="post" name="form1" id="form1" style="width:32%; float:left; border-right:2px dotted #ddd; box-sizing:border-box; display:none;" >
	...
	$last_query = "select * from exh_registration where uid='$registration_id' AND `show`='IIJS PREMIERE 2022' AND last_yr_section_flag='1' AND  curr_last_yr_check='N' order by exh_id desc limit 0,1";
	...
	$query=$conn->query("select * from exh_registration where uid='$registration_id' AND `show`='IIJS PREIERE 2022'");
	...
	?>
       ...
       <div style="margin-top:175"></div>
        ...
        <?php if($selected_area!=""){?><input type="hidden" name="selected_area_hid" id="selected_area_hid" value="<?php echo $selected_area;?>"/><?php }?>
	...
	<!----------------------------------------------  IIJS PREMIERE END --------------------------------------------------------------------------->
	<!-- ======================================================================================================================================= -->
	<!----------------------------------------------  IIJS SIGNATURE START--------------------------------------------------------------------------->
	
	<form method="post" name="form2" id="form2" style="width:49%; float:left;" class="">
  ...
  <?php 
	/*...........................Last year participant  ..........................*/
	//echo "select * from exh_registration where uid='$registration_id' and `show`='IIJS Signature 2019' and last_yr_section_flag='1' and  curr_last_yr_check='N' order by exh_id desc limit 0,1";
	$signQuery = "select * from exh_registration where uid='$registration_id' and `show`='IIJS SIGNATURE 2022' and last_yr_section_flag='1' and  curr_last_yr_check='N' order by exh_id desc limit 0,1";
	...
	$result_signature1['last_yr_participant'];
	...
	?>
        ...
        <?php if($num2>0){ ?>
        <div class="field_box">
            <div class="field_name">IIJS SIGNATURE 2022 details :</div>
            <div class="field_input" style="padding-top:5px; line-height:25px;">
        ...
        <?php if($num1>0){ ?>
         <div class="field_box">
            <div class="field_name">IIJS PREMIERE 2022 details :</div>
            <div class="field_input" style="padding-top:5px; line-height:25px;">
            <strong>Section :</strong> <?php echo strtoupper($result1['section']);?><br />
            <strong>Area :</strong> <?php echo strtoupper($result1['selected_area']);?> <br />
            <strong>Category type :</strong> <?php echo $result1['category'];?> <br />
            <strong>Premium type :</strong> <?php echo strtoupper($result1['selected_premium_type']);?>           
            </div>
            <div class="clear"></div>
        </div>
      <?php } ?>
        ...
        <div style="margin-top:175"></div>
        <?php } else { ?>
         <br/><input type="radio" id="signature_option" class="bgcolor" name="signature_option" value="Same stall position as off IIJS premiere 2022" <?php if($signature_option=="Same stall position as off IIJS premiere 2022"){?> checked="checked" <?php } ?> /> Same stall position as off IIJS premiere 2022        
        <br/><input type="radio" id="signature_option" class="bgcolor" name="signature_option" value="Same area but different location as of previous year Signature" <?php /*if($signature_option=="Same area but different location as of previous year Signature"){?> checked="checked" <?php } */?>/> 
        Same area but different location as of previous year Signature 2022.<br/>		
        <input type="radio" id="signature_option" class="bgcolor" name="signature_option" value="More area than previous year Signature" <?php /*if($signature_option=="More area than previous year Signature"){ ?> checked="checked" <?php } */?>  /> More area than previous year Signature 2022.<br/>
		<?php if($signature_area!=9){ ?>
        <input type="radio" id="signature_option" class="bgcolor" name="signature_option" value="Less area as previous year" <?php /*if($signature_option=="Less area as previous year"){?> checked="checked" <?php } */?>/> Less area as previous year Signature 2022. <?php } ?>
		<?php } ?>
        ...
        <div class="field_input">
          ...
          <option selected="selected" value="">-----Select Category----</option>
            <?php 
			$sql="SELECT * FROM signature_category_master where status='1'";
            $query=$conn->query($sql);
            while($result=$query->fetch_assoc()){ ?>
        ...
        <div class="field_input">
            ...
            <option selected="selected" value="">-----Select Premium Type----</option>
			<?php 
			$sql="SELECT * FROM signature_premium_master where status='1' order by premium_id  asc";
			$query=$conn->query($sql);
			while($result=$query->fetch_assoc()){
            ?>
  ...
  <!----------------------------------------------  IIJS TRITIYA  START ---------------------------------------------------------------------- -->
  <!--  <div class="clear"></div> -->
    <form  method="post" name="form3" id="form3" style="width:49%; float:left;" class="">
	...
	?>
        <!-- <div class="field_box">
            <div class="field_name">Last year participant : <?php if($result_tritiya1['last_yr_participant']=='YES'){ echo $last_yr_signature="YES"; } else {echo $last_yr_signature="NO";}?></div>
              ...
              <input type="hidden" name="last_yr_tritiya_participant" id="last_yr_tritiya_participant" value="NO"/>
        ...
        <div class="field_input"> 
          <select name="tritiya_area" id="tritiya_area" class="bgcolor" <?php if($tritiya_area!="" && $num2!=0){?> disabled="disabled"<?php }?>>
            ...
            <?php
			//if($num2==0){ $sql="SELECT * FROM signature_area_master where area=9"; } else {
			$sql="SELECT * FROM signature_area_master order by `order` asc"; //}
			$query=$conn->query($sql);
			while($result=$query->fetch_assoc()){ ?>
        ...
        <div class="clear"></div>
         ...
         <div class="field_box">
        <div class="field_name">Security Deposit (10% on SubTotal Cost) <?php echo $currency;?> :</div>
        <div class="field_input"><input type="text" class="bgcolor" name="tritiya_security_deposit" id="tritiya_security_deposit" readonly="readonly" value="<?php echo $tritiya_security_deposit;?>" /></div>
        <label for="tritiya_security_deposit" generated="true" class="error"></label>
        <div class="clear"></div>
        </div>  
        <div class="field_box">
        <div class="field_name">GST (18% on SubTotal Cost) <?php echo $currency;?> :</div>
  ...
  <!----------------------------------------------  IIJS TRITIYA  END ---------------------------------------------------------------------- -->
	...
	<a href="exhibitor_registration_step_2.php" class="button">Prev</a>
	<a href="exhibitor_registration_step_4_combo.php" class="button" id="step4combo" style="display: none;">Submit Application </a>
    <input type="submit" id="submitSignature" class="button" value="Submit IIJS SIGNATURE 2023 Information" disabled='disabled' />
    </div>
