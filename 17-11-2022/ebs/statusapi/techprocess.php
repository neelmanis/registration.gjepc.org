...
$date=date("Y-m-d");
//echo "select * from visitor_order_detail where payment_status='P' AND regId='$registration_id' AND (event='signature23' || event='stcombo23') AND payment_type='online'"; exit;
$q=$conn->query("select * from visitor_order_detail where payment_status='P' AND regId='$registration_id' AND 
(event='signature23' || event='stcombo23') AND payment_type='online'");
...
while($r=$q->fetch_assoc()){
	...
	*/
	...
	//	$ssx = "SELECT * FROM `visitor_order_temp` WHERE `registration_id`='$registration_id' AND `show`='$show' AND `year`='$year' AND `paymentThrough`='online' AND orderId='$orderId'";
		$ssx = "SELECT * FROM `visitor_order_temp` WHERE `registration_id`='$registration_id' AND `paymentThrough`='online' AND orderId='$orderId'";
