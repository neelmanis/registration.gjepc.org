...
//	$amountPaid = $payment->amountPaid;
	...
	if($payment['status'] == 'captured'){
		...
		$update_query_sig = "update exh_reg_payment_details set modified_date=NOW(), payment_status='approved', document_status='approved',application_status='approved',application_comment='stepwise' where gid='$gid_signature' and uid='$registration_id' and `show`='IIJS SIGNATURE 2023'";
		$result_update_sig =$conn ->query($update_query_sig);
		}
		if($eventDescription == "IIJS TRITIYA 2023"){
		$update_query_tri = "update exh_reg_payment_details set modified_date=NOW(), payment_status='approved', document_status='approved',application_status='approved',application_comment='stepwise' where gid='$gid_tritiya' and uid='$registration_id' and `show`='IIJS TRITIYA 2023'";
