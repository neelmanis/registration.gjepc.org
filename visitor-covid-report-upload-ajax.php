...
if($action == 'pan_number'){
	...
	if($count>0) 
	{
		...
		if($status =='Y'){
            ...
            $checkHistory = "SELECT * FROM `visitor_order_history` WHERE visitor_id='$visitorID' AND (payment_made_for='signature23' OR payment_made_for='iijs22' OR payment_made_for='combo23'  OR payment_made_for='igjme22' OR payment_made_for ='iijstritiya23' ) AND `status`='1' AND payment_status='Y'";			 
			$getQuery = $conn ->query($checkHistory);
			$checkResult = $getQuery->num_rows;
...
if(isset($_POST['action']) && $_POST['action']=="UpdateCovidReport")
{
	...
	$category_for = getVisitorSelectedMultipleShow($visitor_id,$conn);
