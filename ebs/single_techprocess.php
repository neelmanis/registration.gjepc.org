...
{	
	...
	$custname = $custname.'~'.$registration_id.'~'.$visitor_id;
	
	$orderUpdate ="SELECT * FROM `visitor_order_detail` WHERE orderId='$orderId' AND regId='$registration_id' AND paymentThrough='single' AND payment_status='P'";
	...
	$tpslUpdate ="update visitor_order_detail set tpsl_txn_id_rand='$tpsl_txn_id' WHERE orderId='$orderId' AND regId='$registration_id' AND paymentThrough='single'";
	$tpslResult = $conn->query($tpslUpdate);
	
	$mrctCode = "L278776";  // LIVE
	$tpvAccntNo = "";
	//$itc = "NIC~TXN0001~122333~rt14154~8 mar 2014~Payment~forpayment";
	$itc = $registration_id . '~' . $visitor_id;
	$reqType = "T";
	//$mrctTxtID = $strNo;
	$mrctTxtID = $orderId;
	$currencyType = "INR";
	$returnURL = "https://" . $_SERVER['HTTP_HOST']."/single_payment_success.php";
