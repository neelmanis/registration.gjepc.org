...
$closetime = strtotime("25-01-2020 23:55:00");
?>
	<?php
	/* IVR Start */
...
if($saveUTR=="saveinfo")
{
	...
	elseif(empty($amountPaid)) { $amountPaidError = "Plz Enter Amount Paid"; }
		...
		if($event=="signature"){ $event_for="IIJS Signature 2020"; } 
		if($event=="iijs") { $event_for="IIJS 2020"; }
		...
		if($countUTR > 0)
			...
			$insertUTR = "INSERT INTO `utr_history`(`post_date`, `registration_id`,`gcode`, `utr_number`,`amountPaid`, `tdsAmount`, `show`,`event_selected`, `status`) VALUES (NOW(),'$registration_id','$gcodes','$utr_number','$amountPaid','$tdsAmount','$event_for','$event','1')";
			...
			<div class="inner_container">
				...
				<div class="content_area">
					...
					{	
						...
						echo '<a href="member_obmp_profile.php" class="btn orange" title="Select Show And Make Payment"><span>Step 2</span>Registration Application</a>';
						}
						...
						//echo "<p><a href='#' id='trigger'> *** Domestic Visitor Note</a></p><h2>Domestic Visitor Registration</h2><br><br>";
						echo "<div class='box_dash_container'>
						<div class='box_title'><h3>Airline and Hotel booking</h3></div>
						<div class='box_dash'>";						
						echo '<a href="airline_booking.php" class="btn orange" title="Registration Summary">Airline booking</a>';	
						echo '<a href="hotel_booking.php" class="btn orange" title="Registration Summary">Hotel booking </a>';
						echo '</div></div>';
					...
					<div class="box_dash_container">						
						...
						<?php
						$sql = "select * from exh_reg_payment_details where uid='$registration_id' and `year`='2019' order by payment_id desc limit 0,1";
						$ans = mysql_query($sql);
						$result = mysql_fetch_array($ans);
						$gid = $result['gid'];
						$nans=mysql_num_rows($ans);
					...
					//$sqlShow = "select * from exh_reg_payment_details where uid='$registration_id' AND `show`='IIJS Signature 2020' order by payment_id desc limit 0,2";
						...
						<table id="table_data">
							<tr>
								...
								if($event_selected=="signature"){ $event_for="IIJS Signature 2020"; } 
								if($event_selected=="iijs") { $event_for="IIJS 2020"; }
								?>
								<tr>
						...
						<?php
						$utrExistTotal = "SELECT sum(amountPaid),sum(tdsAmount) FROM `utr_history` WHERE registration_id='$registration_id' AND `event_selected`='signature'";
						...
						<table id="table_data">
							<tr>
							<th align="center">Total Amount Paid (IIJS Signature 2020)</th>
						...
						$existIIJSResultTotal = mysql_query($utrIIJSExistTotal);
						$countTotals  = mysql_num_rows($existIIJSResultTotal);
						if($countTotals > 0) {
						?>
						<table id="table_data">
							<tr>
							...
							<tbody>
							<?php	
							while($print_iijsutr = mysql_fetch_array($existIIJSResultTotal))
							{	//print_r($print_iijsutr);			
							?>
							<tr>
						...
						/*	$q_last_yr=mysql_query("select * from exh_registration where uid='$registration_id' and `show`='IIJS 2019' and section='machinery' order by exh_id desc limit 0,1");
							...
							echo '<a href="#" class="select_dash_btn btn">Application Closed</a>'; ?>
						...
						?>
						<?php
						$query1=mysql_query("select * from exh_registration where uid='$registration_id' and `show`='IIJS 2019' and last_yr_section_flag='1' and  curr_last_yr_check='N' order by exh_id desc limit 0,1");
