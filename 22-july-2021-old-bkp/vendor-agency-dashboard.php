				...
				<div class="content_area box-shadow">
					...
					<div class='d-block mt-3 text-center '>
						<?php if(isset($_REQUEST['action']) && $_REQUEST['action']=='view'){?>
							<a href="vendor-agency-dashboard.php?action=addnew" class="btn col-100 ">Add Visitor</a>
						<?php }else if(isset($_REQUEST['action']) && $_REQUEST['action']=='addnew'){?>
							<a href="vendor-agency-dashboard.php?action=view" class="btn col-100 ">View Visitors</a>
						<?php }else if($_REQUEST['action']=="upload-covid-report"){?>
							<a href="vendor-agency-dashboard.php?action=addnew" class="btn col-100 ">Add Visitor</a>
							<a href="vendor-agency-dashboard.php?action=view" class="btn col-100 ">Back </a>
						<?php }else{?>
							<a href="vendor-agency-dashboard.php?action=addnew" class="btn col-100 ">Add Visitor</a>
						<?php }?>							
					</div>
			    	...
			    	<?php if(isset($_REQUEST['action']) && $_REQUEST['action']=='addnew'){?>
						...
						<div class="field">
					        ...
					        <input name="agency_name" id="agency_name" type="text" class="textField" value="<?php echo $_SESSION['COMPANYNAME']?>" autocomplete="off" readonly />
					    </div>
					    <div class="field">
				        ...
				        <div class="field">
						    ...
						    <input type="tet" name="category" id="category" value="<?php echo getVisitorAgencyCategory($rowcompanyData['category'],$conn);?>" class="textField" readonly="readonly">
						...
						<div class="field" <?php if($rowcompanyData['category']=="2"){?> style="display: block;"<?php }else{?> style="display: none;" <?php }?>>
						    ...
						    <option value="">---Select One---</option>
						    <option value="Chairman (All Access)" <?php if($title=="Chairman (All Access)") echo "selected"; ?>> Chairman (All Access) </option>
						    <option value="Vice-Chairman (All Access)" <?php if($title=="Vice-Chairman (All Access)") echo "selected"; ?>> Vice-Chairman (All Access) </option>
						    <option value="Committee Member" <?php if($title=="Committee Member") echo "selected"; ?>> Committee Member </option>
						    <option value="Committee Member (All Access)" <?php if($title=="Committee Member (All Access)") echo "selected"; ?>> Committee Member (All Access) </option>
						    <option value="Convener" <?php if($title=="Convener") echo "selected"; ?>> Convener </option>
						    <option value="Co-Convener" <?php if($title=="Co-Convener") echo "selected"; ?>> Co-Convener </option>
                        ...
                        <div class="field">
						    ...
						    <select name="payment_made_for" id="payment_made_for" class="selectField">
						     <option value="signature2" selected="selected">IIJS SIGNATURE 2021 Show</option>
						    </select>
						</div>
						 <div class="field">
			        ...
			        <?php if(!isset($_REQUEST['action']) || $_REQUEST['action']=='view'){?>
			        	...
			        	<div class="d-block p-2">
			        		<h3>Registered Visitors for IIJS Signature 2021</h3>
                                       	...
                                       	<?php 
                                            ...
                                            switch ($covid_status) {
                                            	case 'P':
                                            		...
                                            		break;
                                            	case 'Y':
                                            		$covid_result = "<span class=' label success'>Negative</span>"; 
                                            		break;
                                            	case 'N':
                                            		$covid_result = "<span class=' label danger'>Positive</span>"; 
                                            		break;		
                                            	default:
                                            		$covid_result = "<span class=' label warning'>Pending</span>"; 
                                            		break;
			        ...
			        <?php if(isset($_REQUEST['action']) && $_REQUEST['action']=='upload-covid-report' && isset($_REQUEST['agency-visitor']) && $covid_report_link == "Y" ){
			            ...
			            if($rowCheckApproval['person_status'] =='Y'){ ?>
			            	...
			            	<form class="p-2" method="POST" name="covidReportForm" id="covidReportForm" enctype="multipart/form-data" autocomplete="off">
						    <input type="hidden" name="action" value="covid-report-upload-action">
						    <input type="hidden" name="visitor" value="<?php echo $_REQUEST['agency-visitor'];?>">
						   
						    <div class="clear"></div>

						    <div class="d-block col-100">
						    	<h3>Covid Report Details</h3>
						    	<ul class="inner_under_listing">
						    		<li>RT-PCR Test report (Covid-19 test)</li>
						    		<li>It is mandatory to submit RT-PCR test report 72 hours before visit date</li>
						    ...
						    <div class="clear"></div>
						     <div class="field">
					        <label><b>Upload Covid Report:</b> <sup>*</sup><br />&nbsp;</label>
					        <input name="self_report" id="self_report" type="file" class="textField" autocomplete="off" accept=".png,.pdf,.jpg,.jpeg" />
					        <i>Use only Jpg,png, pdf files. Max file size 2MB </i>
					        <label for="self_report" generated="true" class="error" style="display: none;margin-top: 5px"> </label>
					    </div>
					    <div class="clear"></div>
					    <div class="col-100 mt-2">
							<input type="checkbox" name="agreeNegative" value="YES"> I declare that my COVID report is negative
               			    
               			    <div class="d-block"><label for="agreeNegative" generated="true" class="error" style="display: none;margin-top: 5px"></label></div>
						</div>
						<div class="clear"></div>
						<div class="col-100 mt-2">
							<label style="display:block"><input type="checkbox" name="agreeCovid" value="YES"> I declare that my covid report can be share with GJEPC</label>
							
               			   
       			    <div><label for="agreeCovid" generated="true" class="error" style="display: none;margin-top: 5px"></label></div>
						</div>
						
					     <div class="clear"></div>
					     <div class="d-block mt-3">
                          <input type="submit" name="submit" id="submit" value="Submit Report" class="cta">
					      </div>
