...
<div class="inner_container">
    ...
    <div class="form_main">  
    <div class="form_title">Print Acknowledgement <div class="clear"></div></div>
	<?php	
    $sql_signature ="select * from exh_registration where uid='$uid' AND curr_last_yr_check='Y' AND `show`='IIJS 2021' order by gid desc";
    $query_signature = $conn->query($sql_signature);		
	$num1=$query_signature->num_rows;		
    if($num1>0)
	{
    while ($result_signature = $query_signature->fetch_assoc()) {
	if($result_signature['event_selected']=="signature"){ $event_for="IIJS Signature 2021"; } 
	...
	<p><strong style="text-transform:uppercase; font-size:14px; padding-top: 15px">Application For <?php echo $result_signature['section'];?> on <?php echo date('jS F Y',strtotime($result_signature['created_date']));?></strong></p>
        ...
        <strong>Section :</strong>&nbsp; <?php echo $result_signature['section'];?> <br />
        <strong>Category :</strong>&nbsp; <?php echo $result_signature['category'];?> <br />
        ...
        <strong>After Discount space cost INR <?php echo $currency;?> :</strong>&nbsp; <?php echo $result_signature['get_tot_space_cost_rate'];?> <br />
	...
	<?php	
 //    $sql="select * from exh_registration where uid='$uid' AND curr_last_yr_check='Y' AND `show`='IIJS 2020' order by gid desc";
 //    $query=$conn->query($sql);		
	// $num=$query->fetch_assoc();		
 //    if($num>0)
	// {
 //    while ($result = $query->fetch_assoc()) {
	// if($result['event_selected']=="iijs") { $event_for_iijs="IIJS 2020"; }
	?>
    <!-- <button class="accordion">Click here to view Application Summary for <?php echo $event_for_iijs;?>(Exh<?php echo $result['gid'];?>)</button>
	<div class="panel">
	<div class="clear"></div>

	<p><strong style="text-transform:uppercase; font-size:14px; padding-top: 15px">Application For <?php echo $result['section'];?> on <?php echo date('jS F Y',strtotime($result['created_date']));?></strong></p>
        <div class="summary_box">		
        <p><strong style="text-transform:uppercase; font-size:14px;">Application Summary</strong> <br />
        <span class="clear" style="height:1px; background:#ccc; display:block;"></span>
		...
		<strong>Space Cost Discount <?php echo $currency;?> :</strong>&nbsp; <?php echo $result['tot_space_cost_discount'];?> <br />
        <strong>After Discount space cost INR <?php echo $currency;?> :</strong>&nbsp; <?php echo $result['get_tot_space_cost_rate'];?> <br />
		...
		<div class="clear" style="height:10px;"></div> -->
		<?php 
		// $query1=$conn->query("select * from exh_reg_payment_details where uid='$uid' AND `show`='IIJS 2020' order by payment_id desc limit 0,1");
		// $result1=$query1->fetch_assoc();		
		?>        
<!--         <p><strong style="text-transform:uppercase; font-size:14px;">Application Status </strong> <br />
        <span class="clear" style="height:1px; background:#ccc; display:block;"></span>
      ...
      ?>
        ...
        <?php 
			/*..................................Get UTR DETAIL Start.....................................*/
				$utrExist = "SELECT sum(amountPaid) as `amountPaid`,sum(tdsAmount) as `tdsAmount` FROM `utr_history` WHERE registration_id='$uid' AND `show`='IIJS Signature 2020'";
				...
				$getSql_signature ="select * from exh_registration where uid='$uid' AND curr_last_yr_check='Y' AND `show`='IIJS Signature 2020' order by gid desc";
	...
	?>
        ...
        <?php 		
		if($countx > 0 && $countx!='') { ?>
        <div style="float:right; display:block; font-size:12px; font-weight:normal;">
        <?php if($amountUnpaid<=500){?>
        	<a href="manual_signature/index.php?uid=<?php echo $uid;?>&gid=<?php echo $gid;?>&section=<?php echo $result['section'];?>" class="newMaroonBtn">Online Exhibitor Manual</a>
			<!--<a href="javascript: void(0)" onclick = "second_alert()" class="newMaroonBtn">Online Exhibitor Manualss</a>-->
            <?php } else { ?>
