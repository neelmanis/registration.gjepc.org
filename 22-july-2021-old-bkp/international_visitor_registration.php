...
if(isset($_FILES['nri_fid']) && $_FILES['nri_fid']['name']!="")
{
	...
	if(isset($_FILES['vaccination_id']) && $_FILES['vaccination_id']['name']!="")
	{
		/* vaccination_id doc */
		$photo_name=$_FILES['vaccination_id']['name'];
		$photo_temp=$_FILES['vaccination_id']['tmp_name'];
		$photo_type=$_FILES['vaccination_id']['type'];
		$photo_size=$_FILES['vaccination_id']['size'];
		$attach="vaccination_id";
		if($photo_name!="")
		{
			$vaccination_id = uploadImage($photo_name,$photo_temp,$photo_type,$photo_size,$attach);
		}
		$update_vacc="update ivr_registration_details set vaccination_id='$vaccination_id',vaccination_id_updated='$vaccination_id_updated',vaccination_id_approval='P',vaccination_id_reason='' where uid='$uid' AND eid = '$eid'";
		$update_vaccResults = $conn->query($update_vacc);	
	}
...
if(isset($actions)=="saveInfo")
{
	...
	if(!empty($_FILES['vaccination']['name']) && $_FILES['vaccination']['name']!="")
	{
	if(isset($_FILES['vaccination']) && $_FILES['vaccination']['name']!="")
	{
		/* vaccination_id doc */
		$photo_name=$_FILES['vaccination']['name'];
		$photo_temp=$_FILES['vaccination']['tmp_name'];
		$photo_type=$_FILES['vaccination']['type'];
		$photo_size=$_FILES['vaccination']['size'];
		$attach="vaccination_id";
		if($photo_name!="")
		{
			$vaccination_id = vaccination_documents($photo_name,$photo_temp,$photo_type,$photo_size,$attach);
		}
		if($vaccination_id=='invalid')
		{ 
		echo "<script langauge=\"javascript\">alert(\"Invalid format.\");location.href='international_visitor_registration.php?eid=$eid';</script>";
		exit;
		}
		elseif($vaccination_id=='failed')
		{ 
		echo "<script langauge=\"javascript\">alert(\"Not uploaded due to server error\");location.href='international_visitor_registration.php?eid=$eid';</script>";
		exit;
		} else {
		$update_vacc="update ivr_registration_details set vaccination_id='$vaccination_id',vaccination_id_updated='yes',vaccination_id_approval='P' where uid='$uid' AND eid = '$eid'";
		$update_vaccResults = $conn->query($update_vacc);
	...
	echo "<meta http-equiv=refresh content=\"0;url=international_visitor_registration.php?eid=$eid\">";
		...
		echo "<script langauge=\"javascript\">alert(\"Kindly upload vaccination certificate\");location.href='international_visitor_registration.php?eid=$eid';</script>";
		exit;
	}
...
<!--                <div>              
              ...
              <div class="box-shadow">
				...
				<?php } else {	?>
					...
					<tr><td><input type="file" name="vaccination" id="vaccination" />Upload Vaccination Certificate</td>
					</tr>
					<tr> 
              ...
              <table class="responsive_table newTable">
                  ...
                  <?php $vaccination_id_reason=$data['vaccination_id_reason'];?>
                   <tr>
                    <td class="bold bgImage">Vaccination Approval</td>
                    <td class="bgImage" data-column="Vaccination Approval"><?php
					   	if($data['vaccination_id_approval']=='Y')
							echo "Approved";	
						else if($data['vaccination_id_approval']=='P')
							echo "Pending";	
						else
							echo "Disapproved <span style='margin-left:50px;font-style:italic;'>$vaccination_id_reason</span>";	
					?></td>
                  </tr> 
                  </tbody>                  
