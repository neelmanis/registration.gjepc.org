...
if($action == 'email_id'){
	...
	if($count>0) 
	{
		...
		$status = $row['application_approved'];
		 ...
		 $checkHistory = "SELECT * FROM `ivr_registration_details` WHERE email='$email_id' AND trade_show ='IIJS 2021' AND `application_approved`='Y'";
...
if($action == 'send_otp'){
	...
	<tr><td style="background-color:#744F9E;padding:3px;"></td></tr>
	  ...
	  <td>One Time Password for International Visitor Verification is <b>'.$otp.'</b></td></tr>
	   <tr><td>       
		  <p>Kind Regards,<br>
		  <b>GJEPC Web Team,</b>
		  </p>
...
if(isset($uid) && $uid!=""){
	$update_query = "update ivr_registration_details set erp_code='single',agree='$agree', trade_show='$trade_shows',modified_date=NOW() where eid='$eid' AND uid='$uid'";	
...
function uploadIntlVIsitor($file_name,$file_temp,$file_type,$file_size,$id,$name)
{
	...
	{
		if((($file_type == "application/pdf" || $file_type == "image/jpg" || $file_type == "image/png")) && $file_size < 2097152)
		{
			$random_name = rand();
			...
			{				
				...
				if($self_report_name!="")
				{
					...
					mkdir($create_self_report, 0777);
					  ...
					  $self_report=uploadIntlVIsitor($self_report_name,$self_report_temp,$self_report_type,$self_report_size,$eid,$attach);
					 if ($self_report =="fail") {
	...
	if($countData > 0){
        $updateData =  $conn->query("UPDATE visitor_lab_info SET via='$via',self_report='$self_report',`self_declaration`='$self_declaration',`certificate`='$certificate' WHERE registration_id='$uid' AND visitor_id='$eid' AND category_for='INTL'");
        unset($_SESSION['uid']);
        unset($_SESSION['eid']);
        echo json_encode($data = array("status"=>'success',"message"=>"Your COVID details have been updated successfully")); exit;
	} else {
       $sqlx = "INSERT INTO `visitor_lab_info` (`registration_id`, `visitor_id`, `pan_no`, `mobile_no`, `via`,`self_report`,`self_declaration`,`lab_name`,`location`,`certificate`, `status`,`category_for`) VALUES ('$uid', '$eid', '$eid', '$eid', '$via','$self_report','$self_declaration','0','0','$certificate',1','INTL');";
	    $ansData = $conn ->query($sqlx);
	    unset($_SESSION['uid']);
        unset($_SESSION['eid']);
	    echo json_encode($data = array("status"=>'success',"message"=>"Your COVID details have been submitted successfully")); exit;
	}
}
?>