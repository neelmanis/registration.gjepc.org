...
//	$CompanyName = getExhibitorName($exhibitor_code,$conn); 
   ...
   // $checkData = "SELECT * FROM visitor_lab_info WHERE registration_id='$registration_id' AND visitor_id='$visitor_id' AND `category_for`='$category_for'";
    $checkData = "SELECT * FROM visitor_lab_info WHERE registration_id='$registration_id' AND visitor_id='$visitor_id' AND mobile_no='$mobile_no'" ;
 //   $checkData = "SELECT * FROM visitor_lab_info WHERE mobile_no='$mobile_no' AND `category_for`='$category_for'";
   ...
   // get_data($smsContent,$mobile_no);
    ...
    if($certificate =='dose2'){
		$messagev = "We will update you After approval of your vaccination certificate, Your Badge will be available soon.";
    } else {
    	$messagev = "";
    }
