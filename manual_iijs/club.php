...
$result_catalog=mysql_query($catalog_data);
$fetch_catalog=mysql_fetch_array($result_catalog);
...
if($_REQUEST['action']=="ADD")
{
		...
		$qpreviousimg=mysql_query("select Catalog_ProductLogo from iijs_catalog where Exhibitor_Code='$exhibitor_code'");
		...
		$update_plogoresult = mysql_query($update_plogo);
		}
		
		if(isset($_FILES['Catalog_CompanyLogo']) && $_FILES['Catalog_CompanyLogo']['name']!="")
		{
		//Unlink the previuos image
		$qpreviousimg=mysql_query("select Catalog_CompanyLogo from iijs_catalog where Exhibitor_Code='$exhibitor_code'");
		$rpreviousimg=mysql_fetch_array($qpreviousimg);
		...
		$update_clogoresult = mysql_query($update_clogo);
		}		

if(isset($_POST['submit']))
{
		$Exibitor_Name=mysql_real_escape_string($_POST['Exibitor_Name']);
		$Catalog_Phone=mysql_real_escape_string($_POST['Catalog_Phone']);
		$Catalog_ContactPerson=mysql_real_escape_string($_POST['Catalog_ContactPerson']);
		$Catalog_Fax=mysql_real_escape_string($_POST['Catalog_Fax']);
		$Catalog_Designation=mysql_real_escape_string($_POST['Catalog_Designation']);
		$Catelog_mobile=mysql_real_escape_string($_POST['Catelog_mobile']);
		$Catalog_Address1=mysql_real_escape_string($_POST['Catalog_Address1']);
		$Catalog_Email=mysql_real_escape_string($_POST['Catalog_Email']);
		$Catalog_City=mysql_real_escape_string($_POST['Catalog_City']);
		$Catalog_Email1=mysql_real_escape_string($_POST['Catalog_Email1']);
		$Catalog_Pincode=mysql_real_escape_string($_POST['Catalog_Pincode']);
		$Catalog_Website=mysql_real_escape_string($_POST['Catalog_Website']);
		$Catalog_CountryId=mysql_real_escape_string($_POST['Catalog_CountryId']);		
		$Catalog_CountryId=getCountryId($Catalog_CountryId);	
		$Catalog_StallNo=mysql_real_escape_string($_POST['Catalog_StallNo']);
		$Catalog_State=mysql_real_escape_string($_POST['Catalog_State']);
		...
		$ProductLogo_Approved=mysql_real_escape_string($_REQUEST['ProductLogo_Approved']);
		if($ProductLogo_Approved=='Y')
		{
		$ProductLogo_Reason="";	
		}else
		{
		$ProductLogo_Reason=mysql_real_escape_string($_REQUEST['ProductLogo_Reason']);
		}
		...
		$CompanyLogo_Approved=mysql_real_escape_string($_REQUEST['CompanyLogo_Approved']);
		if($CompanyLogo_Approved=='Y')
		{
		$CompanyLogo_Reason="";	
		}else
		{
		$CompanyLogo_Reason=mysql_real_escape_string($_REQUEST['CompanyLogo_Reason']);
		}
...
{   
		...
		<p>Kindly login at our website - https://iijs-signature.org to verify the same.</p>
		...
		$subject = "Signature 2019 Exhibitor Manual - Form No. 1. COMPULSORY CATALOGUE ENTRY"; 
...
<table border="0" cellspacing="0" cellpadding="0" class="formManual">
  ...
  <tr>
    ...
    <td>:</td>
    <td><input type="text" name="Catalog_CountryId" id="Catalog_CountryId" class="textField" value="<?php echo getCountryName($Catalog_CountryId); ?>" /></td>
    <td>&nbsp;</td>
