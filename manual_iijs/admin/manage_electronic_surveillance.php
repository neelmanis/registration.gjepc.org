...
if($_REQUEST['Reset']=="Reset")
{
  ...
  header("Location: manage_electronic_surveillance.php?action=view");
...
<div class="breadcome_wrap">
	<div class="breadcome"><a href="manage_electronic_surveillance.php">Home</a> > Electronic Surveillance</div>
</div>

<div id="main">
	<div class="content">
    	<div class="content_head"><div class="content_head_button">Electronic Surveillance</div></div>
<div class="content_details1">
<?php 
	/*if($_SESSION['admin_role']=='Super Admin')
	{
		$sql5="SELECT a.*,b.*,c.* FROM `iijs_cctv` a,iijs_payment_master b,iijs_exhibitor c WHERE b.Form_ID='8' and  a.`Exhibitor_Code`=c.Exhibitor_Code and  b.`Exhibitor_Code`=c.`Exhibitor_Code` and a.Payment_Master_ID=b.Payment_Master_ID group by a.`Exhibitor_Code`";		
	} */
	
	if($_SESSION['admin_role']=='Super Admin' || $_SESSION['admin_role']=="Vendor")
	{	
		if($_SESSION['admin_admin_access']=='' || $_SESSION['admin_admin_access']=='ALL')
		{
		$sql5="SELECT a.*,b.*,c.* FROM `iijs_cctv` a,iijs_payment_master b,iijs_exhibitor c WHERE b.Form_ID='8' and  a.`Exhibitor_Code`=c.Exhibitor_Code and  b.`Exhibitor_Code`=c.`Exhibitor_Code` and a.Payment_Master_ID=b.Payment_Master_ID group by a.`Exhibitor_Code`";		
		}
		else
		{
		$sql5="SELECT a.*,b.*,c.* FROM `iijs_cctv` a,iijs_payment_master b,iijs_exhibitor c WHERE b.Form_ID='8' and  a.`Exhibitor_Code`=c.Exhibitor_Code and  a.`CCTV_CompanyName`='".$_SESSION['cc_tv_access']."' and a.Payment_Master_ID=b.Payment_Master_ID group by a.`Exhibitor_Code`";
		}
	}
	...
	else if($_SESSION['admin_role']=='Admin')
	{
		...
		{
		$sql5="SELECT a.*,b.*,c.* FROM `iijs_cctv` a,iijs_payment_master b,iijs_exhibitor c WHERE 1 and a.`Payment_Master_ID`=b.`Payment_Master_ID` and b.Form_ID='8' and a.`Exhibitor_Code`=c.Exhibitor_Code and b.`Exhibitor_Code`=c.`Exhibitor_Code` and c.Exhibitor_Region='".$_SESSION['admin_region_name']."' group by a.`Exhibitor_Code`";
		}
		else
		{
		$sql5="SELECT a.*,b.*,c.* FROM `iijs_cctv` a,iijs_payment_master b,iijs_exhibitor c WHERE 1 and a.`Payment_Master_ID`=b.`Payment_Master_ID` and b.Form_ID='8' and a.`Exhibitor_Code`=c.Exhibitor_Code and b.`Exhibitor_Code`=c.`Exhibitor_Code` and c.Exhibitor_DivisionNo='".$_SESSION['admin_division_name']."' group by a.`Exhibitor_Code`";
		}
	}
	//echo $sql5;
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  ...
  <?php
	...
	/*if($_SESSION['admin_role']=='Super Admin')
	{
		$sql="SELECT a.*,b.*,c.* FROM `iijs_cctv` a,iijs_payment_master b,iijs_exhibitor c WHERE b.Form_ID='8' and  a.`Exhibitor_Code`=c.Exhibitor_Code and  b.`Exhibitor_Code`=c.`Exhibitor_Code` and a.Payment_Master_ID=b.Payment_Master_ID";		
	} */
	...
	{	
		...
		{
		$sql="SELECT a.*,b.*,c.* FROM `iijs_cctv` a,iijs_payment_master b,iijs_exhibitor c WHERE b.Form_ID='8' and  a.`Exhibitor_Code`=c.Exhibitor_Code and  a.`CCTV_CompanyName`='".$_SESSION['cc_tv_access']."' and a.Payment_Master_ID=b.Payment_Master_ID ";
		}
	}
	else if($_SESSION['admin_role']=='Admin')
	{
		if($_SESSION['admin_admin_access']=='Region Wise')
		{
		$sql="SELECT a.*,b.*,c.* FROM `iijs_cctv` a,iijs_payment_master b,iijs_exhibitor c WHERE b.Form_ID='8' and a.`Exhibitor_Code`=c.Exhibitor_Code and  b.`Exhibitor_Code`=c.`Exhibitor_Code` and c.Exhibitor_Region='".$_SESSION['admin_region_name']."'";
		}
		else
		{
		$sql="SELECT a.*,b.*,c.* FROM `iijs_cctv` a,iijs_payment_master b,iijs_exhibitor c WHERE b.Form_ID='8' and a.`Exhibitor_Code`=c.Exhibitor_Code and  b.`Exhibitor_Code`=c.`Exhibitor_Code` and c.Exhibitor_DivisionNo='".$_SESSION['admin_division_name']."'";
		}
	}
	...
	{
	$sql.="a.Payment_Master_Approved='P' or ";
	}
  ...
  <tr>
	...
	?>
   	...
   	<td><a href="Form8.php?Exhibitor_Code=<?php echo $rows['Exhibitor_Code'];?>" class="edit">Edit</a></td>
  </tr>
...
?>	
<div class="pages_1">Total number of Application: <?php echo $rCount;?><?php echo pagination($limit,$page,'manage_electronic_surveillance.php?action=view&page=',$rCount); //call function to show pagination?></div>        
