...
function getSap_material_no($area)
{
...
{	//echo "<pre>"; print_r($_POST);exit;
	...
	$query="SELECT a.*,b.* FROM `iijs_housekeeping` a,iijs_exhibitor b WHERE 1 and a.`Exhibitor_Code`=b.Exhibitor_Code AND a.txn_status!='0300' AND a.Payment_Approved='Y' AND b.Exhibitor_Code='$exhibitor_code'";
	...
	$item_material = trim(getSap_material_no($area));
	$item_rate = trim(getRate($area));
           ...
           // curl_setopt($ch1, CURLOPT_TIMEOUT, 10);
			...
			foreach($xml->xpath('//soapenv:Body') as $header)
			{
					...
					{	$flag=1;
						...
						$sqlx = "UPDATE `iijs_housekeeping` SET `sap_push_date`=NOW(),`sap_push_admin`='$adminID',`sales_order_no`='$sales_order_no',`sap_sale_order_create_status`='1',api_used='onspot' WHERE `Exhibitor_Code`='$exhibitor_code' AND `orderId` ='$order_id'";
