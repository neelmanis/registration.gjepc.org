...
function getSap_material_no($area,$conn)
{
	$query_sel = "SELECT sap_material_no FROM iijs_housekeeping_material_master where area='$area'";	
...
function getRate($area,$conn)
{
	$query_sel = "SELECT rate FROM iijs_housekeeping_material_master where area='$area'";	
	$result_sel = $conn ->query($query_sel);								
		if($row = $result_sel->fetch_assoc())		 	
		{ 		
			return $row['rate'];
...
function getEventWBS($conn)
{
	$query_sel = "SELECT wbs FROM exh_event_master where status='1'";
	$result_sel =$conn->query($query_sel);								
	if($row = $result_sel->fetch_assoc())			 	
	{ 		
		return $row['wbs'];
...
{	//echo "<pre>"; print_r($_POST);exit;
	...
	$query="SELECT a.*,b.* FROM `iijs_housekeeping` a,iijs_exhibitor b WHERE 1 and a.`Exhibitor_Code`=b.Exhibitor_Code AND a.txn_status='0300' AND b.Exhibitor_Code='$exhibitor_code'";
	...
	$area = trim($challanResult['Exhibitor_Area']);	
	$item_material = trim(getSap_material_no($area,$conn));
	$item_rate = trim(getRate($area,$conn));
			
	if($country_code=="IN"){ 
		$bank_acnt_no="255802"; 	/*.................Domestic Account....................*/
	} 
           ...
           // curl_setopt($ch1, CURLOPT_TIMEOUT, 10);
			...
			foreach($xml->xpath('//soapenv:Body') as $header)
			{
					...
					{	$flag=1;
						...
						$sqlx = "UPDATE `iijs_housekeeping` SET `sap_push_date`=NOW(),`sap_push_admin`='$adminID',`sales_order_no`='$sales_order_no',`sap_sale_order_create_status`='1',api_used='pre' WHERE `Exhibitor_Code`='$exhibitor_code' AND `orderId` ='$order_id'";
