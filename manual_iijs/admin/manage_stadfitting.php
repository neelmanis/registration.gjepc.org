...
<title> Manual || Manage Standfitting</title>
...
<div class="breadcome_wrap">
	<div class="breadcome"><a href="search_application.php">Home</a> > Manage Standfitting Service </div>
</div>

<div id="main">
	<div class="content">
    	<div class="content_head"><div class="content_head_button">Manage Standfitting Services</div></div>
...
<?php 
	if($_SESSION['admin_role']=='Super Admin')
	{
		...
		if($_SESSION['admin_section']==""){
		    $sql5="select * from (SELECT a.Stand_ID,a.Info_Approved,a.Info_Reason,a.Application_Complete,a.Create_Date,b.Payment_Master_Approved,b.Payment_Master_Reason,c.* FROM `iijs_stand` a,iijs_payment_master b,iijs_exhibitor c WHERE b.Form_ID='3'and a.`Exhibitor_Code`=c.Exhibitor_Code and  b.`Exhibitor_Code`=c.`Exhibitor_Code` and a.Payment_Master_ID=b.Payment_Master_ID  order by a.`Application_Complete`) as mytable group by `Exhibitor_Code`";
		}
		else{
			$sql5="select * from (SELECT a.Stand_ID,a.Info_Approved,a.Info_Reason,a.Application_Complete,a.Create_Date,b.Payment_Master_Approved,b.Payment_Master_Reason,c.* FROM `iijs_stand` a,iijs_payment_master b,iijs_exhibitor c WHERE b.Form_ID='3'and a.`Exhibitor_Code`=c.Exhibitor_Code and  b.`Exhibitor_Code`=c.`Exhibitor_Code` and a.Payment_Master_ID=b.Payment_Master_ID order by a.`Application_Complete`) as mytable where Exhibitor_Section='$exhibitorSection' group by `Exhibitor_Code` ";
		}
	}
	else if($_SESSION['admin_role']=='Admin')
	{
		...
		{
		$sql5="select * from (SELECT a.Stand_ID,a.Info_Approved,a.Info_Reason,a.Application_Complete,a.Create_Date,b.Payment_Master_Approved,b.Payment_Master_Reason,c.* FROM `iijs_stand` a,iijs_payment_master b,iijs_exhibitor c WHERE b.Form_ID='3' 
and  a.`Exhibitor_Code`=c.Exhibitor_Code and  b.`Exhibitor_Code`=c.`Exhibitor_Code` and a.Payment_Master_ID=b.Payment_Master_ID  and c.Exhibitor_Region='".$_SESSION['admin_region_name']."' order by a.`Application_Complete`) as mytable group by `Exhibitor_Code`";
		}
		else
		{

		$sql5="select * from (SELECT a.Stand_ID,a.Info_Approved,a.Info_Reason,a.Application_Complete,a.Create_Date,b.Payment_Master_Approved,b.Payment_Master_Reason,c.* FROM `iijs_stand` a,iijs_payment_master b,iijs_exhibitor c WHERE b.Form_ID='3' 
and  a.`Exhibitor_Code`=c.Exhibitor_Code and  b.`Exhibitor_Code`=c.`Exhibitor_Code` and a.Payment_Master_ID=b.Payment_Master_ID  and c.Exhibitor_DivisionNo='".$_SESSION['admin_division_name']."' order by a.`Application_Complete`) as mytable group by `Exhibitor_Code`";
		}
	}
	else if($_SESSION['admin_role']=='Vendor')
	{
		if($_SESSION['admin_vendor_access']=="All"){
			...
			//	print_r($admin_vendor_access);
				$div1=$admin_vendor_access[0];
				$div2=$admin_vendor_access[1];
				$div3=$admin_vendor_access[2];
				$sql5.="select * from (SELECT a.Stand_ID,a.Info_Approved,a.Info_Reason,a.Application_Complete,a.Create_Date,b.Payment_Master_Approved,b.Payment_Master_Reason,c.* FROM `iijs_stand` a,iijs_payment_master b,iijs_exhibitor c WHERE b.Form_ID='3' 
				and  a.`Exhibitor_Code`=c.Exhibitor_Code and  b.`Exhibitor_Code`=c.`Exhibitor_Code` and a.Payment_Master_ID=b.Payment_Master_ID  and (";
				if($div1!="")
					$sql5.="c.vendor='$div1'";
				if($div2!="")
					$sql5.=" || c.vendor='$div2'";
				if($div3!="")
					$sql5.=" || c.vendor='$div3'";
				$sql5.=" ) order by a.`Application_Complete`) as mytable group by `Exhibitor_Code`";
		}
	}
	//echo $sql5;
	...
	while($rows5 = $result5->fetch_assoc())
	{
		...
		}else if($rows5['Info_Approved']=='P' && $rows5['Application_Complete']=='P' && $rows5['Payment_Master_Approved']=='P')
		{
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  ...
  <?php
	...
	if($_SESSION['admin_role']=='Super Admin')
	{
		$sql="select * from (SELECT a.Stand_ID,a.Info_Approved,a.Info_Reason,a.Application_Complete,a.Create_Date,a.Modify_Date,b.Payment_Master_Approved,b.Payment_Master_Reason,c.* FROM `iijs_stand` a,iijs_payment_master b,iijs_exhibitor c WHERE b.Form_ID='3' 
and  a.`Exhibitor_Code`=c.Exhibitor_Code and  b.`Exhibitor_Code`=c.`Exhibitor_Code` and a.Payment_Master_ID=b.Payment_Master_ID";		
	}
	else if($_SESSION['admin_role']=='Admin')
	{
		if($_SESSION['admin_admin_access']=='Region Wise')
		{
		$sql="select * from (SELECT a.Stand_ID,a.Info_Approved,a.Info_Reason,a.Application_Complete,a.Create_Date,a.Modify_Date,b.Payment_Master_Approved,b.Payment_Master_Reason,c.* FROM `iijs_stand` a,iijs_payment_master b,iijs_exhibitor c WHERE b.Form_ID='3' 
and  a.`Exhibitor_Code`=c.Exhibitor_Code and  b.`Exhibitor_Code`=c.`Exhibitor_Code` and a.Payment_Master_ID=b.Payment_Master_ID and c.Exhibitor_Region='".$_SESSION['admin_region_name']."'";
		}
		else
		{
		$sql="select * from (SELECT a.Stand_ID,a.Info_Approved,a.Info_Reason,a.Application_Complete,a.Create_Date,a.Modify_Date,b.Payment_Master_Approved,b.Payment_Master_Reason,c.* FROM `iijs_stand` a,iijs_payment_master b,iijs_exhibitor c WHERE b.Form_ID='3' 
and  a.`Exhibitor_Code`=c.Exhibitor_Code and  b.`Exhibitor_Code`=c.`Exhibitor_Code` and a.Payment_Master_ID=b.Payment_Master_ID and c.Exhibitor_DivisionNo='".$_SESSION['admin_division_name']."'";
		}
	}
	else if($_SESSION['admin_role']=='Vendor')
	{
...
and  a.`Exhibitor_Code`=c.Exhibitor_Code and  b.`Exhibitor_Code`=c.`Exhibitor_Code` and a.Payment_Master_ID=b.Payment_Master_ID";
			...
			$sql.="select * from (SELECT a.Stand_ID,a.Info_Approved,a.Info_Reason,a.Application_Complete,a.Create_Date,b.Payment_Master_Approved,b.Payment_Master_Reason,c.* FROM `iijs_stand` a,iijs_payment_master b,iijs_exhibitor c WHERE b.Form_ID='3' 
			and  a.`Exhibitor_Code`=c.Exhibitor_Code and  b.`Exhibitor_Code`=c.`Exhibitor_Code` and a.Payment_Master_ID=b.Payment_Master_ID  and (";
			if($div1!="")
				$sql.="c.vendor='$div1'";
			if($div2!="")
	...
	{
	$sql.=" and c.Exhibitor_HallNo = '".$_SESSION['Exhibitor_HallNo']."'";
	}
	$sql.=" order by a.`Application_Complete`) as mytable group by `Exhibitor_Code` ";
	//echo $sql;
  ...
  ?>
   <tr>
    ...
    <td><?php echo getCountryName(getExhibitorCountryID($rows['Exhibitor_Code'],$conn),$conn);?></td>
	...
	?>
   	...
   	<td><a href="Form3.php?Exhibitor_Code=<?php echo $rows['Exhibitor_Code'];?>" class="edit">Edit</a></td>
  </tr>
...
?>	
<div class="pages_1">Total number of order: <?php echo $rCount;?><?php echo pagination($limit,$page,'manage_stadfitting.php?action=view&page=',$rCount); //call function to show pagination?></div>        
