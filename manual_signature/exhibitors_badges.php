...
<table border="0" cellpadding="0" cellspacing="0" class="formManual">
  <tr>
   ...
   <?php
	...
	$Exhibitor_Section=$fetch_data_c['Exhibitor_Section'];
  
	$hostname1 = "192.168.40.107";
	$uname1 = "gjepcliveuserdb";
	$pwd1 = "KGj&6(pcvmLk5";
	$database1 = "gjepclivedatabase";   
	$conn1 = new mysqli($hostname1, $uname1, $pwd1, $database1);    
  
	function getVendorStatusFromGlobal($reg_id,$vis_id,$Exhibitor_Section,$conn1){
       if($Exhibitor_Section !=="machinery"){
       	$query_sel = "SELECT uniqueIdentifier FROM globalExhibition where `registration_id`='$reg_id' AND `visitor_id`='$vis_id' AND  
		  `participant_Type`='EXH' AND event='signature23' AND isReplaced is NULL limit 1";	
		} else {
			$query_sel = "SELECT uniqueIdentifier FROM globalExhibition where `registration_id`='$reg_id' AND `visitor_id`='$vis_id' AND  
		  `participant_Type`='EXHM' AND event='igjme23' AND isReplaced is NULL limit 1";	
		}
	...
	while($result = $query->fetch_assoc()){
		...
		$unique_id = getVendorStatusFromGlobal($registration_id,$result['Badge_Item_ID'],$Exhibitor_Section,$conn1);
   ?>
