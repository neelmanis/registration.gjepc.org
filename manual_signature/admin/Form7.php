...
$conn->query("update iijs_payment_master set Payment_Mode_ID='$Payment_Mode_ID',Payment_Master_Approved='$Payment_Master_Approved',Payment_Master_Reason='$Payment_Master_Reason' where  Payment_Master_ID='$Payment_Master_ID' and Form_ID='7'");

$conn->query("update iijs_wirelessinternet set Items_Approved='$Info_Approved',Items_Reason='$Info_Reason',Application_Complete='$Application_Complete' where WireLessInternet_ID='$WireLessInternet_ID'");
...
$message ='<table width="96%" bgcolor="#fbfbfb" style="margin:2% auto; border:2px solid #ddd; font-family:Arial, sans-serif; color:#333333; font-size:13px; border-radius:10px;">
		...
		<p>Your details for the Online Application for <strong>Form No. 7. WIRED INTERNET CONNECTION</strong> has been updated by Signature Admin.</p>
		...
		$subject = "".$event." Exhibitor Manual - Form No. 7. WIRED INTERNET CONNECTION";
		$headers  = 'MIME-Version: 1.0' . "\n";	
		$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\n";
		$headers .= 'From:admin@gjepc.org';	
		@mail($to, $subject, $message, $headers);
		header('location:Form7.php?Exhibitor_Code='.$Exhibitor_Code);
}
?>
...
<title>Manual || Wired Internet Connection||</title>
...
<div class="breadcome_wrap">
	<div class="breadcome"><a href="search_application.php">Home</a> >Wired Internet Connection</div>
</div>

<div id="main">
	<div class="content">
    	<div class="content_head">Wired Internet Connection</div>
...
<table  cellspacing="0" cellpadding="0" class="common">
    ...
    <?php
	$query=$conn->query("SELECT a.*,b.* FROM `iijs_wirelessinternet` a,iijs_payment_master b WHERE 1 and a.`Payment_Master_ID`=b.`Payment_Master_ID` and a.Exhibitor_Code='$exhibitor_code' and b.Form_ID='7' order by b.Payment_Master_ID asc");
	...
	?>
        <tr>
        <td ><?php echo $i;?></td>
        <td><a href="Form7.php?Payment_Master_ID=<?php echo $result['Payment_Master_ID'];?>&WireLessInternet_ID=<?php echo $result['WireLessInternet_ID'];?>&orderno=<?php echo $i;?>&Exhibitor_Code=<?php echo $exhibitor_code;?>" style="color:#FF0000;"><?php echo date("j M Y", strtotime($result['Create_Date']));?>(Click Here)</a></td>
		...
		<td  class="centerAlign">
			<img src='../images/red_cross.png'  alt='' class="deleteOrder <?php echo $result['Payment_Master_ID']?> <?php echo $result['WireLessInternet_ID']?> <?php echo $result['Exhibitor_Code']?>" style="cursor:pointer;" />
        </td>
        </tr>
    <?php $i++;}?>     
    </tbody>
...
<table border="0" cellspacing="0" cellpadding="0" class="formManual">
    ...
    <tr>
	  ...
	  <input type="hidden" name="WireLessInternet_ID" id="WireLessInternet_ID" value="<?php echo $_REQUEST['WireLessInternet_ID'];?>"/>
      <select class="textField" id="Item_ID" name="Item_ID" <?php if($_REQUEST['orderno']==""){?> disabled="disabled"<?php }?>>
      <option value="" selected="selected">--Select Item--</option>
      <?php 
        $query=$conn->query("SELECT `id`, `description`, `charges` FROM `iijs_wifi_master` WHERE status='1'");
        while($result=$query->fetch_assoc()){?>
...
<table border="0" cellspacing="0" cellpadding="0" class="formManual">
  ...
  <tbody id="Applied_Items">
    <?php 
	if(isset($_REQUEST['WireLessInternet_ID']) && $_REQUEST['WireLessInternet_ID']!="")
	{
    ...
    <tr>
      ...
      <td>
	  <img src="images/delete.png" class="deleteItem <?php echo $result['WirelessInternet_Items_ID'];?> <?php echo $WireLessInternet_ID;?> <?php echo $_REQUEST['Payment_Master_ID'];?> <?php echo $result['WirelessInternet_Items_Master_Id'];?>" style="cursor:pointer;">
	  </td>
   </tr>
...
<?php 
$WireLessInternet_ID=$_REQUEST['WireLessInternet_ID'];
$query=$conn->query("select Items_Approved,Items_Reason from iijs_wirelessinternet where WireLessInternet_ID='$WireLessInternet_ID'");
$result=$query->fetch_assoc();
$Info_Approved=$result['Items_Approved'];
$Info_Reason=$result['Items_Reason'];
?>
<div class="clear"></div>
...
<li>Cheque should be in favour of ? <strong>"The Gem and Jewellery Export Promotion Council."</strong></li>
    ...
    <div align="center">
	 ...
	 if($_REQUEST['orderno']!=""){?>
	 <a href="print_acknowledge/wireless.php?Payment_Master_ID=<?php echo $_REQUEST['Payment_Master_ID']?>&WireLessInternet_ID=<?php echo $_REQUEST['WireLessInternet_ID'];?>&orderno=<?php echo $_REQUEST['orderno'];?>&Exhibitor_Code=<?php echo $exhibitor_code;?>" target="_blank" class="button5">Print AcknowledgeMent</a>
	 <?php }?>
	 <input type="hidden" name="Exhibitor_Code" id="Exhibitor_Code" value="<?php echo $exhibitor_code;?>" />
	 <input type="hidden" name="Payment_Master_ID" id="Payment_Master_ID" value="<?php echo $_REQUEST['Payment_Master_ID'];?>"/>
	 <input type="hidden" name="WireLessInternet_ID" id="WireLessInternet_ID" value="<?php echo $WireLessInternet_ID;?>" />
