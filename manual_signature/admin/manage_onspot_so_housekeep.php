...
if($_REQUEST['Reset']=="Reset")
{
  ...
  header("Location: manage_onspot_so_housekeep.php?action=view");
...
<div class="breadcome_wrap">
	<div class="breadcome"><a href="manage_house_keeping.php">Home</a> > House OnSpot Keeping SO</div>
</div>

<div id="main">
	<div class="content">
    	<div class="content_head"><div class="content_head_button">OnSpot House Keeping Services</div></div>  
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  ...
  <?php
	...
	{
		$sql="SELECT a.*,b.* FROM `iijs_housekeeping` a,iijs_exhibitor b WHERE 1 and a.`Exhibitor_Code`=b.Exhibitor_Code AND a.txn_status!='0300' AND a.Payment_Approved='Y'";
	}
	/*else if($_SESSION['admin_role']=='Admin')
	{
		if($_SESSION['admin_admin_access']=='Region Wise')
		{
		$sql="SELECT a.*,b.* FROM `iijs_housekeeping` a,iijs_exhibitor b WHERE 1 and a.`Exhibitor_Code`=b.Exhibitor_Code AND a.txn_status='0300' and b.Exhibitor_Region='".$_SESSION['admin_region_name']."'";
		}else
		{
		$sql="SELECT a.*,b.* FROM `iijs_housekeeping` a,iijs_exhibitor b WHERE 1 and a.`Exhibitor_Code`=b.Exhibitor_Code AND a.txn_status='0300' and b.Exhibitor_DivisionNo='".$_SESSION['admin_division_name']."'";
		}
	} */
 ...
 while($rows=$result1->fetch_assoc())
  ...
  <tr>
	...
	<td class="so" data-url="<?php echo $rows['Exhibitor_Code'];?> <?php echo $rows['orderId'];?>">CREATE SO</td>
    <?php } else { ?>
...
?>	
<div class="pages_1">Total number of Memberships: <?php echo $rCount;?><?php echo pagination($limit,$page,'manage_onspot_so_housekeep.php?action=view&page=',$rCount); //call function to show pagination?></div>        
...
$(".so").click(function() {
	...
	var order_id = values[1];
		
	if (confirm('Are you sure you want to Create Housekeeping Sales order onspot')) {
		$.ajax({
		url: "sap_onspot_housekeep_api.php",
		method:"POST",
		data:{Exhibitor_Code:Exhibitor_Code,order_id:order_id},
		type: "POST",
		beforeSend: function() {
        	$("#overlay").show();
    	},
