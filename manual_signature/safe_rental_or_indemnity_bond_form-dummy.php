...
//$checking_time=strtotime("19-01-2018 14:35:00");
?>
<?php  
...
if(isset($_POST['save']) && $_POST['save']=="Save" )
{
	...
	$query_cnt=mysql_query("SELECT * FROM `iijs_safe_rental_items_tmp` WHERE `Exhibitor_Code`='$Exhibitor_Code'");
	$safe_cnt=mysql_num_rows($query_cnt);
	if($safe_cnt==0)
	{
	...
	mysql_query("update iijs_badge_items set Badge_IsKeyPerson='1' where Badge_Item_ID='$Badge_Item_ID1'");
	mysql_query("update iijs_badge_items set Badge_IsKeyPerson='1' where Badge_Item_ID='$Badge_Item_ID2'");
	
	/*..........................Payment Update..................................*/
	
	mysql_query("insert into iijs_payment_master set Form_ID='5A',Exhibitor_Code='$Exhibitor_Code',Payment_Mode_ID='$Payment_Mode_ID',Payment_Master_Amount='$Payment_Master_Amount',Payment_Master_AmountPaid='$Payment_Master_AmountPaid',tds_tax='0',tds_amount='0',net_payable_amount='0',Create_Date='$Create_Date'");
	...
	mysql_query("insert into iijs_safe_rental set Exhibitor_Code='$Exhibitor_Code',gstin='$gstin',utr_no='$utr_no',Location_Layout='$Location_Layout',Create_Date='$Create_Date',Modify_Date='$Create_Date',Payment_Master_ID='$Payment_Master_ID'");
	$Safe_Rental_ID=mysql_insert_id();
	//mysql_query("update iijs_safe_rental set Location_Layout='$Location_Layout' where Exhibitor_Code='$Exhibitor_Code'");

	$query=mysql_query("SELECT * FROM `iijs_safe_rental_items_tmp` WHERE `Exhibitor_Code`='$Exhibitor_Code'");
	...
	mysql_query("insert into iijs_safe_rental_items set Safe_Rental_ID='$Safe_Rental_ID',Safe_ID='$Safe_ID',Item_Quantity='$Item_Quantity',Item_Rate='$Item_Rate',Create_Date='$Create_Date'");
	
	/*......................Get Stand Fitting Item Quantity...............................*/
	$qitem_quantity=mysql_query("select Item_Quantity from iijs_safe_rental_master where Safe_ID='$Safe_ID'");
	...
	mysql_query("update iijs_safe_rental_master set Item_Quantity='$remain_quantity' where Safe_ID='$Safe_ID'");
	
	mysql_query("delete from iijs_safe_rental_items_tmp where Safe_Rental_Items_ID='$Safe_Rental_Items_ID'");
	}	
	...
	<strong>IIJS Signature Web Team,</strong>
	</p>
	</td>
	</tr>
	<tr>
	...
	$subject = "IIJS Signature 2020 Exhibitor Manual - SAFE RENTAL";	
	...
	@mail($to, $subject, $message, $headers);
		...
		echo '<script type="text/javascript">'; 
		echo 'alert("You have successfully submitted your application...\n Kindly take print acknowledgement of your order and submit to council office along with requisite payment");'; 
...
if(isset($_POST['update']))
{
	...
	$qpreviousimg=mysql_query("select Location_Layout from iijs_safe_rental where where Safe_Rental_ID='$Safe_Rental_ID'");
	$rpreviousimg=mysql_fetch_array($qpreviousimg);
	$filename="images/Location_Layout/".$exhibitor_code."/".$rpreviousimg['Location_Layout'];
	unlink($filename);
	
mysql_query("update iijs_safe_rental set Location_Layout='$Location_Layout',Layout_Approve ='P',Layout_Reason='',Application_Complete='P' where Safe_Rental_ID='$Safe_Rental_ID'");
...
/*if(isset($_POST['update']))
{
	...
	move_uploaded_file($tmp,$target_path );	
	$Safe_Rental_ID=$_POST['Safe_Rental_ID'];
mysql_query("update iijs_safe_rental set Location_Layout='$Location_Layout',Layout_Approve ='P',Application_Complete='P' where Safe_Rental_ID='$Safe_Rental_ID'");
	header('location:safe_rental_or_indemnity_bond_form.php');
}*/

$action=$_REQUEST['action'];
if($action!="ADD")
mysql_query("delete  from iijs_safe_rental_items_tmp where Exhibitor_Code='$exhibitor_code'");
?>
...
<li>
    ...
    After form submission, please print an acknowledgement and<br />
	<a href="images/pdf/Safe_Rental_Indemnity_Bond_Sig_18.pdf" target="_blank" ><strong>"AFFIDAVIT CUM INDEMNITY BOND"</strong> </a>printed on your company's letter head and <br />
    <strong>Return to:</strong><br />
     The Gem &amp; Jewellery Export Promotion Council<br />
     G2 Trade Centre, Opposite MTNL Building <br />
     Bandra Kurla Complex,<br />
     Bandra (E) Mumbai 400 051 - India<br />
     Tel: +91-22 43541800 Fax: +91-22 26524769<br />
     Email: <a href="mailto:signature@gjepcindia.com">signature@gjepcindia.com </a><br />  
</li>

<li>For payment approval Wire Transfer should reach the council within 3 working days after order date.</li>
<li>The final deadline for the online form submission is 1th January 2020.</li>
<?php }if($Exhibitor_Country_ID==IN){?>
	...
	<li>The AFFIDAVIT CUM INDEMNITY BOND should be notarised before submission. </li>	
	<li>For payment approval UTR No and Payment details to be updated within 3 working days after order date.</li>
<?php }?>
</ol>
</span>
</div>
<span  class="spanbox">Deadline : <strong>11th January 2020</strong></span>
<span  class="spanbox"><a href="images/pdf/Safe_Rental_Indemnity_Bond_Sig_20.pdf" target="_blank" style="color:#fff;"><strong>DOWNLOAD AFFIDAVIT CUM INDEMNITY BOND</strong></a></span>
...
<?php 
$query=mysql_query("SELECT a.*,b.* FROM `iijs_safe_rental` a,iijs_payment_master b WHERE 1 and a.`Payment_Master_ID`=b.`Payment_Master_ID` and a.Exhibitor_Code='$exhibitor_code' and b.Form_ID='5A' order by b.Payment_Master_ID asc");
...
<table width="533" border="0" cellpadding="0" cellspacing="0" class="formManual" > 
  ...
  <?php 
  $i=1;
  $query=mysql_query("select * from iijs_safe_rental_master");
  while($result=mysql_fetch_array($query)){  
  ?>
    <tr>
...
<table border="0" cellspacing="0" cellpadding="0" class="formManual">
	...
	if(isset($_REQUEST['Safe_Rental_ID']) && $_REQUEST['Safe_Rental_ID']!="")
	{
		...
		$query=mysql_query("select * from iijs_safe_rental_items where Safe_Rental_ID='$Safe_Rental_ID'");
	}
	else
	{
    	$query=mysql_query("select * from iijs_safe_rental_items_tmp where Exhibitor_Code='$exhibitor_code'");
	...
	$grand_tot=0;
	$safe_cnt=mysql_num_rows($query);
	while($result=mysql_fetch_array($query)){  
	$tot=$result['Item_Rate']*$result['Item_Quantity'];
	$grand_tot=$grand_tot+$tot;
	?>
    <tr>
      <td><?php echo getSaferentalId($result['Safe_ID']);?></td>
	  <td><?php echo $tot;?></td>
...
<?php
$i=1; 
$query1=mysql_query("SELECT * FROM `iijs_badge_items` WHERE `Exhibitor_Code`='$exhibitor_code' AND (`Badge_Type`='E' ||`Badge_Type`='M') AND Badge_IsKeyPerson='1'");
...
<div class="clear">
  <table border="0" cellspacing="0" cellpadding="0" class="formManual">
	...
	<?php 
		$sql=mysql_query("SELECT * FROM `iijs_badge_items` WHERE `Badge_Item_ID`='$Badge_Item_ID1'");
		$result=mysql_fetch_array($sql);
		$name1=$result['Badge_Name'];
		$Badge_Designation1=$result['Badge_Designation'];
		$Badge_Photo1=$result['Badge_Photo'];
		$sql=mysql_query("SELECT * FROM `iijs_badge_items` WHERE `Badge_Item_ID`='$Badge_Item_ID2'");
		$result=mysql_fetch_array($sql);
  ...
  <table border="0" cellspacing="0" cellpadding="0" class="formManual">
	...
	<?php 
	$query=mysql_query("select gstin,utr_no,Location_Layout,Layout_Approve,Layout_Reason from iijs_safe_rental where Safe_Rental_ID='".$_REQUEST['Safe_Rental_ID']."'");
  ...
  <table border="0" cellspacing="0" cellpadding="0" class="formManual">
   ...
   ?>
     ...
     <tr>
      ...
      <td width="44">:</td>
      <td width="478" colspan="3"><input type="radio" name="Payment_Mode_ID" id="Payment_Mode_ID" value="4" <?php if($Payment_Mode_ID==4){?> checked="checked"<?php }?>  />NEFT
	  ...
	  <input type="text"name="gstin" id="gstin" value="<?php echo $gstin;?>" minlength="15" maxlength="15" autocomplete="off" required/>	
    </tr>	
	<td width="143"  class="bold"><sup>*</sup> UTR NO / Payment details.</td>	
      <td width="44">:</td>	
      <td width="478" colspan="">	
	  <input type="text"name="utr_no" id="utr_no" value="<?php echo $utr_no;?>" autocomplete="off" required/>	
    </tr>
...
<?php 
$Safe_Rental_ID=$_REQUEST['Safe_Rental_ID'];
$query=mysql_query("select * from iijs_safe_rental where Safe_Rental_ID='$Safe_Rental_ID'");
