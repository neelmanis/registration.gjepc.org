...
if(isset($Exhibitor_Code) && $Exhibitor_Code!="")
{
	...
	$custname = trim($fetch_data['Exhibitor_Name']);
	$custname = preg_replace("/[^a-zA-Z]/", " ", $custname);
	$mobNo = $fetch_data['Exhibitor_Mobile'];

	$orderUpdate ="SELECT * FROM `iijs_payment_master` WHERE `Form_ID`='7' AND Payment_Master_OrderNo='$orderId' AND Exhibitor_Code='$Exhibitor_Code' AND Payment_Master_Approved='P'";
	...
	$tpslUpdate ="update iijs_payment_master set tpsl_txn_id_rand='$tpsl_txn_id' WHERE `Form_ID`='7' AND Payment_Master_OrderNo='$orderId' AND Exhibitor_Code='$Exhibitor_Code' AND Payment_Master_Approved='P'";
	...
	$mrctTxtID = $orderId;
	$currencyType = "INR";
	$returnURL = "https://" . $_SERVER['HTTP_HOST']."/manual_signature/wifi_payment_success.php";
