...
$("#save_key_person_form").submit(function(e) {
    $("label[for='safeKeyPerson1']").html("");
    $("label[for='safeKeyPerson2']").html("");
    e.preventDefault(); // avoid to execute the actual submit of the form.
...
<span style="float:right;margin-left: 10px;" class="spanbox">Deadline without surcharge : <?php echo getFormDeadLine(2,$conn);?></span>
...
<p><b>Note: We are in discussion with the Govt Authorities on increasing the no of badges. We will soon inform about the development on the same. Accordingly, no of badges per booth will be duly informed</b></p>
<div class="clear"></div>
...
<div class="title"><h4>Safe Key Persons </h4></div>
<?php 

$query_selected_key_person = $conn ->query("select * from iijs_badge_items where Exhibitor_Code='$exhibitor_code' AND Badge_IsKeyPerson='1'");
$count_selected_key_person   = $query_selected_key_person ->num_rows;
$selected_key_persons = array();
while($row_selected_key_person  = $query_selected_key_person->fetch_assoc()){
 
$selected_key_persons[]  = $row_selected_key_person['Badge_Item_ID'];
}
if(!empty($selected_key_persons)){
	$query_iijs_badge_items_key_1 =$conn ->query("select * from iijs_badge_items where Exhibitor_Code='$exhibitor_code' AND Badge_Item_ID ='$selected_key_persons[0]'");
	$query_iijs_badge_items_key_2 =$conn ->query("select * from iijs_badge_items where Exhibitor_Code='$exhibitor_code' AND Badge_Item_ID ='$selected_key_persons[1]'");
}else{
	$query_iijs_badge_items_key_1 =$conn ->query("select * from iijs_badge_items where Exhibitor_Code='$exhibitor_code' ");
	$query_iijs_badge_items_key_2 =$conn ->query("select * from iijs_badge_items where Exhibitor_Code='$exhibitor_code' ");
}


$count_iijs_badge_items = $query_iijs_badge_items_key_1->num_rows;


$query_check_safe_rental = $conn ->query("select * from iijs_safe_rental where Exhibitor_Code='$exhibitor_code'");
if($query_check_safe_rental->num_rows > 0 ){ ?>
<form method="post" id="save_key_person_form" action="update_badges.php">
	

<table border="0" cellspacing="0" cellpadding="0" class="formManual">
  <tr>
    <th class="bold">Safe key Person 1</th>
   
    <th class="bold">Safe key Person 2</th>
  </tr>
   <tr>
    <td class="bold">  
    	<select name="safeKeyPerson1" id="safeKeyPerson1" class="textField"> 
    		<?php if(empty($selected_key_persons)){ ?>
    		<option value="">Select </option>
    		 <?php } ?>
	    	
	    	<?php if($count_iijs_badge_items > 0 ){ 
	    		
              while($row_iijs_badge_items_key_1 = $query_iijs_badge_items_key_1->fetch_assoc()){ 
              	?>

				<option value="<?php echo $row_iijs_badge_items_key_1['Badge_Item_ID']; ?>" <?php echo (( $row_iijs_badge_items_key_1['Badge_Item_ID'] ==  $selected_key_persons[0]  ) ? "selected " : " ");?>  <?php echo (!empty($selected_key_persons)   ? "disabled " : " ");?> ><?php echo $row_iijs_badge_items_key_1['Badge_Name']; ?></option>
              <?php } } ?>
	    	
	    </select>
	    <label class="" class="error" style="display:block;color: red;" for="safeKeyPerson1"></label>
	</td>
   
    <td class="bold">
    	<select name="safeKeyPerson2" id="safeKeyPerson2" class="textField" <?php echo $selected_key_persons[1] ;?>> 
    		<?php if(empty($selected_key_persons)){ ?>
    		<option value="">Select </option>
    		 <?php } ?>

	    	<?php if($count_iijs_badge_items > 0 ){ 
              while($row_iijs_badge_items_key_2 = $query_iijs_badge_items_key_2->fetch_assoc()){ 
              	?>
				<option value="<?php echo $row_iijs_badge_items_key_2['Badge_Item_ID']; ?>" <?php echo (( $row_iijs_badge_items_key_2['Badge_Item_ID'] ==  $selected_key_persons[1]) ? "selected" : "");?> <?php echo (!empty($selected_key_persons)   ? "disabled " : " ");?>> <?php echo $row_iijs_badge_items_key_2['Badge_Name']; ?></option>
              <?php }  }  ?>
	    </select>
	    <label class="error" style="display:block;color:red" for="safeKeyPerson2"></label>

	</td>
	
  </tr>
  <?php if($count_selected_key_person  == 0){ ?>
  	  <tr>
  <td class="bold">
  	<input type="hidden"  name="action" value="safe_key_person_update">
	    <input type="submit" name="save_key_person" id="save_key_person" value="Save" class="maroon_btn"/>
	</td>
  </tr>
  <?php } ?>
...
-->
<div class="title"><h4>Car Parking</h4></div>
<div class="clear"></div>
<p><b>Parking facility is available at JWCC from Level P0 to P4. Exhibitors can park their vehicles on any of the parking levels on first-cum-first-serve basis. No exhibitor car stickers will be issued to exhibitors</b></p>
...
<table border="0" cellspacing="0" cellpadding="0" class="formManual">
  ...
  <tbody id="replaceBadge" style="display:none;">
    ...
    <td width="100"><input type="text" maxlength="10" name="Badge_Mobile_No" id="Badge_Mobile_No" class="textField numeric" maxlength='10'/></td>
    </tr>
   <tr>
    <td>Photo :<sup>*</sup></td>
    <td>    
