...
if($_REQUEST['Reset']=="Reset")
{
  ...
  header("Location: manage_compressed_air_water.php?action=view");
...
<div id="main">
	<div class="content">
    	<div class="content_head"><div class="content_head_button">Compressed Air Water Connection</div></div>
<div class="content_details1">
<?php 
	if($_SESSION['admin_role']=='Super Admin')
	{
		$sql5="SELECT a.*,b.* FROM `igjme_air_water` a,iijs_exhibitor b WHERE 1 and a.`Exhibitor_Code`=b.Exhibitor_Code";		
	}
	else if($_SESSION['admin_role']=='Admin')
	{
		if($_SESSION['admin_admin_access']=='Region Wise')
		{
		 $sql5="SELECT a.*,b.* FROM `igjme_air_water` a,iijs_exhibitor b WHERE 1 and a.`Exhibitor_Code`=b.Exhibitor_Code and b.Exhibitor_Region='".$_SESSION['admin_region_name']."'";
		}else
		{
			if($_SESSION["admin_hall"]=="2")
			{
				$sql5="SELECT a.*,b.* FROM `igjme_air_water` a,iijs_exhibitor b WHERE 1 and a.`Exhibitor_Code`=b.Exhibitor_Code and b.Exhibitor_HallNo='".$_SESSION['admin_hall']."' and (b.Exhibitor_DivisionNo='1' or b.Exhibitor_DivisionNo='2')";
			}
			else
			{
				$sql5="SELECT a.*,b.* FROM `igjme_air_water` a,iijs_exhibitor b WHERE 1 and a.`Exhibitor_Code`=b.Exhibitor_Code and b.Exhibitor_HallNo='".$_SESSION['admin_hall']."'";
			}
		}
	}
	...
	{
		if($rows5['Info_Approved']=='Y' && $rows5['Payment_Approved']=='Y')
		{
			$total_approve=$total_approve+1;
		}else if($rows5['Info_Approved']=='P' || $rows5['Payment_Approved']=='P')
		{
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="detail_txt" >
  ...
  <?php
	...
	if($_SESSION['admin_role']=='Super Admin')
	{
		$sql="SELECT a.*,b.* FROM `igjme_air_water` a,iijs_exhibitor b WHERE 1 and a.`Exhibitor_Code`=b.Exhibitor_Code";		
	}else if($_SESSION['admin_role']=='Admin')
	{
		if($_SESSION['admin_admin_access']=='Region Wise')
		{
		$sql="SELECT a.*,b.* FROM `igjme_air_water` a,iijs_exhibitor b WHERE 1 and a.`Exhibitor_Code`=b.Exhibitor_Code and b.Exhibitor_Region='".$_SESSION['admin_region_name']."'";
		}else
		{
			if($_SESSION["admin_hall"]=="2")
			{
				$sql="SELECT a.*,b.* FROM `igjme_air_water` a,iijs_exhibitor b WHERE 1 and a.`Exhibitor_Code`=b.Exhibitor_Code and b.Exhibitor_HallNo='".$_SESSION['admin_hall']."' and (b.Exhibitor_DivisionNo='1' or b.Exhibitor_DivisionNo='2')";
			}
			else
			{
				$sql="SELECT a.*,b.* FROM `igjme_air_water` a,iijs_exhibitor b WHERE 1 and a.`Exhibitor_Code`=b.Exhibitor_Code and b.Exhibitor_HallNo='".$_SESSION['admin_hall']."'";
			}
		}
	}
	...
	{
	$sql.=" and a.Info_Approved='P'";
	}
	if($_SESSION['Info_Y']=="Y")
	{
	$sql.=" and a.Info_Approved='Y'";
	}
	if($_SESSION['Info_N']=="N")
	{
	$sql.=" and a.Info_Approved='N'";
	}
	
	if($_SESSION['Payment_P']=="P")
	{
	$sql.=" and a.Payment_Approved='P'";
	}
	if($_SESSION['Payment_Y']=="Y")
	{
	$sql.=" and a.Payment_Approved='Y'";
	}
	if($_SESSION['Payment_N']=="N")
	{
	$sql.=" and a.Payment_Approved='N'";
	}


	if($_SESSION['Application_P']=="P")
	{
	$sql.=" and a.Application_Complete='P'";
	}
	if($_SESSION['Application_Y']=="Y")
	{
	$sql.=" and a.Application_Complete='Y'";
	}
	if($_SESSION['Application_N']=="N")
	{
	$sql.=" and a.Application_Complete='N'";
	}
	...
	$sql1=$sql." order by a.aw_id desc limit $start, $limit"; 
  ...
  <tr>
    ...
    <td align="center">
	<?php 
	if($rows['Payment_Approved']=='Y')
	{
		echo "<img src='images/notification-tick.gif' alt='' />";
	}else if($rows['Payment_Approved']=='N')
	{
		echo "<img src='images/no.gif' alt='' />";
	}else 
	{
	...
	?>	</td>
   	<td><a href="Form11.php?Exhibitor_Code=<?php echo $rows['Exhibitor_Code'];?>" class="edit">Edit</a></td>
  </tr>
