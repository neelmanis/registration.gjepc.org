...
//$checking_time=strtotime("23-08-2021 23:59:59");
$checking_time = getFormDeadLineTime(8,$conn);
?>
<?php
...
if($save=="Save")
{
	...
	$query_cnt = $conn ->query("SELECT * FROM `iijs_cctv_items_master_tmp` WHERE `Exhibitor_Code`='$Exhibitor_Code'");
	$cctv_cnt = $query_cnt->num_rows;
	if($cctv_cnt==0)
	{
	...
	/*..........................Payment Update..................................*/
	$qorder=$conn->query("select count(Payment_Master_ID) as count from  iijs_payment_master where Exhibitor_Code='$Exhibitor_Code' and Form_ID='8'");
	$rorder = $qorder->fetch_assoc();	
	$order_count=$rorder['count'];
	
	$strNo = rand(1,1000000);
	$Payment_Master_OrderNo=$Payment_Master_OrderNo = 'CCTV'.$strNo;

	$payment = $conn ->query("insert into iijs_payment_master set Form_ID='8',Exhibitor_Code='$Exhibitor_Code',Payment_Master_OrderNo='$Payment_Master_OrderNo',Payment_Mode_ID='$Payment_Mode_ID',Payment_Master_Amount='$Payment_Master_Amount',Payment_Master_ServiceTax='$Payment_Master_ServiceTax',tds_tax='0',tds_amount='0',net_payable_amount='0',Payment_Master_AmountPaid='$Payment_Master_AmountPaid',Create_Date='$Create_Date'");
	$Payment_Master_ID = mysqli_insert_id($conn);
	
	$conn ->query("insert into iijs_cctv set Exhibitor_Code='$Exhibitor_Code',Payment_Master_ID='$Payment_Master_ID',CCTV_CompanyName='$CCTV_CompanyName',CCTV_OrderNo='0',Create_Date='$Create_Date'");
	$CCTV_ID = mysqli_insert_id($conn);
	...
	$items = $conn ->query("insert into  iijs_cctv_items set CCTV_ID='$CCTV_ID',CCTV_Items_Master_ID='$CCTV_Items_Master_ID',CCTV_Items_Quantity='$CCTV_Items_Quantity', CCTV_Items_Rate='$CCTV_Items_Rate',Create_Date='$Create_Date'");
	$itemsm = $conn ->query("delete from iijs_cctv_items_master_tmp where id='$id'");
	...
	$js = $conn->query("update iijs_cctv_items_master set Item_Quantity='$remain_quantity' where CCTV_Items_Master_ID='$CCTV_Items_Master_ID'");
	}
	...
	<p>Thank you for applying Online for <strong> ELECTRONIC SURVEILLANCE</strong>. Please note your application is under approval process.</p>
	...
	$subject = "".$event." Exhibitor Manual - ELECTRONIC SURVEILLANCE";	
...
{
	//$conn ->query("delete from iijs_cctv_items_master_tmp where EXHIBITOR_CODE='$exhibitor_code'");
}
?>
...
<span  class="spanbox">Deadline : <strong><?php echo getFormDeadLine(8,$conn);?></strong></span>
<div class="clear"></div>
<?php if($_REQUEST['auth']=="admin" || $_SESSION['ACCESS']=="ADMIN"){ ?>
<a href="electronic_surveillance.php?&auth=admin&action=ADD" class="maroon_btn" style="float:right;">ADD NEW ORDER</a><br />
<?php } ?>
...
<tbody>
    <tr>
		...
		<th valign="top" class="centerAlign">
        <div align="center"><span class="table_head" style="height: 13px"> Print Acknowledgement</span></div></th>
    </tr>

   <?php
    $i=1;
	$query = $conn ->query("SELECT a.*,b.* FROM `iijs_cctv` a,iijs_payment_master b WHERE 1 and a.`Payment_Master_ID`=b.`Payment_Master_ID` and a.Exhibitor_Code='$exhibitor_code' and b.Form_ID='8' order by b.Payment_Master_ID desc");
	$order_num = $query->num_rows;
	while($result = $query->fetch_assoc()){
	?>
        <tr>
        <td><?php echo $i;?></td>
        <td><a href="electronic_surveillance.php?Payment_Master_ID=<?php echo $result['Payment_Master_ID'];?>&CCTV_ID=<?php echo $result['CCTV_ID'];?>&orderno=<?php echo $result['Payment_Master_OrderNo'];?>&action=summary"><?php echo date("j M Y", strtotime($result['Create_Date']));?> (Click Here)</a></td>
		...
		<td valign="middle" colspan="1" class="centerAlign">
		<?php  
			if($Payment_Master_OrderNo!==""){ ?>
			<a href="print_acknowledge/electronic_servillance.php?Payment_Master_ID=<?php echo $result['Payment_Master_ID'];?>&CCTV_ID=<?php echo $result['CCTV_ID'];?>&orderno=<?php echo $result['Payment_Master_OrderNo'];?>" target="_blank" class="button5">Print AcknowledgeMent</a>
			<?php } ?>			
        </td>
        </tr>
    <?php $i++; } ?>   
...
<table border="0" cellspacing="0" cellpadding="0" class="formManual">
  <tr>
	...
	<td width="20"><input type="radio" name="cctv_company" id="cctv_company" value="asterix" <?php if($cctv_company=='asterix'){?> checked="checked"<?php }?> /></td>
    <td>M/s Asterix</td>
  </tr>
</table>
...
$CCTV_ID = $_REQUEST['CCTV_ID'];
$query = $conn->query("select * from iijs_cctv where CCTV_ID='$CCTV_ID'");
