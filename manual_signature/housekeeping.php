...
//$formDeadLine = strtotime("09-12-2022 23:59:59");
?>
...
{ //echo '<pre>'; print_r($_POST); exit;
	...
	if(stripos($strings,$search)){
		$total_amount = filter($_POST['net_payable_amount']);
		$net_payable_amount = substr($total_amount, 0,-2);
	} else {
...
<span style="margin-left:40%;" class="spanbox">Deadline : <strong>
<?php $lastTime = getFormDeadLineTime(6,$conn); echo date("d-m-Y",strtotime($lastTime));?></strong></span>
...
<table width="680" border="0" cellspacing="0" cellpadding="0" class="formManual">  
	...
	<td>:</td>
	<td colspan="2">05th to 09th of JANUARY, 2023</td>
    </tr>  
	<tr>
...
// echo  $current_time."_______".$formDeadLine;
if($_REQUEST['auth']=="admin" || $_SESSION['ACCESS']=="ADMIN" ){ ?>
<input name="submit" id="submit" type="submit" value="Submit"  class="maroon_btn" /> 

<?php } else if($Application_Complete=='P' AND $current_time <= strtotime($formDeadLine)){ ?>
