...
function getExhibitorSection($Exhibitor_Code,$conn)
{
	$query_sel = "SELECT Exhibitor_Section FROM  iijs_exhibitor  where Exhibitor_Code='$Exhibitor_Code'";
	$result_sel =$conn->query($query_sel);								
	if($row = $result_sel->fetch_assoc())			 	
	{ 		
		return $row['Exhibitor_Section'];
...
function getFormStartDate($id,$conn)
{
	$query_sel = "SELECT Start_Date FROM `iijs_form_details` WHERE `Start_Date` <= NOW() and Form_No='$id' limit 1";	
	$result_sel = $conn ->query($query_sel);								
	if($row = $result_sel->fetch_assoc())		 	
	{ 		
...
{
	$query_sel = "SELECT Dedline_Date FROM `iijs_form_details` WHERE 1 and Form_No='$id' limit 1";	
	$result_sel = $conn ->query($query_sel);								
	if($row = $result_sel->fetch_assoc())		 	
	{ 		
