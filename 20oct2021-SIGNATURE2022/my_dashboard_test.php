...
$closetime = strtotime("02-08-2021 23:55:00");
?>
	<?php
	/* IVR Start */
	...
	$fetch_status_pvr="select obmp_application_status from visitor_obmp_details where uid=".$_SESSION['USERID']." and participate_for_show='iijs21' and year='2021' LIMIT 1";
...
<?php
$saveTDS = $_POST['saveTDS'];
if($saveTDS == "saveTDSinfo")
{
	$utr_number = filter(filter_replace($_POST['utr_number']));
	$tdsAmount = filter($_POST['tdsAmount']);

	if(empty($utr_number)) { $utrError = "Plz Select UTR Number"; }
	elseif(empty($tdsAmount)) { $tdsAmountError = "Plz Enter Amount Paid"; }
	else {
		
		$mmx = "SELECT * FROM `utr_history` WHERE registration_id='$registration_id' AND utr_number='$utr_number' AND `show`='IIJS 2021' AND `event_selected`='iijs'";
		...
		if($countUTR > 0)
		{
			...
			$updateUTR = "UPDATE `utr_history` SET `modified_date`=NOW(),`tdsAmount`='$tdsAmount' WHERE `registration_id`='$registration_id' AND id='$id' AND `show`='IIJS 2021' AND `event_selected`='iijs'";
			$resultUTR = $conn->query($updateUTR);
			if($resultUTR) { $tdsNameSuccess = "Saved Successfully"; }
		}
	}
}
?>
		...
		<div class="wrapper">
			...
			<div class="bold_font text-center">
				...
				<div class="content_area">					
					...
					//	{ ?>
						<div class="box_dash_container box-shadow">	
						<div class="box_title"><h3>IIJS Premiere 2021 Show UTR</h3></div>
						...
						<form name="tdsUpdate" method="POST" action="" id="form-horizontal">
						<input type="hidden" name="saveTDS" value="saveTDSinfo">
						
							<div class="d-flex col-100">
							<div class="form-group-inner col-100">
							<label>UTR : </label>
							<select name="utr_number" id="utr_number" class="select-control">
							<option value="">-- Select UTR --</option>
							<?php						
							$tdsExist = "SELECT * FROM `utr_history` WHERE registration_id='$registration_id' AND `show`='IIJS 2021'";
							$tdsResult = $conn->query($tdsExist);
							$countTDSPayment = $tdsResult->num_rows;
							if($countTDSPayment > 0){
							while($result = $tdsResult->fetch_assoc()){	?>				
							<option value="<?php echo $result['utr_number'];?>"><?php echo $result['utr_number'];?></option>
							<?php } } ?>
							</select>
							
							<?php if(isset($utrError)) { echo '<span style="color: red;"/>'.$utrError.'</span>'; } ?>
							</div>
							
							<div class="form-group-inner col-100">
							<label>TDS Amount(If Any) : </label>
							<input type="number" class="form-control" id="tdsAmount" name="tdsAmount" value="<?php echo $tdsAmount;?>"/>
							<?php if(isset($tdsAmountError)) { echo '<span style="color: red;"/>'.$tdsAmountError.'</span>'; } ?>
							<?php if(isset($tdsNameSuccess)){ echo '<span style="color: green;"/>'.$tdsNameSuccess.'</span>';} ?>
							</div>
							</div>
							<div class="form-group " style="padding: 0;">
								<input type="submit" class="cta" id="submit" value="SAVE"/>
