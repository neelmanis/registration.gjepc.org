...
if($action == 'email_id'){
	...
	if($count>0) 
	{
		...
		//if($status =='Y'){
            ...
            $checkEvent = $conn->query("SELECT * FROM ivr_registration_details WHERE eid='$eid' AND uid='$uid' AND `trade_show`='IIJS 2021'");
...
if($action == 'send_otp'){
	...
	<tr><td style="background-color:#744F9E;padding:3px;"></td></tr>
		<tr>
            <td align="left">
                ...
                <div style="float: right;">
               	<img id="ri" src="https://registration.gjepc.org/images/IIJS-Premiere-2021-logo.png">
               </div>
	 ...
	 $headers .= 'CC: snehal.rane@gjepcindia.com' . "\r\n";
...
function uploadIntlVIsitor($file_name,$file_temp,$file_type,$file_size,$id,$name)
{
    ...
    if($certificate =='dose1'){
    	$messagev = "It is compulsory to carry Covid-19 Negative Report(RT PCR Test) done before 72 hrs of your first scheduled entry at IIJS Premiere 2021.";
    } else {
	...
	if($countData > 0){
		...
		if($certificate =='dose1'){
        $sqlx = "INSERT INTO `visitor_lab_info` (`registration_id`, `visitor_id`, `pan_no`, `mobile_no`,`email`, `via`,`certificate`,`dose1`, `status`,`approval_status`,`dose1_status`,`category_for`) VALUES ('$uid', '$eid', '$eid', '$eid','$email', '$via','$certificate','$vaccine_certificate','1','P','P','INTL')";
		} else {
        $sqlx = "INSERT INTO `visitor_lab_info` (`registration_id`, `visitor_id`, `pan_no`, `mobile_no`,`email`, `via`,`certificate`,`dose2`, `status`,`approval_status`,`dose1_status`,`category_for`) VALUES ('$uid', '$eid', '$eid', '$eid', '$email','$via','$certificate','$vaccine_certificate','1','P','P','INTL')";
		}
