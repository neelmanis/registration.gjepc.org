...
if($action == 'pan_number'){
    ...
    $registration_id = intval(filter($row['registration_id']));
	  	$sqlExhibitor = "select * from exh_reg_payment_details where uid='$registration_id' AND `show`='IIJS 2021' AND `year`='2021' AND allow_visitor='N' order by payment_id desc limit 0,1";
		...
		$checkHistory = "SELECT * FROM `visitor_order_history` WHERE visitor_id='$visitorID' AND (payment_made_for='iijs21' OR payment_made_for='igjme21') AND `status`='1' AND payment_status='Y'";
...
if(isset($registration_id) && $registration_id!=""){
	...
	// 	 For Machinery Start
		if($payment_made_for=="igjme21" && $temp_total_amount==0 )
		{
		if(isset($delivery_id) && !empty($delivery_id)) {
				
		$sqlx3 = "INSERT `visitor_order_detail` SET `type_of_member`='$type_of_member',`delivery_id`='$delivery_id',`billing_delivery_id`='$billing_delivery_id' , `regId`='$registration_id' , `orderId`='$orderId' , `paymentThrough`='single',`payment_type`='free',`total_payable`='$amount',`create_date`='$post_date',year='2021',event='$show'"; 
...
$get_partner = $row['partner'];
 $designationType = $row['degn_type'];
   ...
   if($updateResult ==TRUE){
   	$message = "Dear ".$first_name.", Welcome to IIJS Premiere 2021, your data for Visitor has been updated successfully, you will be notified on approval/disapproval";
