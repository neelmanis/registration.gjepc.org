...
if($action == 'pan_number'){
	...
	if($count>0) 
	{
		...
		$registration_id = intval(filter($row['registration_id']));
	  	$sqlExhibitor = "select * from exh_reg_payment_details where uid='$registration_id' AND `show`='$shortcode' AND `year`='$year' AND allow_visitor='N' order by payment_id desc limit 0,1";
		...
		//  $checkHistory = "SELECT * FROM `visitor_order_history` WHERE visitor_id='$visitorID' AND (payment_made_for='$shortcode') AND `status`='1' AND payment_status='Y'";
		...
		// 		$data = array("status"=>'warning', "message"=>"Already registered for $event_name. To get your badge please upload your vaccination certificate on below given link");
		...
		$_SESSION['registration_id'] = $row['registration_id']; 
       	  ...
       	  $data = array("status"=>'success',"company"=>$company, "mobile"=>$mobile,"email"=>$email,"name"=>$name,"lname"=>$lname,"photo"=>$photo,"reg_id"=>$reg_id,"visitor_id"=>$visitorID,"designation"=>$designation);
...
if($action == 'send_otp'){
    ...
    $messageEmail ='<table cellpadding="10" width="100%" style="margin:0px auto;font-family: Roboto; background-color:#eeee;padding:30px; border-collapse:collapse;">
		<tr><td colspan="3" style="background-color:#744F9E;padding:3px;"></td></tr>
			<tr>
