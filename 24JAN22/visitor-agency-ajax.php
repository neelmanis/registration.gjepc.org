...
if(isset($_POST['action']) && $_POST['action']=="confirm_show")
{
	...
	if(!empty($registration_id )){
		if($visitor_id !==""){
			 $sql = "UPDATE `globalExhibition` SET `status` = 'Y' WHERE `visitor_id` = '$visitor_id' AND registration_id='$registration_id' AND participant_Type='$participant_Type' AND status='P'";
