...
/*
$checkHistory = "SELECT * FROM `visitor_order_history` WHERE (payment_made_for='6show' OR payment_made_for='5show' OR payment_made_for='4show' OR payment_made_for='3show' OR payment_made_for='2show' OR payment_made_for='1show' OR payment_made_for='combo') AND `status`='1' AND payment_status='Y' limit 1"; 
$resultQuery = $conn ->query($checkHistory);
$num = $resultQuery->num_rows;
...
if($num>0)
{
	...
	echo	$sqlx2 = "insert into visitor_order_detail set orderId='$orderId',regId='$registration_id',payment_mode='dumps',total_payable='0',payment_status='Y',create_date=NOW(),year='2021',event='iijs21',paymentThrough='dumps',agree='Y'";
	//	$result2 = $conn->query($sqlx2);
		if($result2){
		$sqlx1 = "INSERT INTO `visitor_order_history`(`create_date`, `registration_id`, `orderId`,`visitor_id`, `payment_made_for`, `amount`,`show`, `year`,`payment_status`, `paymentThrough`,`status`) VALUES (NOW(),'$registration_id','$orderId','$visitor_id','$payment_made_for','0','$payment_made_for','2021','Y','dumps','1')";
	//	$result = $conn->query($sqlx1);
