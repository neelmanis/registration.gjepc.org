...
function getguestcountry($guest_country)
{
...
function getCovidStatus($reg_id,$vis_id,$type,$conn){
	$query_sel = "SELECT approval_status FROM visitor_lab_info where `registration_id`='$reg_id' AND `visitor_id`='$vis_id' AND `category_for`='$type'";	
	$result_sel = $conn->query($query_sel);								
	$row = $result_sel->fetch_assoc();		
	return $row['approval_status'];
}
function getCovidDose($reg_id,$vis_id,$type,$conn){
	$query_sel = "SELECT dose2_status FROM visitor_lab_info where `registration_id`='$reg_id' AND `visitor_id`='$vis_id' AND `category_for`='$type'";	
	$result_sel = $conn->query($query_sel);								
	$row = $result_sel->fetch_assoc();		
	return $row['dose2_status'];
}
function getVendorStatusFromGlobal($reg_id,$vis_id,$type,$conn){
	$query_sel = "SELECT status FROM globalExhibition where `registration_id`='$reg_id' AND `visitor_id`='$vis_id' AND `participant_Type`='$type'";	
	$result_sel = $conn->query($query_sel);								
	$row = $result_sel->fetch_assoc();		
	return $row['status'];
}
...
{
	$query_sel = "SELECT short_name FROM  visitor_vendor_category  where id='$id'";	
	$result_sel = $conn->query($query_sel);								
	$row = $result_sel->fetch_assoc();	
