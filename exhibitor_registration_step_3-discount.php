...
if($action=="Save")
{
	...
	$incentive_value = $conn->real_escape_string($_REQUEST['incentive_show']);	
	if($incentive_value=='')
		$incentive_value=0;
	
	$discount_value = $conn->real_escape_string($_REQUEST['discount_show']);	
	if($discount_value=='')
		$discount_value=0;
	
	$incentive = $conn->real_escape_string($_REQUEST['incentive']);	
...
$(document).ready(function(){
  ...
  woman_entrepreneurs=$('input[name="woman_entrepreneurs"]:checked').val();
  ...
  <?php } ?>
 //alert(woman_entrepreneurs);
		$.ajax({ 		
					type: 'POST',
					url: url,
					data: "actiontype=calculatePayment&&section="+section+"&&selected_area="+selected_area+"&&selected_scheme_type="+selected_scheme_type+"&&selected_premium_type="+selected_premium_type+"&&woman_entrepreneurs="+woman_entrepreneurs+"&&category="+category+"&&last_yr_participant="+last_yr_participant+"&&discount="+discount+"&&incentive="+incentive,
					dataType:'html',
					beforeSend: function(){
							$('#preloader').show();
							$('#status').show();
					...
					success: function(data){
							   // alert(data); return false; exit;
								...
								$('#sub_total_cost').val(data[7]);
								$('#security_deposit').val(data[8]);
								$('#govt_service_tax').val(data[9]);
								$('#grand_total').val(data[10]);
...
<div class="inner_container">
	...
	if($event_selected=='iijs'){ $lastYearEvent = "IIJS 2020"; }
	...
	$currentYear = "select * from exh_registration where uid='$registration_id' AND `show`='$event_for' AND event_selected='$event_selected'";
	$query=$conn->query($currentYear);
	...
	else
	{
		...
		$discount  = trim($result['discount']);		
		$incentive = trim($result['incentive']);		
    }
	?>
        <!--<div class="field_box">
		...
		<input type="hidden" name="last_yr_participant" id="last_yr_participant" value="<?php echo $last_yr;?>"/>
        <div class="field_box">
            <div class="field_name">Membership Status : ACTIVE <?php echo $_SESSION['member_type'];?></div>
            <div class="clear"></div>
        </div>
        <?php  if($num1>0){ ?>
        <div class="field_box">
            <div class="field_name">IIJS Premiere 2021 ROI Details / IIJS Premiere 2020 Allotment Details (If Not applied for IIJS Premiere 2021 ROI) :</div>
            <div class="field_input" style="padding-top:5px; line-height:25px;">
            <strong>Section :</strong> <?php echo strtoupper($section);?><br />
            <strong>Area :</strong> <?php echo strtoupper($selected_area);?> Sqmtr <br />
            <strong>Scheme type :</strong> <?php echo getSchemeDescription($selected_scheme_type,$conn);?> <br />
       ...
       <div class="field_box">
        ...
        <?php } else { ?>
		<?php if($event_selected=='iijs'){ ?>
        <input type="radio" id="option" class="bgcolor" name="option" value="Same stall" <?php /* if($option=="Same stall"){?> checked="checked" <?php } */?> /> Same stall area as applied in ROI at IIJS Premiere 2021<br/>
		...
		<?php if($event_selected=='iijs'){ ?>
        <div class="field_box">
        <div class="field_name">Select your area in Sqmtr<span>*</span>:</div>
        <div class="field_input"> 
		...
		<input type="hidden" name="selected_premium_type" id="selected_premium_type_hid" value="normal">
		<?php } else { ?>
		<div class="field_box">
        <div class="field_name">Select your premium type <span>*</span>:</div>
        <div class="field_input">
		...
		<div class="field_box">
        <div class="field_name"><strong>Space cost after Incentive </strong> @<?php echo $incentive;?>per sqmtr:</div>
        <div class="field_input"><input type="text" class="bgcolor" name="incentive_show" id="incentive_show" readonly="readonly" value="<?php echo $incentive_show;?>"/></div>
        <div class="clear"></div>
        </div> 
		
		<div class="field_box">
        <div class="field_name"><strong>Discount as per Membership Type Rs </strong><?php echo $discount;?>:</div>
        <div class="field_input"><input type="text" class="bgcolor" name="discount_show" id="discount_show" readonly="readonly" value="<?php echo $discount_show;?>"/></div>
        <div class="clear"></div>
        </div> 
        ...
        <div class="clear" style="height:10px;"></div>  
			...
			<img src="https://registration.gjepc.org/images/step3.jpeg" />
			</div>		
		</div>
