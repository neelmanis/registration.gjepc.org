    ...
    <form method="POST" name="eventForm" id="eventForm" autocomplete="off">
        ...
        <div class="d-flex flex-column ">
              <div class="d-flex flex-row form-setup">
				...
				$get_applied_events = $conn->query("SELECT * FROM exh_registration where uid='$registration_id' ");
				;
				$applied_events = [];
				while ($result_applied_events = $get_applied_events->fetch_assoc()) {
					$applied_events[] = $result_applied_events['show'];
				}



				if($_SESSION['COUNTRY']=="IN")
				//$getEvent = "SELECT * FROM `exh_event_master` WHERE id=1 OR id=3 OR id=4 order by `serial` asc";
				 $getEvent = "SELECT * FROM `exh_event_master` WHERE `status` ='1' order by `serial` asc";
				else 
				 $getEvent = "SELECT * FROM `exh_event_master` WHERE (`id` =1 OR `id`=4 ) ";			 
				$eventResult = $conn->query($getEvent);
				while($eventRow = $eventResult->fetch_assoc()){
           if(!in_array($eventRow['eventDescription'], $applied_events)){
