...
/*-------------------------------------------Signature Data Insert Process Start--------------------------------------------------------*/
	$checks_signature = "select uid,gid from exh_reg_payment_details where uid='$uid' AND `show`='IIJS SIGNATURE 2023' or `show`='IIJS TRITIYA 2023'  limit 0,1";
	...
	if($numCount_signature >0)
	{
		...
		if($gid_signature!='') //check gid is not blank
		{
			...
			if($gid_tritiya!='') //check gid is not blank
			{
				...
				// }else if($_POST["tds_holder"] == 'Yes' && $getstallInfo['tritiya_last_yr_participant'] == "NO") {
