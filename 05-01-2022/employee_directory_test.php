...
{		
		...
		$degn_type=mysql_real_escape_string($_REQUEST['radio1']);
		$designation=mysql_real_escape_string($_REQUEST['designation']);
		$designation_id=mysql_real_escape_string($_REQUEST['designation_id']);
		...
		$status=mysql_real_escape_string($_REQUEST['agree']);
	...
	if(isset($_FILES['photo']) && $_FILES['photo']['name']!="")
	{
		...
		if($photo_name!="")
		{
			...
			$photo=uploadEVRImage($photo_name,$photo_temp,$photo_type,$photo_size,$mobile,$attach);
		}
	}
	...
	if(isset($_FILES['pan_copy']) && $_FILES['pan_copy']['name']!="")
	{
		...
		if($pan_name!="")
		{
			...
			$pan_copy=uploadEVRImage($pan_name,$pan_temp,$pan_type,$pan_size,$mobile,$attach);
		}
	}
	...
	if(isset($_FILES['salary']) && $_FILES['salary']['name']!="")
	{
		...
		if($salary_name!="")
		{
			...
			$salary=uploadEVRImage($salary_name,$salary_temp,$salary_type,$salary_size,$mobile,$attach);
		}
	}
	...
	if(isset($_FILES['partner']) && $_FILES['partner']['name']!="")
	{
		...
		if($partner_name!="")
		{
			...
			$partner=uploadEVRImage($partner_name,$partner_temp,$partner_type,$partner_size,$mobile,$attach);
		}
	}
$sqlx="SELECT mobile, email, aadhar_no, pan_no FROM visitor_directory WHERE registration_id='$registration_id'";
...
{$signup_error="Please upload the image";}
elseif($designation=='18' && checkOwnerType($registration_id)=='1')
...
{     
		 ...
		 $resultx = mysql_query($sql1);
		 if($resultx)
		 {
		 $message = "Dear ".$name.", Welcome to Signature 2020, your data for Visitor badge has been updated successfully, you will be notified on approval/disapproval";
		 get_data($message,$mobile); 
		 }
...
$sqlx="SELECT mobile, email, aadhar_no, pan_no,designation FROM visitor_directory WHERE visitor_id!='$id' and registration_id='$registration_id'";
$result = mysql_query($sqlx);
$match_mob = array();
$match_email = array();
$match_aad = array();
...
/*	$sql="update visitor_directory set registration_id='$registration_id', shows='$shows', year='$year', gender='$gender', name='$name', lname='$lname', degn_type ='$degn_type', designation='$designation', mobile='$mobile', email='$email',aadhar_no='$aadhar_no', pan_no='$pan_no', photo='$photo', pan_copy='$pan_copy', salary_slip_copy='$salary', partner='$partner', visitor_approval='U', mod_date='$mod_date', isApplied='Y' where visitor_id='$id'"; */
...
var par = $('#p').val();
	...
	$("#form").validate({
		...
		messages: {	
			...
			agree:{
				required: "Please click on aggree button",
			},
	 }
	});
});
...
<div class="inner_container">
  <div class="container">
    <div class="container_leftn">
      <div class="breadcome"><a href="#">Home</a> > Employee Registration </div>
      <div><p style="color:#FF0000;float:left;display: inline;margin-top: 18px"><b>Visitor can apply Registration / make Payment after Approval of Below records</b></p><a href="visitor_registration.php" style="float: right;" class="submitbtn" title="Select Show And Make Payment">Registration Application</a></div>
     <div class="clear"></div>
      ...
      <div id="loginForm">
        ...
        <?php
		$sqlmaster = mysql_query("select * from registration_master where id = '$registration_id'");
		while($rows = mysql_fetch_array($sqlmaster))
		{
		...
		?>         
        <div class="gp_wrp">			
            <form action="" class="cmxform" method="post" name="formupload" id="formupload" enctype="multipart/form-data">		  		
                <div class="field">
                	<label>Company Document</label>
                	<input id="gst_copy" name="gst_copy" type="file" class="textField textField2 imgsel" value="" accept=".jpg,.jpeg,.png,.pdf" data-imgsrc="blah">
				<b>Kindly upload Max 2MB. jpeg, png, .pdf</b></div>                
              	<div class="field">
                	<label>Company PAN Copy</label>
                	<input id="pan_no_copy" name="pan_no_copy" type="file" class="textField textField2 imgsel" value="" accept=".jpg,.jpeg,.png,.pdf" data-imgsrc="blah">  
				</div>				
		...
		<table>
            <thead>
              <tr>
                <th scope="col">Company Doc</th>
                <th scope="col">PAN</th>
              </tr>
            </thead>
			<tbody>
              <tr>
				<td data-label="GST" width="50%"> <?php if($gst_copy_ext =="pdf"){?><a href="images/gst_copy/<?php echo $gst_copy;?>
					" target="_blank"><img src="images/pdf_icon.png"></a> <?php }else{?><img src="images/gst_copy/<?php echo $gst_copy;?>" class="emp_img">  <?php } ?><span class="label"><?php 
				if($company_gstn =="NULL"){echo "Not Applicable";}else{echo $company_gstn;}?></span></td>
				<td data-label="PAN" valign="top" width="50%"> <?php if($pan_no_copy_ext =="pdf"){?><a href="images/gst_copy/<?php echo $gst_copy;?>
					" target="_blank"><img src="images/pdf_icon.png"></a> <?php } else { ?> <img src="images/pan_no_copy/<?php echo $pan_no_copy;?>" class="emp_img"> <?php } ?><span class="label"><?php echo $company_pan_no;?></span></td>				
              </tr>
              <?php if($getimg['approval_status']=="D"){ ?>
              <tr>
              <td colspan="2"><p style="text-align: center;"><span>Remark:&nbsp;&nbsp;&nbsp;</span><span style="color: red"><?php echo $getimg['disapprove'];?></span></p></td>             
              <?php } ?>
              </tr>
             </tbody>
        </table>
        ...
        <div id="formContainer">
          ...
          <div class="title" id="title_open">
            <h4>NEW REGISTRATION / ADD VISITOR<div class="plus">+</div> <div class="minus">-</div></h4>
...
<?php 
  if(($_REQUEST['id']!='') || ($_REQUEST['action']=='edit'))
  {
		...
		$getCount = mysql_num_rows(mysql_query("SELECT * FROM `visitor_directory` WHERE registration_id='$registration_id' AND (designation='19' || designation='20')"));
...
?>
          ...
          <form action="" class="cmxform" method="post" name="from" id="form" enctype="multipart/form-data">
		  <div id="radio" style="font-size: 14px;">
            ...
            <div id="form1">
			  ...
			  <div class="field" id="Owner_degn">
                <label><sup>*</sup>Designation :  </label>
                <select class="select_v" name="designation" id="designation">                                  
              ...
              <div class="field">
                ...
                <input id="name" name="name" type="text" onkeypress="return onlyAlphabets(event,this);" maxlength="14" class="textField" value="<?php echo $name; ?>"/>
              </div>
              
              <div class="field">
                <label> <sup>*</sup>Last Name : </label>
                <input id="lname" name="lname" type="text" onkeypress="return onlyAlphabets(event,this);" maxlength="14" class="textField" value="<?php echo $lname; ?>"/>
              </div>
              
              <div class="field">
                <label><sup>*</sup>Gender :  </label>
                <select class="select_v" name="gender" id="gender">                                  
              ...
              <div class="field">
                ...
                <input id="mobile" name="mobile" type="text" class="textField" maxlength="10" value="<?php echo $mobile; ?>"/>
              </div>
			                
              <div class="field">
                <label><sup>*</sup>E-Mail :  </label>
                <input id="email" name="email" type="text" class="textField" value="<?php echo $email; ?>"/>
              </div>          

              <div class="field">
                <label><sup>*</sup>Individual PAN No :  </label>
                <input id="pan_no" name="pan_no" type="text"  class="textField" maxlength="10" value="<?php echo $pan_no; ?>"/>
              </div>
			  
              <div class="clear"></div>
              <div class="field">
                <label style="height: 65px;"> <sup>*</sup>Photo Passport size with white background : (Max 2MB.jpg, .png, .pdf)<br></label>
                <input id="photo"  name="photo" type="file"  class="textField textField2 imgsel" value="<?php echo $photo; ?>" data-imgsrc="blah"/>  
                <input type="hidden" id="ph" name="ph" class="textField textField2 imgsel" data-imgsrc="blah" value="<?php if(empty($photo)){ echo 0;} else { echo 1;} ?>"/>
				<?php if(!empty($photo)){?>
              ...
              <div class="field">
                <label style="height: 65px;"><sup>*</sup> Individual Pan Card : (Max 2MB.jpg, .png, .pdf)<br></label>
                <input id="pan_copy" name="pan_copy"  type="file"  class="textField textField2 imgsel"  data-imgsrc="blah2" value="<?php echo $pan_copy;?>"/>
                <input type="hidden" id="pn" name="pn" class="textField textField2 imgsel" data-imgsrc="blah2" value="<?php if(empty($pan_copy)){ echo 0;} else { echo 1;} ?>"/>
              ...
              <div class="field" id="salary_slip">
			  ...
			  ?>
                <label style="height: 65px;"><label  id="emp_text" style="display:inline;"> Salary Slip / Bank Statment</label>: (Max 2MB.jpg, .png, .pdf)</label>
                <input id="salary" name="salary" type="file"  class="textField textField2 imgsel" data-imgsrc="<?php echo $blah; ?>" value="<?php echo $emp_files; ?>"/>
		...
		<!--  -->
          ...
          <div class="title margin_t">
            <h4>OWNER DETAILS</h4>
            <p style="color: red;font-weight:bold;">Visitor Already Registered for 3/6 Shows during IIJS PREMIERE 2019 need to preserve the badge</p>
          </div>
          ...
          <table>
            ...
            <tbody>
            <?php
			$query_owner = mysql_query("select * from visitor_directory where degn_type = 'Owner' and registration_id='$registration_id' and status='1'");
			while($owner = mysql_fetch_array($query_owner))
			{
			...
			$sqlBadgeOwn =  "select * from visitor_order_history where visitor_id='$visitor_id' and payment_status='Y' and year='2019' and (payment_made_for='3show' || payment_made_for='6show') and `show`='iijs' and registration_id='$registration_id'";
		  $resultBadgeOwn = mysql_query($sqlBadgeOwn);
		  $ownBadgeCount = mysql_num_rows($resultBadgeOwn); 
			?>

              <tr>
                <td data-label="Pattern Name"><?php echo strtoupper($owner['name']); ?></td>
                <td data-label="SIZE"><?php echo getVisitorDesignationID($owner['designation']); ?></td>
                <td data-label="Star Rating"><?php echo $owner['mobile']; ?></td>
				<td data-label="TRACode"><?php if($owner_photo_ext =="pdf" || $owner_photo_ext =="PDF"){?><a href="images/employee_directory/<?php echo $_SESSION['USERID'];?>/photo/<?php echo $owner['photo'];?>" target="_blank"><img src="images/pdf_icon.png"></a><?php }else{?>   <img src="images/employee_directory/<?php echo $_SESSION['USERID'];?>/photo/<?php echo $owner['photo'];?>" class="emp_img"><?php } ?></td>
                <td data-label="Star Rating">
				<?php 
				if($visitor_approval=="Y"){
				echo "Approved"; if($combo=="3show") { echo "<br> <b><hr>Already registered for 3 Shows <hr>(IIJS 2019+ Signature 2020+ IIJS 2020)</b>"; } 
          ...
          <table>
            ...
            <tbody>
          <?php 
		  $query_emp = mysql_query("select * from visitor_directory where degn_type = 'Employee' and registration_id='$registration_id' and status='1'");
		  ...
		  $combo = $emp['combo'];
		  $disapproval = $emp['disapprove_reason'];
		  $employee_photo_ext =  pathinfo($emp['photo'], PATHINFO_EXTENSION);
		  $sqlBadgeEmp =  "select * from visitor_order_history where visitor_id='$visitor_id' and payment_status='Y' and year='2019' and payment_made_for='3show' and `show`='iijs'";
		  $resultBadgeEmp = mysql_query($sqlBadgeEmp);
		  $empBadgeCount = mysql_num_rows($resultBadgeEmp);
		  ?>
              <tr>
                <td data-label="Pattern Name"><?php echo strtoupper($emp['name']); ?></td>  
                <td data-label="SIZE"><?php echo getVisitorDesignationID($emp['designation']); ?></td>
                <td data-label="Star Rating"><?php echo $emp['mobile']; ?></td>
				<td data-label="TRACode"><?php if($employee_photo_ext =="pdf" || $employee_photo_ext =="PDF"){?><a href="images/employee_directory/<?php echo $_SESSION['USERID'];?>/photo/<?php echo $emp['photo'];?>" target="_blank"> <img src="images/pdf_icon.png"></a><?php }else{?>
...
#title_open h4{padding: 10px 15px; margin-top:0px;}
...
.label{display: inline-block;padding: 5px;border:1px solid#000;width: 50%;float: right;}
