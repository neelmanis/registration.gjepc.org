	...
	if($count>0) 
	{
      ...
      $face_isApplied = $row['face_isApplied']; 
		...
		/*CHECK ENTRY IN GLOBAL*/
		 $sqlGlobal = "SELECT * FROM gjepclivedatabase.globalExhibition  WHERE   visitor_id='$eid' AND  registration_id='$uid' AND (participant_Type ='INTL')  ";
		$resultGlobal = $conn->query($sqlGlobal);
		$rowGlobal = $resultGlobal->fetch_assoc();
		$countGlobal = $resultGlobal->num_rows;
    if($countGlobal == 0){
				$data = array("status"=>'error', "message"=>"You are not registered for any show.");
				echo json_encode($data); exit;
    }else{
    	$globalStatus = $rowGlobal['status'] ;
    	
	    // if($globalStatus !="Y"){
	    // 		$data = array("status"=>'error', "message"=>"Vaccine certificate approval is pending. ");
					// echo json_encode($data); exit;
	    // }
		...
		if($face_isApplied =="N"){
				// $data = array("status"=>'error', "message"=>"Your face recognition photo is not updated please <a href='https://registration.gjepc.org/international_visitor_photo_update.php?key=".base64_encode($email_id)."' class='link' target='_blank' > click here </a> to update the same.");
				// echo json_encode($data); exit;
		}

		if($face_status !="Y"){
				// $data = array("status"=>'error', "message"=>"Your face recognition photo approval is pending.");
				// echo json_encode($data); exit;
		}
		if($status =='Y'){
			...
			$_SESSION['participant_Type'] = $participant_Type ;
		   $_SESSION['registration_id'] = $row['uid']; 
			$_SESSION['visitor_id'] =  $row['eid'];
...
if($action == 'send_otp'){
	...
	if($count>0) 
	{
		...
		if($UpdateEmailOtp){
		 $updateOtpGlobal = $conn->query("UPDATE globalExhibition SET otp='$otp',isVerified ='0' WHERE email='$email' AND visitor_id='$eid' AND registration_id = '$uid'");
...
if($action == 'verifyOTP'){
    ...
    $updateGlobal = $conn->query("UPDATE globalExhibition SET isVerified ='1' WHERE email='$email' AND visitor_id='$eid' AND registration_id = '$uid'");
...
function uploadIntlVIsitor($file_name,$file_temp,$file_type,$file_size,$id,$name)
{
	...
	<tr><td style="background-color:#744F9E;padding:3px;"></td></tr>
	  ...
	  <td>Your Vaccination Certificate has been uploaded successfully. We will notify you on approval/disapproval of the document.</td></tr>
	   ...
	   <tr><td><b>Email us on :</b>  daphne.dcosta@gjepcindia.com, naheed@gjepcindia.com
	 </td></tr> 
	</table>';
  
	 $to = $email.',daphne.dcosta@gjepcindia.com, naheed@gjepcindia.com';
