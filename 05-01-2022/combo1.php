...
$signature_show="IIJS Signature 2020";
...
<script>
 $("#selected_area").live('change',function(){
    ...
    $.ajax({ type: 'POST',
					url: 'ajax.php',
					data: "actiontype=getPremiumYype&&selected_area="+selected_area,
					dataType:'html',
					beforeSend: function(){
							$('#preloader').show();
							$('#status').show();
	...
	if($('#selected_premium_type_hid').val()!='')
	 ...
	 $.ajax({ type: 'POST',
					...
					success: function(data){//alert(data);
						...
						/*$('#selected_scheme_type').prop("selectedIndex", 0);
						$('#selected_premium_type').prop("selectedIndex", 0);
						*/
						if($.trim(res[2])!=""){
...
<div class="inner_container">
    ...
    <form action="" method="post" name="form1" id="form1" style="width:50%; float:left; border-right:2px dotted #ddd; box-sizing:border-box;">
	...
	$query1=mysql_query("select * from exh_registration where uid='$registration_id' and `show`='IIJS Signature 2019' and last_yr_section_flag='1' and  curr_last_yr_check='N' order by exh_id desc limit 0,1");
	$result1=mysql_fetch_array($query1);
	$exh_id=$result1['exh_id'];
	$retain_restrict=$result1['retain_restrict'];
	$num1=mysql_num_rows($query1);

	$query=mysql_query("select * from exh_registration where uid='$registration_id' and gid='$gid' and `show`='IIJS Signature 2020'");
	...
	?>
        <div class="field_box">
            ...
            <div class="chkbox" style="width:100px;">
			  ...
			  <?php //echo $last_yr =  $result1['last_yr_participant'];?>
              </span>
       ...
       <div class="field_box">
        ...
        <?php if($num1==0){?>
        <input type="radio" id="option" class="bgcolor" name="option"  checked="checked" value="New participant" /> 
         
         <label for="b_1">New participant</label><br />
        <?php } else { ?>
        ...
        Same area but different location as of previous year IIJS 2018<br/>  
         
        <?php if($selected_area<54){/* ?>
        <input type="radio" id="option" class="bgcolor" name="option" value="More area than previous year IIJS" <?php if($option=="More area than previous year IIJS 2018"){?> checked="checked" <?php } ?> /> 
        More area than previous year IIJS 2018<br/>
        <?php */} ?>
       ...
       <?php if($_SESSION['member_type']== 'NON_MEMBER') {?>
       		<!--<p>Note:</p>
       		<ul style="color:#F00">
			<li>To Apply in jewellery / loose stone section kindly update your GJEPC membership</li>
            <li>Space booking for machinery section is not live. We will update once it is open.</li>
            </ul>-->
        <?php }?>
        <div class="field_box">
          ...
          <option selected="selected" value="">-----Select Section----</option>
			...
			if($_SESSION['COUNTRY']=="IN")
			{
				if($_SESSION['member_type']=='MEMBER')
					$sql="SELECT * FROM iijs_section_master where member_type ='MEMBER' and status='Y'";
				else
    ...
    <form action="" method="post" name="form2" id="form2" style="width:50%; float:left;">
	...
	/*...........................Last year participant  ..........................*/
	//echo "select * from exh_registration where uid='$registration_id' and `show`='IIJS Signature 2019' and last_yr_section_flag='1' and  curr_last_yr_check='N' order by exh_id desc limit 0,1";
	$query_signature=mysql_query("select * from exh_registration where uid='$registration_id' and `show`='IIJS Signature 2019' and last_yr_section_flag='1' and  curr_last_yr_check='N' order by exh_id desc limit 0,1");
	$result_signature1=mysql_fetch_array($query_signature);
	$signature_exh_id=$result_signature1['exh_id'];
	$num1=mysql_num_rows($query1);	

	$query=mysql_query("select * from exh_registration where uid='$registration_id' AND gid='$gid' AND `show`='$signature_show'");
	$resultx=mysql_fetch_array($query);
	$num=mysql_num_rows($query);
	if($num>0)
	{
	...
	?>
        <div class="field_box">
            ...
            <div class="chkbox" style="width:100px;">
              <span class="chkbox" style="width:100px;">
              <?php if($result_signature1['last_yr_participant']=='YES'){ echo $last_yr="YES"; ?>  
			<?php } else {echo $last_yr="NO";}?>
              </span>
              <input type="hidden" name="last_yr__signature_participant" id="last_yr__signature_participant" value="<?php echo $last_yr;?>"/>
             </div>
            </div>
            <div class="clear"></div>
        </div>
        ...
        <?php if($num1==0){?>
        <input type="radio" id="signature_option" class="bgcolor" name="signature_option" checked="checked" value="New participant"/><label for="b_1">New participant</label><br />
        <?php } else { ?>
        ...
        <div class="field_name">Select your section <span>*</span>:</div>
        <div class="field_input">
          <select name="signature_section" id="signature_section"  class="bgcolor" <?php if($signature_section!="" && ($_SESSION['COUNTRY']=="IN") && $num1!=0){?> disabled="disabled" <?php }?>>
          <option selected="selected" value="">-----Select Section----</option>
            <?php
			if($_SESSION['COUNTRY']=="IN")
			{
			$sql="SELECT * FROM `signature_section_master` order by section";		
            $query=mysql_query($sql);
            while($result=mysql_fetch_array($query)){
            ?>
          ...
          <?php if($signature_section!="" && $num1>0){?><input type="hidden" name="signature_section" id="signature_section_hid" value="<?php echo $signature_section;?>"/><?php }?>          
        </div>
        <div class="clear"></div>
        </div>
        ...
        <div class="field_name">Select your area <span>*</span>:</div>
        <div class="field_input"> 
          <select name="signature_area" id="signature_area" class="bgcolor" <?php if($signature_area!=""){?> disabled="disabled"<?php }?>>
            <option selected="selected" value="">-----Select Area----</option>
            <?php
			$sql="SELECT * FROM signature_area_master order by area_id asc limit 0,1" ;
			$query=mysql_query($sql);
			while($result=mysql_fetch_array($query)){ ?>
        ...
        <input type="hidden" id="signature_selected_scheme_type" name="signature_selected_scheme_type" value="BI"/>
    ...
    <div class="field_box" style=" display: table;text-align: center; width: 100%; margin:0;">
	<a href="exhibitor_registration_step_2.php" class="button">Prev</a>
	<input type="hidden" name="action" value="Save" />
        <input type="submit" id="submit" class="button" value="Proceed to next step" style="float:none; margin:2% auto 0 auto;" disabled="disabled" />
    </div>
    </div>
    
    <div class="container_right">
	<table align="center" width="100%" cellpadding="8" border="1" bordercolor="#ddd" style="color:#333333; line-height:20px; font-size:12px; border-radius:10px; border-collapse:collapse;">
  
  <tbody>
    <tr><td colspan="3"><strong>Cost of Participation &amp; Premium Charges:</strong></td></tr>
    <tr><td><strong>Type</strong></td>
      <td><strong>Built-in Space Cost Per Sqmt. (Rs.)</strong></td>
    </tr>    
    <tr>
      <td>Mechinery Member</td>
      <td>Rs. 16500 / USD 325</td>
    </tr>    
    <tr>
	<td>Mechinery Non Member</td>
      <td>Rs. 17500 / USD 325</td>
    </tr>
    <tr>
      <td>Premium On Corner Location (2 Side / 3 Side open)</td>
      <td>10% Over &amp; Above to space cost</td>
    </tr>
    <tr>
      <td>Premium On Island Booth (4 Side open)</td>
      <td>15% Over &amp; Above to space cost</td>
    </tr>
    <tr>
      <td>Premium Booth Location (Main Aisle)</td>
      <td>25% Over &amp; Above to space cost</td>
    </tr>
    <tr>
      <td>Premium For Mezzanine Booth</td>
      <td>50% Over &amp; Above to space cost</td>
    </tr>
    <tr>
    ...
    <td>18%</td>
          ...
          <div class="cont_link" style="margin:2% 2% 0 2% ;">
            <div class="cont_head_inner"> <strong> Note : </strong> </div>
...
<style>
.field_box .field_name {width:100%; margin:5px 0; font-weight:bold; font-size:12px;}
...
.form_wrp {width:60%; float:left; border-right:2px dotted #ddd;}
...
.field_box .field_input {display:block; width:100%;}
