...
if($action == "get_employees"){
    ...
    if($show  =="signature23"){
      ...
      $sql_vis = "SELECT CONCAT(name, ' ', lname) AS visitor_name, visitor_id,visitor_approval FROM visitor_directory 
			WHERE NOT EXISTS 
			(SELECT * FROM visitor_order_history 
			WHERE visitor_directory.visitor_id = visitor_order_history.visitor_id and visitor_order_history.payment_status='Y' and (visitor_order_history.payment_made_for='iijstritiya23'  || visitor_order_history.payment_made_for='stcombo23' || visitor_order_history.payment_made_for='6show') ) AND `visitor_approval`='Y' AND `registration_id`='".$_SESSION['USERID']."'";
...
if($action == "get_fees"){
	...
	if($show == "signature23" && ($_SESSION['USERID']==600915475 || $_SESSION['USERID']==600916441 || $_SESSION['USERID']==600917013))
