...
//$sqlx = "SELECT id,post_date,uniqueIdentifier as 'unique_code',registration_id,visitor_id as 'RegistrationNumber',company,fname as 'Name',mobile,designation,photo_url,participant_Type as 'category',dose1_status,dose2_status,status,agency_category,committee,updateStatus,isReplaced FROM globalExhibition where 1 AND isDataPosted='N' and status='Y' and dose2_status='Y' order by id desc limit 0,500";

$sqlx = "SELECT id,post_date,uniqueIdentifier as 'unique_code',registration_id,visitor_id as 'RegistrationNumber',company,fname as 'Name',mobile,pan_no,designation,photo_url,participant_Type as 'category',dose1_status,dose2_status,status,agency_category,committee,updateStatus,isReplaced,days_allow,isDataPosted_Offline FROM globalExhibition where 1 AND isDataPosted_Offline='N' and participant_Type='CONTR' and status='Y' order by post_date desc limit 0,500"; /* CONTR */ 
...
if($username=="mukesh@kwebmaker.com" && $password=="123456"){
	if($sqlDatas->num_rows > 0) {
				...
				$download = "update gjepclivedatabase.globalExhibition set isDataPosted_Offline='Y' where id='$id'";
