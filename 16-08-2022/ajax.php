...
$logs = "INSERT INTO zoop_logs SET post_date = NOW(),document = 'PAN',document_no='$company_pan_no',name='$name',zoop_verified='$response_code',pan_type='$pan_type',section='VISITOR',ip='".$_SERVER['REMOTE_ADDR']."'";
...
<?php 
if(isset($_POST['actiontype']) && $_POST['actiontype']=="checkGstinApi"){
...
$logs = "INSERT INTO zoop_logs SET post_date = NOW(),document = 'GST',document_no='$company_gstn',name='$name',zoop_verified='$response_code',section='VISITOR',ip='".$_SERVER['REMOTE_ADDR']."'";
$result = $conn->query($logs);
exit;
...
?>
<?php
if(isset($_POST['actionType']) && $_POST['actionType']=="hotel_registration"){
...
function getVisitorName($id,$conn)
{
 ...
 if(count($visitors) > $company_quota){
 	echo json_encode(array("status"=>"error","message"=>"You are selecting more than assingned limit"));exit;
 }
 if(is_array($visitors) && count($visitors) == 2 ){
 	if($visitors[0] ==$visitors[1]){
		echo json_encode(array("status"=>"error","message"=>"Please Select different key persons"));exit;
    }
 }
 $check_history  = "SELECT * FROM visitor_hotel_registration WHERE registration_id='$registration_id' and event='$payment_made_for'";
 $result_history  = $conn->query($check_history);
 if($result_history->num_rows  > $company_quota ){
	echo json_encode(array("status"=>"error","message"=>"You are already submitted"));exit;
 }
 foreach($visitors as $visitor){
	$check_person  = "SELECT * FROM visitor_hotel_registration WHERE registration_id='$registration_id' and visitor_id='$visitor' and event='$payment_made_for'";
	$result_person = $conn->query($check_person);
	if($result_person->num_rows > 0 ){

         echo json_encode(array("status"=>"error","message"=>"Key Person ".getVisitorName($visitor,$conn)."  is already added"));exit;
	}
 }
foreach($visitors as $vis){
 $check_person_entry = $conn->query("SELECT * FROM visitor_hotel_registration WHERE  visitor_id='$vis' AND registration_id='$registration_id' AND event='$payment_made_for' ");
 if($check_person_entry->num_rows == 0){
	$insert  = "INSERT INTO visitor_hotel_registration SET visitor_id='$vis', registration_id='$registration_id', event='$payment_made_for'";
	$insert_id = $conn->query($insert);
 }
	
}
 echo json_encode(array("status"=>"success","message"=>"Application has been successfully submitted"));exit;


}
if(isset($_POST['actiontype']) && $_POST['actiontype']=="getVisitorInfo"){

 $visitor_id = $_POST['visitor_id'];

 $sql = "SELECT * FROM visitor_directory where visitor_id='$visitor_id'";
 $result = $conn->query($sql);
 $row = $result->fetch_assoc();
 echo json_encode(array("email"=>$row['email'],"mobile"=>$row['mobile']));exit;
...
?>
<?php
if(isset($_POST['actiontype']) && $_POST['actiontype']=="checkDesignationWithCompanyRegistration")
{ //print_r($_SESSION); print_r($_POST); exit;
...
if($designation=='Owner')
{
                ...
                if($num>0)
                {
                                ...
                                while($result=$query->fetch_assoc()){ ?>
                                <option value="<?php echo $result['id'];?>" <?php if($selected==$result['id']){?> selected="selected"<?php }?>><?php echo $result['type_of_designation'];?></option>
                                <?php }
                }
} else {
while($result=$query->fetch_assoc()){ ?>
...
?>
<?php
if(isset($_POST['actiontype']) && $_POST['actiontype']=="getCompanyPersonDetails"){
	$visitor_id = base64_decode($_POST['visitor_id']);
         $statement = "SELECT * FROM visitor_directory WHERE visitor_id = '$visitor_id'";
        $html = "";
            ...
            <thead>
                        ...
                        <td><b> Employee Name</b></td>                            
                           ...
                           <td><b>Designation</b></td>                            
                            <td>'. getVisitorDesignationID($row['designation'],$conn).' </td>                            
                        </tr>                       
                        <tr>
                            <td><b> Mobile </b></td>                            
                            <td>'. $row['mobile'].' </td>
                        ...
                        <td><b>  Photo</b></td>                            
                        <td>';
                       $photoext = pathinfo($row['photo'], PATHINFO_EXTENSION);
                            if($photoext =="pdf" || $photoext =="PDF"){
                       $html .=     '<a href="images/employee_directory/'.$_SESSION['regis_id'].'/photo/'. $row['photo'].'" target="_blank"><img src="images/pdf.png" alt="pdf File" title="Click Here to View"></a>';
                             } else { 
                       $html .= '<img src="images/employee_directory/'.$_SESSION['regis_id'].'/photo/'.$row['photo'].'" style="width:75px"> ';
                         } 
                       $html .= '</td>
						<td><b>  PAN Copy</b></td>                            
                        <td>';
						 $panext = pathinfo($row['pan_copy'], PATHINFO_EXTENSION);
                            if($panext =="pdf" || $panext =="PDF"){
                        $html .= '<a href="images/employee_directory/'.$_SESSION['regis_id'].'/pan_copy/'.$row['pan_copy'].'" target="_blank"><img src="images/pdf.png" alt="pdf File" title="Click Here to View"></a>';
                             }else{ 
                            $html .=   '<img src="images/employee_directory/'.$_SESSION['regis_id'].'/pan_copy/'.$row['pan_copy'].'" style="width:75px">';
						} 
                        $html .=   '</td>                            
                        </tr>                      
                        <tr> ';                       
                            if($row['degn_type']=="Employee"){
                           $html .=   ' <td><b>  Salary Slip Copy</b></td>
                            
                            <td colspan="3">';
                                $salaryext = pathinfo($row['salary_slip_copy'], PATHINFO_EXTENSION);
                            if($salaryext =="pdf" || $salaryext =="PDF"){
                            $html .='<a href="images/employee_directory/'.$_SESSION['regis_id'].'/salary/'.$row['salary_slip_copy'].'" target="_blank"><img src="images/pdf.png" alt="pdf File" title="Click Here to View"></a>';
                            }else{ 
                            $html .='<img src="images/employee_directory/'.$_SESSION['regis_id'].'/salary/'.$row['salary_slip_copy'].' " style="width:75px"> ';
							} 
                           $html .=   ' </td>';
                           } else {                          
                           $html .='<td><b>  Partner</b></td>                            
                            <td>';
                              $partnerext = pathinfo($row['partner'], PATHINFO_EXTENSION);
                            if($partnerext =="pdf" || $partnerext =="PDF"){
                            $html .='<a href="images/employee_directory/'. $_SESSION['regis_id'].'/partner/'. $row['partner'].'" target="_blank"><img src="images/pdf.png" alt="pdf File" title="Click Here to View"></a>';
                             }else{ 
                            $html .='<img src="images/employee_directory/'.$_SESSION['regis_id'].'/partner/'.$row['partner'].'" style="width:75px">'; 
                             } 
                          $html .='</td>';                         
...
?>	
<?php
if(isset($_POST['actiontype']) && $_POST['actiontype']=="removeCompanyPerson"){
		$visitor_id = $_POST['visitor_id'];
		$sql="delete from visitor_directory where visitor_id='$visitor_id'";
...
<?php
if(isset($_POST['actiontype']) && $_POST['actiontype']=="email")
{
	...
	if($email!='')
	{
		$query = $conn->query("select email from ivr_registration_details where email='$email' AND eid!='$eid'");
		$num = $query->num_rows;
		if($num>0)
		{
			echo 1;
