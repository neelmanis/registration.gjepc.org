...
//$checkHistory = "SELECT * FROM `visitor_order_history` WHERE payment_made_for='5show' AND payment_status='Y'"; 
...
if($num>0)
{
	...
	{ 	
		...
		$visitor_id =  $resultHistory['visitor_id'];
		$registration_id = $resultHistory['registration_id'];
		$payment_made_for = "iijs22";
		$orderId = "IIJS22".$visitor_id;
	...
	echo	$sqlx2 = "insert into visitor_order_detail set orderId='$orderId',regId='$registration_id',payment_mode='dumps',total_payable='0',payment_status='Y',create_date=NOW(),year='2022',event='$payment_made_for',paymentThrough='dumps',agree='Y'";
		$result2 = $conn->query($sqlx2);
		if($result2){
		$sqlx1 = "INSERT INTO `visitor_order_history`(`create_date`, `registration_id`, `orderId`,`visitor_id`, `payment_made_for`, `amount`,`show`, `year`,`payment_status`, `paymentThrough`,`status`) VALUES (NOW(),'$registration_id','$orderId','$visitor_id','$payment_made_for','0','$payment_made_for','2022','Y','dumps','1')";
		$result = $conn->query($sqlx1);
		...
		$insertGlobal = "INSERT INTO globalExhibition  SET `uniqueIdentifier`='$uniqueIdentifier',`registration_id`='$registration_id',`visitor_id`='$visitor_id',`fname`='$name',`mobile`='$visitorMobile',`pan_no`='$visitorPAN',`designation`='$visitorDesignation',`company`='$company_name',`photo_url`='$photo_url', `participant_type`='VIS',`covid_report_status`='pending',`status`='P',srl_report_url='dumps',`event`='$payment_made_for' ";
		$insertGlobalResult = $conn->query($insertGlobal);
		}
		//Global Table End 
