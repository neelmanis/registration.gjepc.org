...
if($action == 'pan_number'){
	...
	if($count>0) 
	{
		...
		$registration_id = intval(filter($row['registration_id']));
	  	$sqlExhibitor = "select * from exh_reg_payment_details where uid='$registration_id' AND  (`show`='IIJS SIGNATURE 2023' OR `show`='IIJS TRITIYA 2023' OR `show`='COMBO ( IIJS SIGNATURE 2023 + IIJS TRITIYA 2023)') AND allow_visitor='N' order by payment_id desc limit 0,1";
		...
		$checkHistory = "SELECT * FROM `visitor_order_history` WHERE visitor_id='$visitorID' AND payment_made_for='combo23' AND `status`='1' AND payment_status='Y'";
        ...
        $showHistory = "";
        $counter =1;
        $checkShowRegistration = "SELECT payment_made_for FROM `visitor_order_history` WHERE visitor_id='$visitorID' AND registration_id='$registration_id' AND (payment_made_for='combo23' OR payment_made_for='signature23' OR payment_made_for='iijstritiya23') AND `status`='1' AND payment_status='Y'";
