...
if($action == 'email_id'){
	...
	if($count>0) 
	{
      ...
      $eid = intval(filter($row['eid']));
		...
		if($status =='Y'){
			 $checkHistory = "SELECT * FROM `ivr_registration_history` WHERE visitor_id='$eid' AND registration_id='$uid' AND (payment_made_for='signature23' OR payment_made_for='iijs22' OR payment_made_for='combo23'  OR payment_made_for='igjme22' OR payment_made_for ='iijstritiya23') AND `status`='1' AND payment_status='Y'";			 
       	...
       	$data = array("status"=>'error',"message"=>"Please first register for IIJS PREMIERE 2022 show.");	
       }
		} else if($status =='N'){
...
if($action == 'send_otp'){
	...
	if($count>0) 
	{
	  ...
	  $message ='<table cellpadding="10" width="100%" style="margin:0px auto;font-family: Roboto; background-color:#fff;padding:30px; border-collapse:collapse;border:1px solid #000"><tr><td colspan="2" style="background-color:#a89c5d;padding:3px;"></td></tr><tr><td align="left" ><img src="https://registration.gjepc.org/images/logo.png"></td><td align="right"></td></tr><tr ><td colspan="2">One Time Password for International Visitor Verification is <b>'.$otp.'</b></td></tr><tr><td colspan="2"><p> For Any Queries : </p><b>Email us on :</b>daphne.dcosta@gjepcindia.com, naheed@gjepcindia.com </td></tr><tr><td colspan="2"><p>Kind Regards,<br><b>GJEPC Web Team.</b></p></td></tr></table>';
     /*
	 $to = $email;
	 $subject = "Email Verification OTP"; 
	 $headers  = 'MIME-Version: 1.0' . "\r\n"; 
	 $headers .= 'CC: daphne.dcosta@gjepcindia.com, naheed@gjepcindia.com' . "\r\n";
