...
if($action == 'pan_number'){
	...
	if($count>0) 
	{
		...
		$registration_id = intval(filter($row['registration_id']));
	  	$sqlExhibitor = "select * from exh_reg_payment_details where uid='$registration_id' AND `show`='$event_name' AND `year`='$year' AND allow_visitor='N' order by payment_id desc limit 0,1";
		...
		{ 							
			if(in_array($option_value['visitor_id'], $finalvisitoridarray)) {
				$data = array("status"=>'warning', "message"=>"Already registered for all event. To get your badge please upload your vaccination certificate on below given link");
        ...
        $showHistory = "";
        $counter =1;
         $checkShowRegistration = "SELECT payment_made_for FROM `visitor_order_history` WHERE visitor_id='$visitorID' AND (payment_made_for='combo23' OR payment_made_for='iijs22' OR payment_made_for='signature23' OR payment_made_for='iijstritiya23' )  AND `status`='1' AND payment_status='Y'";
        ...
        $countShowRegistration = $resultShowRegistration->num_rows;
          if($countShowRegistration > 0){
          	 $showHistory .="<p>You are already registered for below shows.</p>";
          }
         $showHistory  .= '<table class="responsive_table ">';
         $showHistory .="<tr>
			    <th>Sr. No.</th>
			    <th>Show</th>
...
if($action =="company_update"){
	...
	$company_pan_no = filter($_POST['company_pan_no']);
	 ...
	 $sql = "SELECT id,company_name FROM registration_master where company_pan_no ='$company_pan_no' and approval_status='Y' ";
    $result = $conn->query($sql);
    $count = $result->num_rows;
