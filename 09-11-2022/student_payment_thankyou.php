...
//print_r($_POST);exit;
	...
	$ssx = "SELECT * FROM `visitor_order_temp` WHERE `registration_id`='$registration_id'  AND visitor_id!='0' AND paymentThrough='online'";
	...
	echo 'window.location.href = "student_registration.php";';
	echo '</script>';	
	exit;
	}
	...
	$payment_made_for = trim($resultP['payment_made_for']);
		...
		if($result2){
		header('Location: ebs/student_techprocess.php');
		}
