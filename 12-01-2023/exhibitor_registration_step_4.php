...
{	
	...
	if($_POST["tds_holder"] == 'Yes' && $getstallInfo['last_yr_participant'] == "YES") {
		...
		//echo '<pre>'; print_r($_POST);  exit;
	} else if($_POST["tds_holder"] == 'Yes' &&  $event_selected == "signature23") {
		$amountPay = $conn->real_escape_string($_POST["cheque_tds_Netamount"]);
...
<div class="inner_container">
    ...
    <div class="form_main">        
		...
		<?php
		if($_SESSION['eventInfo']['event_selected']=="signature23")
			echo getSection_desc($section,$conn);
		if($_SESSION['eventInfo']['event_selected']=="iijs")
			echo getSection_description($section,$conn);?>
		<br />
      <!--   <strong>Category :</strong>&nbsp; <?php echo $category;?> <br /> -->
		...
		if($_SESSION['eventInfo']['event_selected']=="igjme23")
			echo getSchemeDescription($selected_scheme_type,$conn);
		?> <br />
        ...
        <div class="field_input">
			<select class="textField" name="amount_payable" id="amount_payable">
				 ...
				 <?php if($event_selected=='igjme23' || $event_selected=='signature23' || $event_selected=='iijstritiya23'){ ?>
