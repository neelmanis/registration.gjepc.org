...
if(($_REQUEST['action']=='unregister') && ($_REQUEST['agency-visitor']!=''))
{
	...
	echo  $sql = "DELETE FROM visitor_agency_registration  WHERE agency_id='$registration_id' AND pan_no='$pan_no'";exit;
				...
				<div class="content_area box-shadow">
					...
					<div class='d-block mt-3 text-center '>
						<?php if(isset($_REQUEST['action']) && $_REQUEST['action']=='view'){?>
							<a href="vendor-agency-dashboard.php?action=viewDirectory" class="btn  ">Employee Directory</a>
						...
						<?php }else{?>
							<a href="vendor-agency-dashboard.php?action=addnew" class="btn  ">Add Visitor</a>
						<?php }?>
						<a href="vendor-agency-dashboard.php?action=viewVisitor" class="btn">Signature 2023 Data</a>							
					</div>
                ...
                <?php  } ?>
                        ...
                        <div class="field">
						    ...
						    <select name="payment_made_for" id="payment_made_for" class="selectField">
						    	...
						    	$sql_event = "SELECT * FROM `visitor_event_master` WHERE `status` ='1' AND `shortcode`='signature23'  order by `serial_no` ASC";  
			        ...
			        <?php if(!isset($_REQUEST['action']) || $_REQUEST['action']=='view'){?>
			        	...
			        	<?php 
                          $visitors = "SELECT id,agency_id,person_name,company_name,mobile,category,photo,badge_status,`show`,`year`,person_status FROM visitor_agency_registration WHERE `agency_id`='$registration_id' ORDER BY createdDate DESC";
                                       ...
                                       <tbody>
                                       	<?php 
                                            ...
                                            while ($rowVisitors =$visitorsResult->fetch_assoc() ) {
												...
												//$global_status_premiere = getVendorStatusFromGlobal($registration_id,$rowVisitors['id'],'signature23',"CONTR",$conn);
			                   	 					//print_r($global_status_premiere);exit;
												//$global_id_premiere = getVendorUniqueIdFromGlobal($registration_id,$rowVisitors['id'],'signature23',"CONTR",$conn);
												//$global_category_status_premiere = getVendorCategoryStatusFromGlobal($registration_id,$rowVisitors['id'],'signature23',"CONTR",$conn);
												$global_category_status_premiere_unique = getVendorCategoryUniqueStatusFromGlobal($registration_id,$rowVisitors['id'],'signature23',"CONTR",$conn);
                                        	...
                                        	<tr>
												...
												<td>
												<?php if($_SESSION['CATEGORYID'] ==5 || $_SESSION['CATEGORYID'] ==10  || $_SESSION['CATEGORYID'] == 6 || $_SESSION['CATEGORYID'] == 1 || $_SESSION['CATEGORYID'] == 5  ){?> 
												...
												<td>
                                             	<?php if ( $rowVisitors['person_status'] =='Y' && $global_category_status_premiere_unique['status'] =="" ) { ?>

													<?php if( getVisitorAgencyCategory($rowcompanyData['category'],$conn) =="OA") { ?>
															<button class="cta confirmShow" data-visitorid="<?php echo $rowVisitors['id']; ?>" data-event='signature23' data-category="OA"  >Official Agency</button>

															<button class="cta confirmShow" data-visitorid="<?php echo $rowVisitors['id']; ?>" data-event='signature23' data-category="SV"  > Service </button>

													<?php } else { ?>
															<button class="primary confirmShow" data-visitorid="<?php echo $rowVisitors['id']; ?>" data-event='signature23' data-category="<?php echo $rowVisitors['category']; ?>"  > Confirm for EVENT SIGNATURE 2023 </button>
													<?php } ?>
                                       			<?php } else if($global_category_status_premiere_unique['status'] =="Y"){  ?>               
													<button class="label success" title="Approved"><i class="fa fa-check" title="Approved">IIJS SIGNATURE 2023</i></button>
												<?php 	} else { ?>
													<button class="label pending" title="Pending"><i class="fa fa-warning" title="Pending">IIJS SIGNATURE 2023</i></button>
												<?php } ?>
                                       			
                                       			
												</td>
												<td><?php echo $global_category_status_premiere_unique['agency_category']; ?></td>
												...
												<td>
													...
													<?php if( $global_category_status_premiere_unique['status']  ==''){ ?>
												...
												<?php } ?>
												<td>
													<?php if( $global_category_status_premiere_unique['status']  =='Y'){ ?>
														<a href="https://registration.gjepc.org/visitor-badge.php?action=generateBadge&uniqueIdentifier=<?php echo $global_category_status_premiere_unique['uniqueIdentifier']; ?>" class="label success" >Download Badge</a>
													<?php } ?>
					...
					<!-- view visitor -->
					<?php if(isset($_REQUEST['action']) && $_REQUEST['action']=='viewVisitor'){?>

						<?php 
						$visitors = "SELECT ar.id, ar.person_name, ar.mobile,ar.pan_no,ar.agency_id,ar.photo,ar.id_proof_file,ar.person_status,`gb`.`status` as global_status,gb.agency_category 
						FROM visitor_agency_registration ar 
						inner join globalExhibition gb on ar.id=gb.visitor_id and ar.agency_id=gb.registration_id where ar.agency_id='$registration_id' ";
						$visitorsResult = $conn->query($visitors);
						$visitorCount =$visitorsResult->num_rows;
						?>
						<div class="alert alert-success" id="update_success">
...
{   
	...
	$("#regisForm").validate({
	    ...
	    $(document).on("click",".confirmShow",function(){
	    	if(confirm("Are you sure continue for IIJS Signature registration ?")){
