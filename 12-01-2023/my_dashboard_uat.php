...
$closetime = strtotime("21-02-2022 23:55:00");
?>
	<?php
	/* IVR Start */
	...
	$fetch_status_pvr="select obmp_application_status from visitor_obmp_details where uid=".$_SESSION['USERID']." and participate_for_show='signature22' and year='2022' LIMIT 1";
			...
			<div class="bold_font text-center">
				...
				<div class="content_area">					
					...
					{			
						...
						echo '<a href="exhibitor_registration_print_application.php?show=iijs&year=2021" target="_blank" class="btn col-100" title="View Summary">Click here to access Exhibitor Manual/Stall Payment Details </a>';
						}	
						echo '</div></div>';
					} 
					...
					<!-- Stop Space Registration Start Mukesh Panwar -->
						...
						<!-------------- UTR START ----------------------------->
						<?php
						$sqlExhibitor = "select * from exh_reg_payment_details where uid='$registration_id' AND `show`='IIJS SIGNATURE 2022' AND`year`='2022'";
						$ansExh = $conn->query($sqlExhibitor);
						$exhCountx = $ansExh->num_rows;
						if($exhCountx>0 || $rnum>0){
						?>
						<div class="box_dash_container box-shadow">	
						<div class="box_title"><h3>Update UTR / Payment Details</h3></div>
						<div class="d-flex justify-content-center col-100 box_exhibitor">
						
						<p style="color:#FF0000;float:left;display: inline;"><u><b>Exhibitor Only </b></u></p>
						<div class="form_payment ">
						<h3><b>Exhibitor Update Payment Details </b></h3>
						...
						?>
							...
							<?php
							$getEvent = "SELECT * FROM `exh_event_master` WHERE (status='1' || roi_status='1')";				
							...
							<option value="signature22">IIJS SIGNATURE 2022</option>-->
							<option value="<?php echo $eventRow['event_values'];?>"><?php echo $eventRow['eventDescription'];?></option>
							<?php } ?>
							</select>
						...
						$utrExist = "SELECT * FROM `utr_history` WHERE registration_id='$registration_id' AND (event_selected='signature22' || event_selected='iijstritiya22') ";
						...
						<table class="table_responsive">
							<tr>
								...
								if($event_selected=="iijs") { $event_for="IIJS Premiere 2021"; }
								if($event_selected=="vbsm2") { $event_for="IIJS VIRTUAL SHOW 2.0"; }
								if($event_selected=="signature22"){ $event_for="IIJS SIGNATURE 2022"; } 
								*/
								?>
								<tr>
						...
						<?php
						$utrExistTotal = "SELECT sum(amountPaid),sum(tdsAmount) FROM `utr_history` WHERE registration_id='$registration_id' AND `event_selected`='signature22' AND `show`='IIJS SIGNATURE 2022'";
						...
						<div class="clear"></div>
						<?php 
						$checkAmount = "select net_payable_amount,gid from exh_reg_payment_details where uid='$registration_id' AND `show`='IIJS 2022' AND event_selected='iijs' limit 0,1";
						$querycheckAmount = $conn->query($checkAmount);
						$numCount = $querycheckAmount->num_rows; 
						$resultData = $querycheckAmount->fetch_assoc();
							$gidData = $resultData["gid"];
							$net_payable_amount = $resultData["net_payable_amount"];
							
		
						$utrExist = "SELECT * FROM `utr_history` WHERE registration_id='$registration_id' AND `show`='IIJS 2022' AND event_selected='iijs'";
						$existResult = $conn->query($utrExist);
						$countPayment = $existResult->num_rows;
						if($countPayment > 0){
						?>
						...
						<table class="table_responsive">
						<tr> <td><strong class="title">Online Payment Details</strong>  </td></tr>
						</table>
