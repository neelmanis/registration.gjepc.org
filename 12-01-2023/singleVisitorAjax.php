...
if($action == 'pan_number'){
	...
	if($count>0) 
	{
		...
		include ("ebs/statusapi/single_techprocess.php");
		...
		$checkHistory = "SELECT * FROM `visitor_order_history` WHERE visitor_id='$visitorID' AND ( payment_made_for='combo23' OR  payment_made_for='stcombo23' ) AND `status`='1' AND payment_status='Y'";
		...
		// 		$data = array("status"=>'warning', "message"=>"Already registered for all event combo.");
		// 		echo json_encode($data); exit;
		// 	}
		// }

		// if($checkResult > 0){
		// 	$getVisShow = $getQuery->fetch_assoc();
		// 	$payment_made_for =  $getVisShow['payment_made_for'];
		// 	if($payment_made_for =="combo23" ){
		// 		$data = array("status"=>'warning', "message"=>"Already registered combo for IIJS PREMIERE 2022 + IIJS SIGNATURE 2023 + IIJS TRITIYA 2023   ");
		// 	}else{
		// 		$data = array("status"=>'warning', "message"=>"Already registered combo for  IIJS SIGNATURE 2023 + IIJS TRITIYA 2023 ");
		// 	}
		//   echo json_encode($data); exit;
		// }
        ...
        $showHistory = "";
        $counter =1;
        $checkShowRegistration = "SELECT payment_made_for FROM `visitor_order_history` WHERE visitor_id='$visitorID' AND registration_id='$registration_id' AND (payment_made_for='combo23' OR payment_made_for='signature23' OR payment_made_for='iijstritiya23' OR payment_made_for='stcombo23' OR payment_made_for='igjme23') AND `status`='1' AND payment_status='Y'";
        $resultShowRegistration = $conn->query($checkShowRegistration);
        $countShowRegistration = $resultShowRegistration->num_rows;
          if($countShowRegistration > 0){
          	 $showHistory .='<p class="mb-1">You are already registered for below shows.</p>';
          
         $showHistory  .= '<table class="responsive_table mb-3">';
         $showHistory .="<thead>
				<tr>
			    <th>Sr. No.</th>
			    <th>Show</th>
        ...
        if($countShowRegistration > 0){
           while($rowShowRegistration =  $resultShowRegistration->fetch_assoc()){
			$showHistory .='<tr>
			    <td>'.$counter.'</td>
			    <td>'.getVisEventName($rowShowRegistration['payment_made_for'],$conn).'</td>
			    <td>Registered</td>			    
			  </tr>';
			  $counter++;
           }	
       ...
       $showHistory .= "</tbody></table>";
		...
		$_SESSION['registration_id'] = $row['registration_id']; 
       	  ...
       	  $data = array("status"=>'success',"company"=>$company, "mobile"=>$mobile,"email"=>$email_new,"name"=>$name,"lname"=>$lname,"photo"=>$photo,"reg_id"=>$reg_id,"visitor_id"=>$visitorID,"designation"=>$designation,"showHistory"=>$showHistory);
...
if($action == 'singleVisitorRegistration')
{
	...
	if(isset($_POST['agree']) && $_POST['agree'] == 'YES'){   $agree = "1"; } else {  $agree = "0"; }
 ...
 /*
 ** UPDATE Visitor Directory whatsapp service flag
 */
   $update_dir = $conn->query("UPDATE visitor_directory SET whatsapp_service='$agree'  WHERE visitor_id='$visitor_id' AND registration_id='$registration_id' ");
	...
	// IF PAID FOR COMBO OF SIGNATUTE AND TRITIYA
	$checkSTComboPayment = checkEventPayment($visitor_id,$registration_id,"stcombo23",$conn);
	if($checkSTComboPayment == true){
			echo json_encode(array("status"=>"error","message"=>"Payment is already done for the all show"));exit;
	}
	...
	if($checkSignaturePayment ==true && ($payment_made_for=='combo23' || $payment_made_for=='stcombo23')){
	...
	if($checkTritiyaPayment ==true && ($payment_made_for=='combo23' || $payment_made_for=='stcombo23')){
...
if(isset($registration_id) && $registration_id!=""){
	...
	// 	 For Machinery Start
		...
		{			
			$sqlx3 = "INSERT `visitor_order_detail` SET `type_of_member`='$type_of_member',`delivery_id`='$delivery_id',`billing_delivery_id`='$billing_delivery_id', `regId`='$registration_id' , `orderId`='$orderId' , `paymentThrough`='single',`payment_type`='free',`total_payable`='$amount', `type_of_member`='$type_of_member',`create_date`='$post_date',year='$year',event='$show',agree='$agree'"; 
			$result2 = $conn->query($sqlx3);		
			if($result2){				
		...
		{
	    $sqlx2 = "insert into visitor_order_detail set orderId='$orderId',regId='$registration_id',payment_type='$payment_type',total_payable='$amount',type_of_member='$type_of_member',`delivery_id`='$delivery_id',`billing_delivery_id`='$billing_delivery_id' ,create_date='$post_date', paymentThrough='single',year='$year',event='$show',agree='$agree'";
...
if($action =="company_update"){
  /* CHECK PAYMENT HISTORY FOR VISITOR */
  $visitor_id = trim($_SESSION['visitor_id']);
  $checkShowRegistration = "SELECT payment_made_for FROM `visitor_order_history` WHERE visitor_id='$visitor_id' AND (payment_made_for='combo23' OR payment_made_for='signature23' OR payment_made_for='iijstritiya23' OR payment_made_for='igjme22') AND `status`='1' AND payment_status='Y'";
    $resultShowRegistration = $conn->query($checkShowRegistration);
    $countShowRegistration = $resultShowRegistration->num_rows;
    if($countShowRegistration > 0){
        echo json_encode(array("status"=>"error","message"=>"Sorry you can't change the company. You are already register for the show. "));exit;
    }
