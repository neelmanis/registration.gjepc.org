...
if($count_event > 0){
   ...
   $website = $row_event['website'];
	...
	$city = strtoupper(getCompanyCityName($registration_id,$conn));
	$stateCode = strtoupper(getCompanyStateName($registration_id,$conn));
	$state = strtoupper(getStateName($stateCode,$conn));
?>
...
if(isset($_POST))
{
	...
	if($tpsl_txn_time=="NA"){ $tpsl_txn_time = date('d-m-Y H:i:s'); }
	...
	{		
		...
		$sqlx1 = "INSERT INTO `visitor_order_history`(`create_date`, `registration_id`, `orderId`,`visitor_id`, `payment_made_for`, `amount`,`show`, `year`,`payment_status`, `status`,`paymentThrough`) VALUES ('$day','$registration_id','$orderId','$visitor_id','$payment_made_for','$amount','$show','$year','Y','1','online')";
