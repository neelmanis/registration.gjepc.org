...
<div class="inner_container">
 ...
 $sql_signature ="select * from exh_registration where uid='$uid' AND curr_last_yr_check='Y' AND `show`='IIJS PREMIERE 2022' order by gid desc";
	...
	if($show=="tritiya"){
		...
		/*..................................Get UTR DETAIL Start.....................................*/
		$utrExistTri = "SELECT sum(amountPaid) as `amountPaid`,sum(tdsAmount) as `tdsAmount` FROM `utr_history` WHERE registration_id='$uid' AND `show`='$shows' AND payment_status='captured'";
		$existResult = $conn->query($utrExistTri);	
		while($printutr = $existResult->fetch_assoc())
		{				
			$amountPaidFromUTR = $printutr['amountPaid'];
			$tdsAmountUTR = $printutr['tdsAmount'];
		...
		function getTriSpaceTDSAmount($uid,$conn)
		{
		$query_sel = "select cheque_tds_amount from exh_reg_payment_details where uid='$uid' AND `show`='IIJS TRITIYA 2023' AND event_selected='iijstritiya23' limit 0,1";
		$result_sel = $conn->query($query_sel);								
		$row = $result_sel->fetch_assoc();								
		return $row['cheque_tds_amount'];							
		}
				
		$tds_amount = getTriSpaceTDSAmount($uid,$conn) + $tdsAmountUTR;
		$tdsAmount = $tds_amount; 
				
		$getSql_tri ="select * from exh_registration where uid='$uid' AND curr_last_yr_check='Y' AND `show`='$shows' order by gid desc";
		$getQuery_tri = $conn->query($getSql_tri);	
		$getResult_tri = $getQuery_tri->fetch_assoc();
		$getgid = $getResult_tri['gid'];
		/*..................................Get UTR DETAIL END.....................................*/	
				
		/* 25% Payment for Tritiya only */
		/*$checkPartPayment = "SELECT id FROM `utr_history` WHERE registration_id='$uid' AND `show`='IIJS TRITIYA 2023' AND (payment_made_for='PART' || payment_made_for='SPACE') AND payment_status='captured'";
		...
		if($existResultPayment){			
				...
				echo $sexh="SELECT manual_tritiya.iijs_exhibitor.Exhibitor_Gid,manual_tritiya.iijs_exhibitor.Exhibitor_Registration_ID,manual_tritiya.iijs_exhibitor.exempt_gst,manual_tritiya.iijs_exhibitor.Exhibitor_HallNo,manual_tritiya.iijs_exhibitor.Exhibitor_Section,manual_tritiya.iijs_exhibitor.Exhibitor_Scheme,manual_tritiya.iijs_exhibitor.Exhibitor_DivisionNo,manual_tritiya.iijs_exhibitor.Exhibitor_StallNo1,manual_tritiya.iijs_exhibitor.Exhibitor_StallNo2,manual_tritiya.iijs_exhibitor.Exhibitor_StallNo3,manual_tritiya.iijs_exhibitor.Exhibitor_StallNo4,manual_tritiya.iijs_exhibitor.Exhibitor_StallNo5,manual_tritiya.iijs_exhibitor.Exhibitor_StallNo6,manual_tritiya.iijs_exhibitor.Exhibitor_Area,manual_tritiya .iijs_exhibitor.Exhibitor_StallType,manual_tritiya .iijs_exhibitor.Exhibitor_Premium,manual_tritiya .iijs_exhibitor.amountPaid, manual_tritiya.iijs_exhibitor.amountUnpaid FROM manual_tritiya.iijs_exhibitor where manual_tritiya.iijs_exhibitor.Exhibitor_Registration_ID='$uid' and manual_tritiya.iijs_exhibitor.Exhibitor_Gid='$getgid'";
		...
		/*$utrPaidAmount = "SELECT sum(amountPaid) as `amountPaid`,sum(cheque_tds_amount) as `cheque_tds_amount` FROM `utr_history` WHERE registration_id='$uid' AND `show`='IIJS TRITIYA 2023' AND ( payment_made_for='SPACE' ||  payment_made_for='PART') AND payment_status='captured'";
		...
		if($countx1 > 0 && $countx1!='') { ?>
        <?php if($amountUnpaid<=500){?>
        	<!--<a href="manual_tritiya/index.php?uid=<?php echo $uid;?>&gid=<?php echo $getgid;?>&section=<?php echo $rexh['Exhibitor_Section'];?>" class="cta fade_anim d-table mx-auto">Online Exhibitor Manual</a>-->
			<!--<a href="javascript: void(0)" onclick = "second_alert()" class="newMaroonBtn">Online Exhibitor Manuals</a>-->
            <?php } else { ?>
