...
$(document).ready(function(){
  ...
  <?php } else if($event_selected=='signature23'){ ?>
  var url = 'signature_combo_ajax.php';
  <?php }else{?>
	var url = 'tritiya_combo_ajax.php';
  <?php } ?>
...
<script>
  $("#option").live('change',function(){
	...
	if($('#selected_premium_type_hid').val()!='')
		...
		<?php if($event_selected=='signature23'){ ?>
	  ...
	  lastYearArea=$('#lastYearArea').val();
	  lastYearSignatureArea=$('#lastYearSignatureArea').val();
	  ...
	  <?php } else if($event_selected=='signature23'){ ?>
	  var url = 'signature_combo_ajax.php';
	  var getdata = "actiontype=optionValue&&option="+option+"&&lastYearArea="+lastYearArea+"&&lastYearSignatureArea="+lastYearSignatureArea+"&&section="+section;
	  <?php } ?>
...
<div class="inner_container">
	...
	if($event_selected=='iijs'){ $lastYearEvent = "IIJS 2021"; }
	if($event_selected=='signature23'){ $lastYearEvent = "IIJS Signature 2022"; }
	...
	<?php if($lastYearEvent !=""){ ?>
 <div class="field_box"> 
            <div class="field_name"><?php echo $lastYearEvent; ?> Participant :</div>
	...
	<?php } ?>
		...
		<!-- <div class="field_box"> 
            <div class="field_name"><?php echo $event_for;?> NEW Participant :</div>
            <div class="field_input" style="padding-top:5px;">
            <div class="chkbox" style="width:100px;">
              <span class="chkbox" style="width:100px;">
              <?php if($roi_status=='YES'){ echo $roi_status; } else { echo $roi_status="NO"; } ?>
              </span>
            </div>
            </div>
            <div class="clear"></div>
        ...
        <?php if($num1>0){ ?>
        <div class="field_box">
            <div class="field_name"><?php echo $lastYearEvent; ?> Year details :</div>
       ...
       <div class="field_box">
        ...
        More area as alloted at IIJS Premiere 2020<br/>
        
        <?php if($selected_area>9){ ?>
        <input type="radio" id="option" class="bgcolor" name="option" value="Less area as previous year at diffrent location" <?php /*if($option=="Less area as previous year at diffrent location"){?> checked="checked" <?php } */?> /> Less area as alloted at IIJS Premiere 2020<br/>
		...
		<!-- <input type="radio" id="option" class="bgcolor" name="option" value="Same stall position size as of previous year" <?php if($option=="Same stall position size as of previous year"){?> checked="checked" <?php } ?> /> Same stall position as off Previous Year Signature 2022.<br/> -->
        ...
        <input type="radio" id="option" class="bgcolor" name="option" value="More area than previous year Signature" <?php if($option=="More area than previous year Signature"){?> checked="checked" <?php } ?>/> More area than previous year signature 2022.<br/>
		<?php if($selected_area!=9){?>
        <input type="radio" id="option" class="bgcolor" name="option" value="Less area as previous year" <?php if($option=="Less area as previous year"){?> checked="checked" <?php } ?> /> Less area as previous year Signature 2022<?php } ?>
        ...
        <div class="field_input">
          ...
          <option selected="selected" value="">-----Select Section----</option>
			...
			if($_SESSION['COUNTRY']=="IN")
			{
				...
				{ 	
					...
					} else if($event_selected=='signature23') { 
						$sql="SELECT * FROM signature_section_master WHERE member_type ='MEMBER' AND status='Y' ORDER BY section_desc";
					}else{
						$sql="SELECT * FROM tritiya_section_master WHERE member_type ='MEMBER' AND status='Y' ORDER BY section_desc";
					}
				} else {
					if($event_selected=='iijs'){
					...
					$sql="SELECT * FROM signature_section_master WHERE member_type ='NON_MEMBER' and status='Y' ORDER BY section_desc";
						...
						$sql="SELECT * FROM tritiya_section_master WHERE member_type ='NON_MEMBER' AND status='Y' ORDER BY section_desc";
					}
				}
          ...
          <?php if($section!="" && $num1>0){?><input type="hidden" name="section" id="section_hid" value="<?php echo $section;?>" disabled/><?php }?>          
        </div>
        <div class="clear"></div>
        </div>
		
		<?php 
		if($event_selected=='iijs' || $event_selected=='signature' ){
		if($event_selected=='iijs'){
		if($category!=""){?>
		...
		<div class="field_box">
        <div class="field_name">Select your category <span>*</span>: Premium will be applicable as per stall alloted</div>
        <div class="field_input">
		...
		<?php } }else{ ?>
			<input type="hidden" name="category" id="category_hid" value="" />
		<?php } ?>
        ...
        <div class="field_input"> 
          <select name="selected_area" id="selected_area" class="bgcolor" <?php if($selected_area!="" && $num1!=0){?> disabled="disabled"<?php }?>>
			...
			if($num1==0){ $sql="SELECT * FROM iijs_area_master where 1"; } 
          ...
          <input type="hidden" name="lastYearSignatureArea" id="lastYearSignatureArea" value="<?php echo $selected_area;?>"/>
        ...
        <div class="field_input"> 
			...
			$sql="SELECT * FROM signature_area_master order by area asc";
			$query=$conn->query($sql);
			while($result=$query->fetch_assoc()){
            ?>
            <option value="<?php echo $result['area'];?>" <?php if($result['area']==$selected_area){?> selected="selected" <?php }?>><?php echo $result['area'];?></option>
		...
		if($event_selected=='iijs' || $event_selected=='signature23'){ 
		if($event_selected=='iijs'){ 
			?>
        <?php if($section!='machinery'){ ?>
        <div class="field_box">
        ...
        <div class="field_name">Select your premium type <span>*</span>:</div>
        <div class="field_input">
       		<select class="bgcolor" name="selected_premium_type" id="selected_premium_type" <?php if($option=="Same stall position size as of previous year"){?> disabled="disabled"<?php } ?>>
            <option selected="selected" value="">-----Select Premium Type----</option>
			<?php 
