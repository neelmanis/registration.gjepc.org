...
$institutes = "select id from registration_master where id=".$_SESSION['USERID']." and type_of_company='institute'";
$institutesResult = $conn->query($institutes);
$countInstitute = $institutesResult->num_rows;
if($countInstitute > 0)
{
...
$closetimeIGJME = strtotime("20-10-2022 23:55:00");
?>
	<?php
	/* IVR Start */
			...
			<div class="bold_font text-center">
				...
				<div class="content_area">					
					...
					if($_SESSION['COUNTRY']=="IN")
					{
						...
						$sql = "select * from exh_reg_payment_details where uid='$registration_id' AND `show`='IIJS TRITIYA 2023' AND allow_visitor='N' order by payment_id desc limit 0,1";
						$ans = $conn->query($sql);
					...
					*/	?>
					...
					{											
						...
						if($countxx==0){
							echo "<div class='box_dash_container box-shadow'>
								<div class='box_title'><h3>IIJS TRITIYA </h3></div>
								<div class='d-flex justify-content-center col-100 box-shadow'>";	
								$schk_defaulter="select * from registration_master where id='$registration_id'";
								$qchk_defaulter=$conn->query($schk_defaulter);
								$rchk_defaulter=$qchk_defaulter->fetch_assoc();
								$rchk_defaulter['payment_defaulter'];
								...
								Exhibitor Registration / Stall Booking</h3></div>
								<div class="d-flex justify-content-center col-100">
									<!--<a href="#" class="btn ">Coming Soon</a>-->
									 <?php
									 if($current_time<=$closetime)
						...
						if($countxx==0){
							...
							if($rchk_defaulter['payment_defaulter']=="N"){ ?>								
								...
								<div class="d-flex justify-content-center col-100">
									...
									if($current_time<=$closetime)
										//echo '<a href="event_selection_uat.php" class="btn col-100" title="Select Show">Apply</a>';
										echo '<a href="#" class="btn col-100" title="Select Show">Closed</a>';
									else
										echo '<a href="#" class="btn col-100">Application Closed</a>';
									?>			
								</div>
				...
				$result_summary = $conn->query($sql_summary);
					$count_summary = $result_summary->num_rows;
						...
						if($count_sign > 0){ ?>
							<div class="col-md-4"><a href="exhibitor_registration_print_application.php?show=signature&year=2023" target="_blank" style="display: table;" class="btn" title="View Summary">IIJS SIGNATURE 2023 Balance Payment / Exhibitor Manual </a>		</div>
						<?php } ?>
						...
						if($count_tritiya > 0){ ?>
							<div class="col-md-4"><a href="exhibitor_registration_print_application.php?show=tritiya&year=2023" target="_blank" style="display: table;" class="btn" title="View Summary">IIJS TRITIYA 2023 application summary / Balance Payment</a>		</div>
						<?php }
							/*
							$sql_car = "select * from globalparking where registration_id='$registration_id' ";
							$result_car = $conn->query($sql_car);
							$count_car = $result_car->num_rows;

							if($count_car > 0){ ?>
								<div class="col-md-4"><a href="car_parking_booking.php" target="_blank" style="display: table;" class="btn" title="View Summary">Car Parking Passes </a>		</div>
							<?php }	
							*/						
							echo '</div></div></div>'; 
					...
					<!--------------------------------- START IGJME Space Registration ------------------------------------------------->
					<?php
					/*
					$chkEvent = "select * from exh_registration where uid='$registration_id' and (`show`='IIJS SIGNATURE 2023' OR `show`='IIJS TRITIYA 2023')"; 
					$event_summary = $conn->query($chkEvent);
					$count_event = $event_summary->num_rows;
					if($count_event==0){
						if($_SESSION['COUNTRY']=="IN" && $_SESSION['member_type']=="MEMBER")
						{		
									
						echo "<div class='box_dash_container box-shadow'>
						<div class='box_title'><h3>IGJME 2023 Show</h3></div>
						<div class='d-flex justify-content-center col-100'>";						
						$sqlt = "select * from exh_registration where uid='$registration_id' AND (`show`='IIJS SIGNATURE 2023' || `show`='IIJS TRITIYA 2023')";
						$anst = $conn->query($sqlt);
						$countxx = $anst->num_rows;
						if($countxx==0){
							$schk_defaulter="select * from registration_master where id='$registration_id'";
							...
							if($rchk_defaulter['payment_defaulter']=="N"){ ?>
								...
								<div class="d-flex justify-content-center col-100">
									<?php
									$sqlxt = "select * from exh_registration where uid='$registration_id' and `show`='IGJME'";
									$nResult = $conn->query($sqlxt);
									$countlxx = $nResult->num_rows;
									if($countlxx==0){
									if($current_time<=$closetimeIGJME)
										echo '<a href="event_selection_igjme.php" class="btn col-100" title="Select Show">Apply</a>';
									else
										echo '<a href="#" class="btn">Application Closed</a>';
									} else { 
										echo '<a href="exhibitor_registration_print_application.php?show=igjme&year=2023" target="_blank" class="btn col-100" title="View Summary">IGJME Application Summary</a>';
									}
									?>			
								</div>
								</div>
							...
							if($rchk_defaulter['payment_defaulter']=="N"){ ?>
								...
								<div class="d-flex justify-content-center col-100">
									...
									if($current_time<=$closetimeIGJME)
										...
										echo '<a href="exhibitor_registration_print_application.php?show=igjme&year=2023" target="_blank" class="btn col-100" title="View Summary">IGJME 2023 Balance Payment / Exhibitor Manual</a>';
									}
									?>			
								</div>
								</div>
...
.customTabs..nav-tabs .nav-link{
	border-bottom: 1px solid #000;
						...
						<div class="border">
							<?php 
                            $sqltShows = "select * from exh_registration where uid='$registration_id' AND (`show`='IIJS SIGNATURE 2023' || `show`='IIJS TRITIYA 2023')";
                            $showQuery = $conn->query($sqltShows);
                            $countShows = $showQuery->num_rows;
                            
                            $sqltIGJMEShows = "select * from exh_registration where uid='$registration_id' AND `show`='IGJME'";
                            $showigjmeQuery = $conn->query($sqltIGJMEShows);
                            $countigjmeShows = $showigjmeQuery->num_rows;
                            ?>
							<ul class="nav nav-tabs customTabs pt-2" id="myTab" role="tablist">
                              ...
                              <?php if($countShows>0){ ?>
                              <li class="nav-item ">
                                <a class="nav-link " id="profile-tab" data-toggle="tab" href="#profile" role="tab" aria-controls="profile" aria-selected="false">Online Payment Details IIJS SIGNATURE 2023</a>
                              </li>
                              <li class="nav-item ">
                                <a class="nav-link active"  id="contact-tab" data-toggle="tab" href="#contact" role="tab" aria-controls="contact" aria-selected="false">Online Payment Details IIJS TRITIYA 2023</a>
                              </li>
                              <?php } ?>
                              <?php if($countigjmeShows=='1'){ ?>
                              <li class="nav-item ">
                                <a class="nav-link " id="contact-tab" data-toggle="tab" href="#igjme" role="tab" aria-controls="contact" aria-selected="false">Online Payment Details IGJME 2023</a>
                              </li>
                              <?php } ?>
                            </ul>
							<div class="tab-content box-shadow p-2" id="myTabContent" >
							  ...
							  <!-- ============================= SIGNATURE START =========================================== -->
							  <?php if($countShows>0){ ?>
                              <div class="tab-pane fade <?php /*if($countShows>0){?>show active<?php } */?>" id="profile" role="tabpanel" aria-labelledby="profile-tab">
							  	<?php
									...
									function getTDSAmountfromUTR($registration_id,$event,$conn)
									{
										$query_sel = "SELECT sum(tdsAmount) as `tdsAmount` FROM `utr_history` WHERE registration_id='$registration_id' AND `show`='$event' AND payment_status='captured'";
										$result_sel = $conn->query($query_sel);								
										$row = $result_sel->fetch_assoc();								
									...
									<table class="table_responsive">
										<tr>
											...
											$sign_payment_made_for = $sign_printutr["payment_made_for"];
											?>
											<tr>
											...
											<?php if($sign_payment_made_for=="SPACE"){ ?>
											<td align="center"><?php echo getTDSAmount($registration_id,"IIJS SIGNATURE 2023",$conn);?></td>
											<?php } ?>
											<?php if($sign_payment_made_for=="ALLOTMENT"){ ?>
											<td align="center"><?php echo getTDSAmountfromUTR($registration_id,"IIJS SIGNATURE 2023",$conn);?></td>
											<?php } ?>
							  ...
							  <?php if($countShows>0){ ?>
							  <div class="tab-pane fade <?php if($countShows>0){?>show active<?php } ?>" id="contact" role="tabpanel" aria-labelledby="contact-tab">
							  	<?php
									...
									<table class="table_responsive">
										<tr>
											...
											$tritiya_gidData = $tritiya_printutr["gcode"];
											 $tritiya_payment_made_for = $tritiya_printutr["payment_made_for"];
											?>
											<tr>
											...
											<?php if($tritiya_payment_made_for=="SPACE"){ ?>
											<td align="center"><?php echo getTDSAmount($registration_id,"IIJS TRITIYA 2023",$conn);?></td>
											<?php } ?>
											<?php if($tritiya_payment_made_for=="PART"){ ?>
											<td align="center"><?php echo getTDSAmountfromUTR($registration_id,"IIJS TRITIYA 2023",$conn);?></td>
											<?php } ?>
                              ...
                              <div class="tab-pane fade " id="igjme" role="tabpanel" aria-labelledby="contact-tab">
                                <?php
                                $igjme_utrExistSuccess = "SELECT * FROM `utr_history` WHERE registration_id='$registration_id' AND `show`='IGJME' AND payment_status='captured'";
                                $igjme_successResult = $conn->query($igjme_utrExistSuccess);
                                $igjme_countSuccessPayment = $igjme_successResult->num_rows;
                                if($igjme_countSuccessPayment > 0){
                                    $igjme_utrExistQuery = "SELECT * FROM `utr_history` WHERE registration_id='$registration_id' AND `show`='IGJME' AND payment_status='captured'";
                                    } else {
                                    $igjme_utrExistQuery = "SELECT * FROM `utr_history` WHERE registration_id='$registration_id' AND `show`='IGJME'";                                     
                                    }
                                    $igjme_successResultQuery = $conn->query($igjme_utrExistQuery);
                                    $igjme_countPayment = $igjme_successResultQuery->num_rows;
                                    if($igjme_countPayment > 0){ ?>
                                    
                                    <table class="table_responsive">
                                    <tr> <td><strong class="title">Online Payment Details IGJME 2023</strong>  </td></tr>
                                    </table>
                                    ...
                                    <table class="table_responsive">
                                        <tr>
                                            ...
                                            <?php
                                            while($igjme_printutr = $igjme_successResultQuery->fetch_assoc())
                                            {
                                            $igjme_payment_date = $igjme_printutr['payment_date']; 
                                            $igjme_id = $igjme_printutr['id']; 
                                            $igjme_getUTR_no = $igjme_printutr['utr_number']; 
                                            $igjme_event_for=$igjme_printutr['show']; 
                                            $igjme_event_selected = $igjme_printutr['event_selected']; 
                                            $igjme_amountPaid = $igjme_printutr['amountPaid'];
                                            $igjme_utr_approved = $igjme_printutr['utr_approved'];
                                            $igjme_payment_status = $igjme_printutr['payment_status'];
                                            $igjme_tds_holder = $igjme_printutr["tds_holder"];
                                            $igjme_cheque_tds_amount = $igjme_printutr["cheque_tds_amount"];
                                            $igjme_gidData = $igjme_printutr["gcode"];
                                            ?>
                                            <tr>
                                            <td align="center"><?php echo $igjme_payment_date;?></td>
                                            <td align="center"><?php echo $igjme_getUTR_no;?></td>
                                            <td align="center"><?php echo $igjme_event_for;?></td>
                                            <td align="center"><?php echo $igjme_amountPaid;?></td>
                                            <td align="center"><?php echo $igjme_cheque_tds_amount;?></td>
                                            <td align="center">
                                            <?php if($igjme_payment_status=="pending" || $igjme_payment_status==""){ ?>
                                            <button class="cta fade_anim d-table mx-auto" id="save" onclick="return savePayment.save('igjme23')">Click here for Payment</button>   
                                            <?php } ?>
                                            </td>
                                            <td align="center"><?php echo $igjme_payment_status;?></td>
                                            </tr>
                                            <?php }  ?>
                                     </table>
                                     <input type="hidden" name="show_type" id="show_type">
                                    <input type="hidden" id="igjme_gidData" value="<?php echo $igjme_gidData;?>">
                                    <input type="hidden" id="net_payable_amount_igjme" value="<?php echo base64_encode($igjme_amountPaid);?>">
                                    <input type="hidden" name="action" value="submit">
                                     </form>
                                    <?php } ?>
                              </div>
