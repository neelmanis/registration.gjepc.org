...
if(isset($_POST['action']) && $_POST['action']=="confirm_show")
{
	...
	if(!empty($registration_id )){
		if($visitor_id !==""){
			 $sql = "UPDATE `globalExhibition` SET `status` = 'Y' WHERE `visitor_id` = '$visitor_id' AND registration_id='$registration_id' AND participant_Type='$participant_Type' AND status='P' AND `event`='$event'";

			$resultx = $conn->query($sql);
	 		if($resultx){
	       	echo json_encode($data = array("status"=>'success',"message"=>"Thank you, you have registered for IIJS SIGNATURE 2022 successfully")); exit;
	       }else{
	       	echo json_encode($data = array("status"=>'error',"message"=>"Sorry, Something went wrong. Please try again. ")); exit;
	       }
	    }else{
