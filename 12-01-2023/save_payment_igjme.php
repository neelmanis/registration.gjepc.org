...
{	   
	...
	//	$net_payable_amount = 1;	
        ...
        if($razorpayOrderId){           
            $data = [
					...
					"amount" => $amount,
					"name"   => "GJEPC",
			    "description" => "IGJME Space Application Dashboard",
	...
	$sqlShow = "select * from exh_reg_payment_details where uid='$registration_id' AND gid='$gidData' AND `show`='IGJME' order by payment_id desc limit 0,1";
	$ansResults = $conn->query($sqlShow);
	$resultShowss = $ansResults->fetch_assoc();
	$cheque_tds_per =  $resultShowss['cheque_tds_per'];
	$tds_holder =  $resultShowss['tds_holder'];
	$cheque_tds_amount =  $resultShowss['cheque_tds_amount'];
	
						
	$insertUTR = "INSERT INTO `utr_history`(`post_date`, `registration_id`,`gcode`,`utr_number`,`tdsAmount`,`amountPaid`, `razorpay_order_id`, `merchant_order_id`,`payment_status`,`payment_date`,`show`,`event_selected`, `year`, `status`,`error_description`,`comment`,`payment_made_for`,tds_holder,cheque_tds_per,cheque_tds_amount) VALUES (NOW(),'$registration_id','$gidData','$merchant_order_id','0','$net_payable_amount','$razorpayOrderId','$merchant_order_id','pending','$payment_date','IGJME','igjme23','2023','1','dashboard','dashboard','SPACE','$tds_holder','$cheque_tds_per','$cheque_tds_amount')";
	$insert = $conn->query($insertUTR);
	if(!$insert) { die('Error: Insert query failed' . $conn->error); }
	$payLog = "INSERT INTO `space_payment_log`(`registration_id`,`gid`,`merchant_order_id`,`amount`, `razorpay_order_id`,`post_date`,`source`,`payment_status`,`event_selected`) VALUES ('$registration_id','$gidData','$merchant_order_id','$net_payable_amount','$razorpayOrderId','$payment_date','dashboard','pending','igjme23')";
...
if(isset($_POST['razorpay_payment_id']) && isset($_POST['razorpay_order_id'])) 
{
	...
	$update_query = "update exh_reg_payment_details set payment_status='approved', document_status='approved',application_status='approved',application_comment='dashboard' where gid='$gid' and uid='$registration_id' and `show`='IGJME'";
	$result_update=$conn ->query($update_query);
		
	$sqlmx="select contact_person,region from exh_reg_general_info where uid='$registration_id' and 
	event_selected='igjme23' AND event_for='IGJME' order by id desc limit 0,1";
	...
	$sqlm = "SELECT * FROM `exh_registration` WHERE uid='$registration_id' AND gid='$gid' AND `show`='IGJME' AND event_selected='igjme23' order by exh_id desc limit 0,1";
	...
	<table width="100%" border="0" background="#fff" cellpadding="0" cellspacing="0">
		<tr>
	    	...
	    	<td align="right"><a href="https://gjepc.org/iijs-signature/"><img src="https://registration.gjepc.org/images/signature_logo_new_2023.jpg"/></a></td>
	    </tr>  
		...
		<tr>
	    	...
	    	<p>Thank you for applying Online for IGJME 2023 Exhibitor Registration. Please note your application is under approval process.</p>  	
	    	</td>
	    </tr>
