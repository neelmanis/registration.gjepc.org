...
if(!isset($_SESSION['uid'])){ header("location:international_visitor_event_registration.php");	exit; }
if(!isset($_SESSION['show']) && $_SESSION['show']==""){ header("location:international_visitor_event_registration.php");	exit; }
$uid = $_SESSION['uid'];
$eid = $_SESSION['eid']; 
...
<div class="container">
	...
	<?php
		...
		//	echo '<pre>'; print_r($data); exit;
            ...
            /*Global Table Start */
		$first_name = str_replace(array('&amp;','&AMP;'), '&', $data['first_name']);
		$last_name = str_replace(array('&amp;','&AMP;'), '&', $data['last_name']);
		...
		$company_name = str_replace(array('&amp;','&AMP;'), '&', $data['company_name']);
		$isSecondary = "N";
		$email = trim($data['email']);
		$country_code = trim($data['country']);
		$country = strtoupper(getCountryName($country_code,$conn));
	    ...
	    if($checkGlobalCount>0){
		 ...
		 `secondary_mobile`='',`isSecondary`='$isSecondary',`designation`='$visitorDesignation',`company`='$company_name',`photo_url`='$photo_url',`covid_report_status`='pending',country='$country',state='$state',city='$city' WHERE `registration_id`='$uid' AND `visitor_id`='$eid' AND `email`='$email' AND `participant_Type`='INTL' AND `event`='$shortcode' ";
		$updateGlobalResult = $conn->query($updateGlobal);	
		} else { 
		$insertGlobal = "INSERT INTO globalExhibition  SET `uniqueIdentifier`='$uniqueIdentifier',`registration_id`='$uid',`visitor_id`='$eid',`fname`='$name',`mobile`='$visitorMobile',`email`='$email',`isSecondary`='$isSecondary',`designation`='$visitorDesignation',`company`='$company_name',`photo_url`='$photo_url',`participant_type`='INTL',`covid_report_status`='pending',`status`='P',`event`='$shortcode',country='$country',state='$state',city='$city'";
