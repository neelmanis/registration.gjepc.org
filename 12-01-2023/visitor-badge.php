...
if($participant_Type == "CONTR" && $action =="generateBadge" ){
    ...
    $sql = "SELECT * FROM gjepclivedatabase.globalExhibition  WHERE participant_Type ='$participant_Type' AND visitor_id='$visitor_id' AND registration_id='$registration_id' and event!='iijstritiya23'";
}
...
$save = file_put_contents($_SERVER['DOCUMENT_ROOT'].'/images/badges/'.$filename.'.pdf', $dompdf->output()); 
if($save){
    if($action =="generateBadge"){
        $dompdf->stream($filename, array("Attachment" => 1));
        //header("location: /images/badges/".$filename.".pdf");
