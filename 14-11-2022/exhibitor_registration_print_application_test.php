...
_pan_card_number=".$billing_pan_card_no."&billing_aadhar_card_number=".$billing_aadhar_card_no."&tree_count=".$tree_qty;
	...
	if($show=="signature"){
     ...
     $shows = "IIJS SIGNATURE 2023";
		...
		*/
		...
		<?php if($amountUnpaid>=500){ ?>
				<button class="cta btn fade_anim  " id="save" onclick="return savePayment.save()">Click here for Payment</button>
				
				<?php 		
		if($countx1 > 0 && $countx1 != '') { ?>
       <!--  <div style="float:right; display:block; font-size:35px; font-weight:normal;background-color:#924b77"> -->
        <?php if($amountUnpaid<=500){?>
        	<a href="manual_signature/index.php?uid=<?php echo $uid;?>&gid=<?php echo $getgid;?>&section=<?php echo $rexh['Exhibitor_Section'];?>" style="width:420px;margin-left:20px" class="cta btn fade_anim ">Online Exhibitor Manual</a>

			<!--<a href="javascript: void(0)" onclick = "second_alert()" class="newMaroonBtn">Online Exhibitor Manuals</a>-->
            <?php } else { ?>

            <a href="javascript: void(0)" onclick = "balance_alert()" class='cta btn fade_anim ' style="width:420px;margin-left:20px">Online Exhibitor Manual</a>
        ...
        ?>
        <a class="cta btn fade_anim " data-toggle="modal" data-target="#exampleModal" href="https://sankalptaru.org/hyderabad-community-plantation/" target="_blank" >Do you want to plant a tree ? </a>
       
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Plant Donation</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
      	<form class="form" id='plant_registration'>
				 <input type="hidden" name="action" value="plant_registration">
      			...
      			<div class="form-group">
		        	...
		        	<input type="text" class="form-control" name="plant_rate" id="plant_rate" value="155" readonly  >
		        	<label for="plant_rate" class=""></label>
      			</div>
      			<div class="form-group">
      				<label>Total Price:</label>
		        	<input type="text" class="form-control" name="tree_amount" id="tree_amount" value="5000" readonly >
		        	<label for="tree_amount" class=""></label>
      			</div>
				<div class="form-group">
      				<label>Name on Tree:</label>
		      		<input type="text" class="form-control" name="tree_name" id="tree_name">
				...
				<div class="form-group">
		      		...
		      		<input type="text" class="form-control" name="billing_postcode" id="billing_postcode" maxlength="6" minlength="6" autocomplete="off"/>
		        	<label for="billing_postcode" class=""></label>
      			</div> 
				<div class="form-group">
      				<label>Billing PAN Card No:</label>
		      		<input type="text" class="form-control" name="billing_pan_card_no" id="billing_pan_card_no" maxlength="10" autocomplete="off"/>
		        	<label for="billing_pan_card_no" class=""></label>
      			</div>
				<div class="form-group">
      				<label>Billing Aadhar Card No:</label>
		      		<input type="text" class="form-control" name="billing_aadhar_card_no" id="billing_aadhar_card_no"/>
		        	<label for="billing_aadhar_card_no" class=""></label>
      			</div>
      			<div class="modal-footer d-flex justify-content-center">
        <button type="button" class="btn cta btn-secondary" data-dismiss="modal">Close</button>
        <!-- <a  href="https://sankalptaru.org/hyderabad-community-plantation/" target="_blank" type="button" class=" btn cta fade_anim">Pay Now</a> -->
	...
	<?php
		...
		<input type="hidden" name="show_type" id="show_type" value="<?php echo $show; ?>">
		<input type="hidden" name="gid" id="gid" value="<?php echo $getgid;?>">
		<?php $amountUnpaid = 600; if($amountUnpaid>=500){ ?>
				<button class="cta fade_anim d-table mx-auto" id="save" onclick="return savePayment.save()">Click here for Payment</button>
		<?php } else { echo '';}?>
	</form>		
	</div>
	...
	{
	if($result_igjme['event_selected']=="signature23"){ $event_for="IIJS SIGNATURE 2023"; } 
	if($result_igjme['event_selected']=="iijs"){ $event_for="IIJS PREMIERE 2022"; $event_show = "IIJS PREMIERE 2022";} 
...
function plantAction(){
		$('#plant_registration').val('please wait...');
		$('#plant_registration').attr('disabled',true);
	    var form = $('#regisForm');
	    var formdata = false;
	    if (window.FormData){
	    ...
	    $.ajax({
		    ...
		    success:function(data){
			    ...
			    if(data.status == 'success'){
			    	// $("#plant_registration")[0].reset();
	
					window.location.href= data.response_url;
					$('#plant_registration').val('Pay Now');		
					$('#plant_registration').attr('disabled',false);		
