...
if($action == 'email_id'){
	...
	if($count>0) 
	{
      ...
      $eid = intval(filter($row['eid']));
		...
		$checkHistory = "SELECT * FROM `ivr_registration_history` WHERE visitor_id='$eid' AND registration_id='$uid' AND payment_made_for ='signature23'";
	    ...
	    $data = array("status"=>'error',"message"=>"You are already registered for this show.");
