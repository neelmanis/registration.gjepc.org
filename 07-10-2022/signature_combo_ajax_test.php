...
if(strtoupper($_SESSION['COUNTRY'])=="IN")
{
	...
	else if($section=="jewellery_findings")
	{
		$charge="22000";
	}
...
{
	$sql="select * from signature_area_master where area in ('9','12','16','18','27','36','45','54','72','108') and area<$selected_area";
}
...
{
	$sql="select * from signature_area_master where area in ('9','12','16','18','27','36','45','54','72','108')";
}
...
if(isset($_POST['actiontype']) && $_POST['actiontype']=="optionValue"){
	...
	$lastYearArea=$_POST['lastYearArea'];  /// IIJS AREA
	$lastYearSignatureArea=$_POST['lastYearSignatureArea'];  /// SIGNATURE  AREA
	...
	{
		$sql="select * from signature_area_master where area in ('12','24','36','48') and area>$lastYearSignatureArea";	
	}
	else if($section=="signature_club" && $option=="Less area as previous year")
	{
		$sql="select * from signature_area_master where area in ('12','24','36','48') and area<$lastYearSignatureArea";	
	...
	{
		$sql="select * from signature_area_master where area in ('9','12','16','18','27','36','45','54','72','108') and area<$lastYearSignatureArea";
	}
	else if($option=="More area than previous year Signature" && $section!="signature_club")
	{
		$sql="select * from signature_area_master where area in ('9','12','16','18','27','36','45','54','72','108') and area>$lastYearSignatureArea";
	...
	{
		 $sql="select * from signature_area_master where area in ('9','12','16','18','27','36','45','54','72','108') and area<=$lastYearArea";
	}
	///////new code same as iijs
	// if($option=="Less area as previous year" || $option=="Less area as previous year at diffrent location")
	// {
	// 	$sql="select * from signature_area_master where area in ('9','18','27','36','54','72','108') and area<$lastYearSignatureArea";
	// }
	// else if($option=="More area than previous year Signature" && $section=="machinery" && $lastYearSignatureArea=="9")
	// {
	// 	 $sql="select * from signature_area_master where area in (18,27)";
	// }
	// else if($option=="More area than previous year Signature" && $lastYearSignatureArea=="6")
	// {
	// 	$sql="select * from signature_area_master where area in (18)";
	// }
	// else if($option=="More area than previous year Signature" && $lastYearSignatureArea=="9")
	// {
	// 	$sql="select * from signature_area_master where area in (18,27,36,54,72,108)";
	// }
	// else if($option=="More area than previous year Signature" && $lastYearSignatureArea=="12")
	// {
	// 	$sql="select * from signature_area_master where area in (27)";
	// }
	// else if($option=="More area than previous year Signature" && $lastYearSignatureArea=="24")
	// {
	// 	$sql="select * from signature_area_master where area in (45)";
	// }
	// else if($option=="More area than previous year Signature" && $lastYearSignatureArea=="18")
	// {
	// 	$sql="select * from signature_area_master where area in (27,36,54,72,108)";
	// }
	// else if($option=="More area than previous year Signature" && $lastYearSignatureArea=="27")
	// {
	// 	$sql="select * from signature_area_master where area in (36,54,72,108)";
	// }
	// else if($option=="More area than previous year Signature" && $lastYearSignatureArea=="36")
	// {
	// 	$sql="select * from signature_area_master where area in (54,72,108)";
	// }
	// else if($option=="More area than previous year Signature" && $lastYearSignatureArea=="45")
	// {
	// 	$sql="select * from signature_area_master where area in (54)";
	// }
	// else if($option=="More area than previous year Signature" && $lastYearSignatureArea=="54")
	// {
	// 	$sql="select * from signature_area_master where area in (72,108)";
	// }
	// else if($option=="More area than previous year Signature" && $lastYearSignatureArea=="72")
	// {
	// 	$sql="select * from signature_area_master where area in (108)";
	// } 
	// else if($option=="Same stall position as off IIJS premiere 2022")
	// {
	// 	 $sql="select * from signature_area_master where area in ('9','12','16','18','27','36','45','54','72','108') and area<=$lastYearArea";
	...
	// else
	// {
	// 	$sql="select * from signature_area_master where area=$lastYearSignatureArea";
	// }
	// echo $sql;
	$query=$conn->query($sql);
	$num=$query->num_rows;
	...
	while($result=$query->fetch_assoc()){
	?>
     <?php if($option=="Same stall position as off IIJS premiere 2022"){ ?>
		<option value="<?php echo $result['area'];?>" <?php if($result['area']==$lastYearArea){?> selected="selected" <?php }?>><?php echo $result['area'];?></option>
    <?php  }else{ ?>
		<option value="<?php echo $result['area'];?>" <?php if($result['area']==$lastYearSignatureArea){?> selected="selected" <?php }?>><?php echo $result['area'];?></option>
    <?php }?>
	...
	//if($option=="Same stall position as off IIJS premiere 2022"){ 
		...
		$query_section=$conn->query($sql_section);

		$last_query = "select * from exh_registration where uid='$uid' AND `show`='IIJS PREMIERE 2022' AND last_yr_section_flag='1' AND  curr_last_yr_check='N' order by exh_id desc limit 0,1";
		$query3=$conn->query($last_query);
		$exh_registration_result=$query3->fetch_assoc();
		$num3=$query3->num_rows;
		if($num3 > 0){
			echo "<option value=''>------Select Section----</option>";
			while($result3=$query_section->fetch_assoc()){ ?>
				<?php if($option=="Same stall position as off IIJS premiere 2022"){ ?>
					<option value="<?php  if($result3['section']==$exh_registration_result['section']){echo $result3['section'];}?>" <?php if($result3['section']==$exh_registration_result['section']){?> selected="selected"   <?php }?>><?php echo $result3['section_desc'];?></option>
				<?php  }  if($option != "Same stall position as off IIJS premiere 2022" && $result3['section'] == $section) { ?>
					<option value="<?php  if($result3['section'] == $section){ echo $result3['section']; }?>" <?php if($result3['section'] == $section){?> selected="selected" <?php }?>><?php echo $result3['section_desc'];?></option>
		<?php	} //else{ ?>
